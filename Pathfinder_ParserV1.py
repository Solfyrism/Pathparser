...
async def daily_task_at_midnight():
    ...
    for guild in guilds:
        ...
        for request in old_requests:
            await EventCommand.clear_group(self, guild.id, request[0])
        if bool(now.dst()) != dst[0]:
            await EventCommand.fix_dst(self, guild.id)


# Calculate the time until 8:00 PM and start the loop then
@daily_task_at_midnight.before_loop
async def before_midnight():
    now = datetime.datetime.now(pytz.timezone('US/Eastern'))  # Ensure the time is in your timezone
...
def convert_to_unix(military_time: str, timezone_str: str) -> str:
    ...
    time_combined = datetime.datetime(current_date.year, current_date.month, current_date.day, hours, minutes)
...
async def create(ctx: commands.Context, kingdom: str, password: str, government: str, alignment: str):
    ...
    if alignment_validity is not None and kingdom_validity is None and government_validity is not None:
        ...
        await EventCommand.create_kingdom(self, kingdom, password, government, alignment, economy, loyalty, stability,
                                          guild_id, author)
        await ctx.response.send_message(content=status)
    cursor.close()
    db.close()
...
async def modify(ctx: commands.Context, old_kingdom: str, new_kingdom: str, old_password: str, new_password: str,
    ...
    if result is None:
        ...
        await EventCommand.modify_kingdom(self, old_kingdom, new_kingdom, new_password, new_government, new_alignment,
                                          guild_id, author)
        ...
        if character_info is None:
            ...
            if gold_value < 0:
                ...
                if result is not None and result[1] == password:
                    ...
                    if stabilization_points >= 0:
                        await EventCommand.adjust_stabilization_points(self, kingdom, amount, guild_id, author,
                                                                       character_info[0])
                        await interaction.response.send_message(
...
async def blueprint_add(ctx: commands.Context, building: str, build_points: int, lots: int, economy: int, loyalty: int,
    ...
    if result is None:
        ...
        await EventCommand.create_blueprint(self, building, build_points, lots, economy, loyalty, stability, fame,
...
async def blueprint_modify(ctx: commands.Context, building: str, build_points: int, lots: int, economy: int,
    ...
    if result is not None:
        ...
        await EventCommand.modify_blueprint(self, building, build_points, lots, economy, loyalty, stability, fame,
                                            unrest, corruption, crime, productivity, law, lore, society, danger,
...
async def kingdom_modifiers(ctx: commands.Context, kingdom: str, control_dc: int, economy: int, loyalty: int,
    ...
    if result is not None:
        ...
        await EventCommand.customize_kingdom_modifiers(self, kingdom, control_dc, economy, loyalty, stability, fame,
...
async def settlement_modifiers(ctx: commands.Context, kingdom: str, settlement: str, corruption: int, crime: int,
    ...
    if result is not None:
        ...
        await ctx.response.send_message(status)
        await EventCommand.custom_settlement_modifiers(self, kingdom, settlement, corruption, crime, productivity, law,
...
async def improvement_add(ctx: commands.Context, improvement: str, build_points: int, road_multiplier: int,
    ...
    db.close()
    if result is None:
        await EventCommand.add_hex_improvements(self, improvement, build_points, road_multiplier, economy, loyalty,
...
async def improvement_modify(ctx: commands.Context, old_improvement: str, new_improvement: str, new_build_points: int,
    ...
    if result1 is not None:
        if result2 is None:
            ...
            await ctx.response.send_message(status)
            await EventCommand.modify_hex_improvement(self, old_improvement, new_improvement, new_build_points,
    ...
    if player_info is not None:
        ...
        if job_name == 'None' and misc_milestones == 0:
            ...
            if current_level is None:
                ...
                await EventCommand.adjust_milestones(self, character_name, milestone_total, remaining, character_level,
                                                     guild_id, author)
...
async def gold_adjust(ctx: commands.Context, character_name: str, amount: typing.Optional[float],
    ...
    if amount is None and effective_gold is None and lifetime_gold is None:
        ...
        if player_info is None:
            ...
            await EventCommand.gold_change(self, guild_id, author, author_id, player_info[3], gold_info[3],
...
async def undo_transaction(ctx: commands.Context, transaction_id: int):
    ...
    if transaction_info is not None:
        ...
        await EventCommand.undo_transaction(self, guild_id, transaction_id, gold, effective_gold, max_effective_gold,
        ...
        if related_transaction_info is not None:
            ...
            embed.add_field(name=f"**{related_transaction_info[2]}'s Transaction Info:**",
                            ...
                            inline=False)
            await EventCommand.undo_transaction(self, guild_id, related_transaction_info[6], gold, effective_gold,
    ...
    if session_simple is None:
        ...
        if gold is not None and gold < 0 or easy is not None and easy < 0 or medium is not None and medium < 0 or hard is not None and hard < 0 or deadly is not None and deadly < 0 or flux is not None and flux < 0 or trials is not None and trials < 0:
            ...
            if party_reward is not None:
                ...
                if session_simple[16] is None:
                    ...
                    reward_thread = await reward_msg.create_thread(
                        ...
                        auto_archive_duration=10080, reason=f"{party_reward}")
            ...
            for player in session_complex:
                ...
                #                    DONE WITH GOLD
                difference = round(difference, 2)
                await EventCommand.session_rewards(self, player[0], guild_id, player[1], true_level,
                ...
                await EventCommand.gold_change(self, guild_id, player[0], player[6], player[1], difference, difference,
                                               gold, 'Session Reward', 'Session Reward')
                await EventCommand.update_session_log_player(self, guild_id, session_id, player[1], rewarded, trials,
                ...
                logging_thread = guild.get_thread(player_info[25])
                await logging_thread.send(embed=logging_embed)
            await EventCommand.update_session_log(self, guild_id, session_id, gold, flux, easy, medium, hard, deadly,
    ...
    if character_name is None and cleanse is None:
        ...
        if player_id_info is not None:
            if player_id_info[1] is not None or 'worldanvil' in player_id_info[2]:
                try:
                    e = None
                    await EventCommand.create_bio(self, guild_id, player_id_info[0], player_id_info[1],
                                                  player_id_info[2])
                except Exception as e:
                    print(e)
                    pass
            ...
            await EventCommand.log_character(self, guild_id, character_name, bio_message.id, character_log_message.id,
                                             thread.id)
            if e is None:
...
async def customize(ctx: commands.Context, character_name: str, destination: discord.app_commands.Choice[int],
    ...
    if player_info is None:
        ...
        await EventCommand.customize_characters(self, guild_id, author, player_info[3], destination_name,
...
async def modify(ctx: commands.Context, kingdom: str, password: str, leader: str, title: str, modifier: int):
    ...
    while kingdom_modifier is None:
        ...
        await EventCommand.modify_leader(self, kingdom, leader, title, modifier, column, economy_modifier,
...
async def claim(ctx: commands.Context, character_name: str, amount: float, reason: str):
    ...
    if amount <= 0:
        ...
        if player_info is None:
            ...
            gold_info = gold_calculation(player_info[7], player_info[6], player_info[13], player_info[14],
                                         player_info[15], amount)
            await EventCommand.gold_change(self, guild_id, author, author_id, character_name, gold_info[3],
    ...
    if expenditure <= 0:
        ...
        if player_info[0] is None:
            ...
            if player_info[13] >= abs(expenditure):
                ...
                await EventCommand.gold_change(self, guild_id, author, author_id, character_name, expenditure,
                    ...
                    while True:
                        try:
                            ...
                            if reaction.emoji == u"\u2705":
                                ...
                                await EventCommand.gold_change(self, guild_id, author, author_id, character_from,
                                ...
                                await EventCommand.gold_change(self, guild_id, send_info[0], send_info[1], send_info[2],
                                ...
                                await EventCommand.gold_transact(self, transaction_id_from[0], transaction_id_to[0],
                                                                 guild_id)
                                await EventCommand.gold_transact(self, transaction_id_to[0], transaction_id_from[0],
                                                                 guild_id)
...
async def glorify(ctx: commands.Context, character: str, fame: int, prestige: int, reason: typing.Optional[str]):
    ...
    if player_info is not None:
        ...
        await logging_thread.send(embed=logging_embed)
        await EventCommand.glorify(self, guild_id, author, character, fame + player_info[1], prestige + player_info[2],
                                   reason)
    else:
...
async def create(interaction: discord.Interaction, session_name: str, session_range: discord.Role, player_limit: int,
    ...
    if hammer_timing == "<t:":
        ...
        hour = "<t:" + timing + ":t>"
        arrival = "<t:" + timing + ":R>"
    await EventCommand.create_session(self, author, session_name, session_range_name, session_range_id, play_location,
...
async def edit(interaction: discord.Interaction, session_id: int, session_range: typing.Optional[discord.Role],
    ...
    if info is not None:
        ...
        print(footer_text)
        await EventCommand.edit_session(self, guild_id, author, session_id, session_name, session_range_name,
                ...
                if randomizer == 0 and player_1 is None and player_2 is None and player_3 is None and player_4 and player_5 is None and player_6 is None and specific_character is None:
                    ...
                    if player_1 is not None:
                        ...
                        if player_info is None and signups_information is None:
                            ...
                            await EventCommand.accept_player(self, guild_id, session_info[0], session_id,
                    ...
                    try:
                        ...
                        if reaction.emoji == u"\u2705":
                            ...
                            mythic_information = mythic_calculation(guild_id, player_info[7], player_info[11],
                                                                    -abs(reward_info[5]))
                            await EventCommand.session_rewards(self, author, guild_id, player_info[2],
                            ...
                            await EventCommand.gold_change(self, guild_id, player_info[0], player_info[1],
                            ...
                            cursor.close()
                            db.close()
                            await EventCommand.remove_player(self, guild_id, session_id, reward_info[0], reward_info[1],
                                                             author)
    cursor.close()
    db.close()
        ...
        if session_info[7] == 1:
            ...
            if session_players == []:
                ...
                for player in session_players:
                    ...
                    await EventCommand.session_rewards(self, author, guild_id, player[2], level_info[0],
                    ...
                    await EventCommand.gold_change(self, guild_id, player[0], player[1], player[2], gold_info[3],
                                                   gold_info[3], gold, 'Session Reward', 'Session Reward')
                    await EventCommand.session_log_player(self, guild_id, session_id, player_info[0], player_info[1],
                ...
                await EventCommand.session_log(self, guild_id, session_id, gold, 10, easy, medium, hard, deadly, trials,
    ...
    if session_info is not None:
        ...
        if player_1 is not None and player_1_reward is not None:
            ...
            if session_player_info is not None:
                ...
                character_info = cursor.fetchone()
                await EventCommand.session_endowment(self, author, guild_id, session_id, player_1.name, player_1_reward,
                                                     character_info[0])
        ...
        if player_2 is not None and player_2_reward is not None:
            ...
            if session_player_info is not None:
                cursor.execute(f"""SELECT Character_Name, Thread_ID FROM Player_Characters WHERE Character_Name = ?""",
                               (session_player_info[0],))
                character_info = cursor.fetchone()
                await EventCommand.session_endowment(self, author, guild_id, session_id, player_2.name, player_2_reward,
                                                     character_info[0])
                source = f"Personal reward for Session ID: {session_id}"
                logging_embed = log_embed(session_player_info[0], author, None, None, None, None, None, None, None,
                                          None, None, None, None, None, None, None, None, None, None, None,
        ...
        if player_3 is not None and player_3_reward is not None:
            ...
            if session_player_info is not None:
                cursor.execute(f"""SELECT Character_Name, Thread_ID FROM Player_Characters WHERE Character_Name = ?""",
                               (session_player_info[0],))
                character_info = cursor.fetchone()
                await EventCommand.session_endowment(self, author, guild_id, session_id, player_3.name, player_3_reward,
                                                     character_info[0])
                source = f"Personal reward for Session ID: {session_id}"
                logging_embed = log_embed(session_player_info[0], author, None, None, None, None, None, None, None,
                                          None, None, None, None, None, None, None, None, None, None, None,
        ...
        if player_4 is not None and player_4_reward is not None:
            ...
            if session_player_info is not None:
                cursor.execute(f"""SELECT Character_Name, Thread_ID FROM Player_Characters WHERE Character_Name = ?""",
                               (session_player_info[0],))
                character_info = cursor.fetchone()
                await EventCommand.session_endowment(self, author, guild_id, session_id, player_4.name, player_4_reward,
                                                     character_info[0])
                source = f"Personal reward for Session ID: {session_id}"
                logging_embed = log_embed(session_player_info[0], author, None, None, None, None, None, None, None,
                                          None, None, None, None, None, None, None, None, None, None, None,
        ...
        if player_5 is not None and player_5_reward is not None:
            ...
            if session_player_info is not None:
                cursor.execute(f"""SELECT Character_Name, Thread_ID FROM Player_Characters WHERE Character_Name = ?""",
                               (session_player_info[0],))
                character_info = cursor.fetchone()
                await EventCommand.session_endowment(self, author, guild_id, session_id, player_5.name, player_5_reward,
                                                     character_info[0])
                source = f"Personal reward for Session ID: {session_id}"
                logging_embed = log_embed(session_player_info[0], author, None, None, None, None, None, None, None,
                                          None, None, None, None, None, None, None, None, None, None, None,
        ...
        if player_6 is not None and player_6_reward is not None:
            ...
            if session_player_info is not None:
                cursor.execute(f"""SELECT Character_Name, Thread_ID FROM Player_Characters WHERE Character_Name = ?""",
                               (session_player_info[0],))
                character_info = cursor.fetchone()
                await EventCommand.session_endowment(self, author, guild_id, session_id, player_6.name, player_6_reward,
                                                     character_info[0])
                source = f"Personal reward for Session ID: {session_id}"
                logging_embed = log_embed(session_player_info[0], author, None, None, None, None, None, None, None,
                                          None, None, None, None, None, None, None, None, None, None, None,
    ...
    if session_info is None:
        ...
        if validate_recipient is not None:
            ...
            if previous_rewards is not None:
                    ...
                    while True:
                        try:
                            ...
                            if reaction.emoji == u"\u2705":
                                ...
                                cursor.close()
                                db.close()
                                await EventCommand.remove_player(self, guild_id, session_id, previous_rewards[0],
                                ...
                                await EventCommand.session_rewards(self, validate_recipient[0], guild_id,
                                                                   ...
                                                                   session_id)
                                await EventCommand.gold_change(self, guild_id, validate_recipient[0],
                                ...
                                cursor.close()
                                db.close()
                                await EventCommand.session_log_player(self, guild_id, session_id, validate_recipient[0],
                ...
                await EventCommand.gold_change(self, guild_id, validate_recipient[0], validate_recipient[1],
            ...
            if character_info is not None:
                ...
                if level_range_info is None or session_info[7] == 4:
                    ...
                    if participation is None and signups is None:
                        await EventCommand.session_join(self, guild_id, session_info[0], session_id, author, author_id,
                    ...
                    if level_range_validation != 1:
                        ...
                        signups = cursor.fetchone()
                        if participation is None and signups is None:
                            await EventCommand.session_join(self, guild_id, session_info[0], session_id, author,
...
async def timesheet(ctx: commands.Context, day: discord.app_commands.Choice[int], timezone: str, start_time: str,
    ...
    if change_value >= 3:
        cursor.close()
        db.close()
        await EventCommand.clear_timesheet(self, guild_id, change_value, day_value, ctx.user.name)
        if change_value == 3:
            embed = discord.Embed(
    ...
    if validation == 0:
        ...
        start_time_unix = datetime.datetime.fromisoformat(str(start_datetime))
        end_time_unix = datetime.datetime.fromisoformat(str(end_datetime))
        if change_value == 1:
            embed = discord.Embed(
                title="Time Sheet Update",
                description=(
...
async def request(ctx: commands.Context, character_name: str, group_name: str, description: str,
    ...
    if player_info is not None and choice == 1:
        ...
        if group_info is not None:
            ...
            cursor.close()
            db.close()
            await EventCommand.group_request(self, guild_id, ctx.user.name, character_name, group_name, choice,
                                             description)
