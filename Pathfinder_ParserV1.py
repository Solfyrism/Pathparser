...
async def daily_task_at_midnight():
    ...
    for guild in guilds:
        ...
        dst = cursor.fetchone()
        cursor.execute(f"SELECT Group_ID, Created_Date from Sessions_Group where Created_Date < DATETIME(CURRENT_DATE, '-28 days')")
...
def fetch_timecard_data_from_db(guild_id, player_name, day, utc_offset):
    ...
    if utc_offset > 0:
        ...
        cursor.execute(f"SELECT {select_clause} FROM Player_Timecard PT1 Left Join Player_Timecard PT2 on PT1.Player_Name = PT2.Player_Name where PT1.Player_Name = ? and PT1.Day = ? AND PT2.Day = ?", (player_name, day_two, day))
        row = cursor.fetchone()
    elif utc_offset < 0:
        select_columns = []
        for col in time_labels:
        ...
        cursor.execute(f"SELECT {select_clause} FROM Player_Timecard PT1 Left Join Player_Timecard PT2 on PT1.Player_Name = PT2.Player_Name where PT1.Player_Name = ? and PT1.Day = ? AND PT2.Day = ?", (player_name, day, day_two))
        row = cursor.fetchone()
...
async def group_id_autocompletion(interaction: discord.Interaction, current: str) -> typing.List[app_commands.Choice[str]]:
    data = []
    guild_id = interaction.guild_id
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    current = unidecode(str.title(current))
    cursor.execute(f"Select Group_ID, Group_Name  FROM Sessions_Group WHERE Group_Name LIKE ? Limit 15", (f"%{current}%",))
    session_list = cursor.fetchall()
    for test_text in session_list:
        if current in str(test_text[0]) or str.lower(current) in str.lower(test_text[1]):
            name_result = f"{test_text[0]}: {test_text[1]}"
        ...
        if gold is not None and gold < 0 or easy is not None and easy < 0 or medium is not None and medium < 0 or hard is not None and hard < 0 or deadly is not None and deadly < 0 or flux is not None and flux < 0 or trials is not None and trials < 0:
            ...
            if party_reward is not None:
                ...
                if session_simple[16] is None:
                    ...
                    reward_thread = await reward_msg.create_thread(name=f"Session name: {session_simple[1]} Party Rewards, Session ID: {session_simple[0]}", auto_archive_duration=10080, reason=f"{party_reward}")
...
async def create(interaction: discord.Interaction, session_name: str, session_range: discord.Role, player_limit: int, play_location: str, game_link: typing.Optional[str], group_id: typing.Optional[int], hammer_time: str, overview: str, description: str, plot: str = '9762aebb-43ae-47d5-8c7b-30c34a55b9e5',  overflow: discord.app_commands.Choice[int] = 1):
    ...
    try:
        ...
        if group_id is not None:
            ...
            cursor.execute(f"Select SG.Group_Name, SP.Group_ID, SP.Player_Name, PC.Player_ID  from Sessions_Presign SP left join Player_Characters PC on SP.Player_Name =  PC.Player_Name left join Sessions_group SG on SP.group_id = SG.group_id where SP.Group_ID = ? group by player_id", (group_id,))
        ...
        message = msg.id
        thread = await msg.create_thread(name=f"{info[0]}: {session_name}", auto_archive_duration=10080, reason=f"{description}")
        ...
        if group_id is not None:
            ...
            if group_base is not None:
                ...
                for player in group_base:
                    session_text += f'<@{player[3]}>'
        msg = await session_channel.send(content=session_text, embed=embed, allowed_mentions=discord.AllowedMentions(roles=True))
        thread = await msg.create_thread(name=f"Session name: {session_name} Session ID: {info[0]}", auto_archive_duration=10080, reason=f"{description}")
        message = msg.id
        await interaction.response.send_message(f"Session created! Session ID: {info[0]}.", ephemeral=True)
        await EventCommand.create_session_message(self, {info[0]}, message, thread.id, guild_id)
        cursor.close()
...
async def reward(interaction: discord.Interaction, session_id: int, gold: float, easy: int = 0, medium: int = 0, hard: int = 0, deadly: int = 0, trials: int = 0, reward_all: str = None, fame: int = 2, prestige: int = 2, party_reward: str = None, overview: str = None):
    ...
    #                Obtaining Character Information
                ...
                if party_reward is not None:
                    reward_thread = await reward_msg.create_thread(name=f"Session name: {session_info[1]} Party Rewards, Session ID: {session_id}", auto_archive_duration=10080, reason=f"{party_reward}")
                    reward_thread_id = reward_thread.id
...
async def timesheet(ctx: commands.Context, day: discord.app_commands.Choice[int], timezone: str, start_time: str, end_time: str, change: discord.app_commands.Choice[int] = 1):
    ...
    if change > 2:
        cursor.close()
        db.close()
        await EventCommand.clear_timesheet(self, guild_id, change, day_value, ctx.user.name)
        if change == 3:
    ...
    if change < 3:
        ...
        cursor.close()
        db.close()
        validation = await EventCommand.timesheet(self, guild_id, ctx.user.name, timezone, start_day, start_hours, start_minutes, end_day, end_hours, end_minutes, change)
        print(validation)
        if validation == 0:
...
async def availability(ctx: commands.Context, player: typing.Optional[discord.Member], day: discord.app_commands.Choice[int]):
    ...
    if day_value < 1 or day_value > 7:
        ...
        if player_utc_offset is not None:
            ...
            embed = discord.Embed(title=f"Player Error", description=f'{player} did not have a valid timecard!!', colour=discord.Colour.red())
            await ctx.response.send_message(embed=embed)
    cursor.close()
    db.close()
