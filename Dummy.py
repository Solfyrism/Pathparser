...
if not timezone_cache:
    print("Timezone cache is empty!")
else:
    print("Laura is the most beautiful woman in the world.")
...
async def on_ready():
    ...
    print("Starting the scheduled task at 8:00 PM...")
    daily_task_at_midnight.start()

@tasks.loop(hours=24)
async def daily_task_at_midnight():
    # Your code for what to do at 8:00 PM
    guilds = bot.guilds
    for guild in guilds:
        db = sqlite3.connect(f"Pathparser_{guild.id}.sqlite")
        cursor = db.cursor()
        cursor.execute(f"SELECT Search FROM Admin WHERE Identifier = 'DST'")
        dst = cursor.fetchone()
        cursor.execute(
            f"SELECT Group_ID, Created_Date from Sessions_Group where Created_Date < DATETIME(CURRENT_DATE, '-28 days')")
        old_requests = cursor.fetchall()
        tz = datetime.timezone('US/Eastern')
        now = datetime.now(tz)
        cursor.close()
        db.close()
        for request in old_requests:
            await EventCommand.clear_group(guild.id, request[0])
        if bool(now.dst()) != dst[0]:
            await EventCommand.fix_dst(guild.id)


# Calculate the time until 8:00 PM and start the loop then
@daily_task_at_midnight.before_loop
async def before_midnight():
    now = datetime.now(pytz.timezone('US/Eastern'))  # Ensure the time is in your timezone

    # Set the target time to 8:00 PM (20:00)
    target_time = now.replace(hour=0, minute=0, second=0, microsecond=0)

    # If it's already past 8:00 PM, calculate the time for the next day
    if now > target_time:
        target_time += datetime.timedelta(days=1)

    print(f"Target time: {target_time}")  # Debug print for target time
    seconds_until_8pm = (target_time - now).total_seconds()
    print(f"Sleeping for {seconds_until_8pm} seconds until Midnight...")

    try:
        await asyncio.sleep(seconds_until_8pm)  # Manually sleep for the calculated time
        print("Woke up, ready to run theMidnight!")
    except Exception as e:
        print(f"Error while waiting for Midnight: {e}")
...
def mythic_calculation(guild_id, true_level, trial_total, rewarded):
    ...
    cursor.execute(
        f"SELECT Tier, Trials, Trials_Required from AA_Trials  WHERE Trials <= {new_trials_total} ORDER BY Trials DESC  LIMIT 1")
    ...
    cursor.execute(
        f"SELECT Tier, Trials, Trials_Required from AA_Trials  WHERE Tier = {true_tier} ORDER BY Trials DESC  LIMIT 1")
...
async def help(ctx: commands.Context):
    ...
    embed = discord.Embed(title=f"Kingdoms Help", description=f'This is a list of kingdom help commands',
                          colour=discord.Colour.blurple())
    embed.add_field(name=f'**Create Kingdom**',
                    value=f'This command allows a player to take the first step of kingdom management, creating their kingdom and allowing them to interact with it. NOTE: There is NO encryption. When creating a password do NOT use a personalized password.',
                    inline=False)
    embed.add_field(name=f'**Destroy Kingdom**',
                    value=f'This command will delete all properties of kingdom management, deleting it from the database.',
                    inline=False)
    embed.add_field(name=f'**Modify Kingdom**',
                    value=f'This command allows the kingdom leaders to modify the name of the kingdom.', inline=False)
    embed.add_field(name=f'**Display Kingdom**',
                    value=f'This command displays all kingdoms along with some basic information about them.',
                    inline=False)
    embed.add_field(name=f'**Detail Kingdoms**', value=f'This displays the detailed view of a singular kingdom.',
                    inline=False)
    embed.add_field(name=f'**BP**',
                    value=f'Modifies the build points allocated to a kingdom by a negative or positive value',
                    inline=False)
    embed.add_field(name=f'**SP**',
                    value=f'Modifies the [stabilization points](https://docs.google.com/document/d/1c_W0d-fDgQukteeX8fwNXv3egKD41b6FXtuSPurUoLg/edit?usp=sharing) allocated to a kingdom by a negative or positive value',
                    inline=False)
    embed.add_field(name=f'**Reference Links**',
                    value=f'[Government](https://www.aonprd.com/Rules.aspx?Name=Forms%20of%20Government&Category=Optional%20Kingdom%20Rules),[Build Points](https://www.d20pfsrd.com/gamemastering/other-rules/kingdom-building#TOC-Build-Points), [Decay](https://docs.google.com/document/d/1c_W0d-fDgQukteeX8fwNXv3egKD41b6FXtuSPurUoLg/edit?usp=sharing)')
...
async def create(ctx: commands.Context, kingdom: str, password: str, government: str, alignment: str):
    ...
    cursor.execute(
        f"""SELECT Alignment, Economy, Loyalty, Stability FROM AA_Alignment WHERE Alignment = '{alignment}'""",
        {'Alignment': alignment})
    alignment_validity = cursor.fetchone()
    cursor.execute(f"""SELECT Government FROM AA_Government WHERE Government = '{government}'""",
    ...
    if alignment_validity is not None and kingdom_validity is None and government_validity is not None:
        ...
        await EventCommand.create_kingdom(kingdom, password, government, alignment, economy, loyalty, stability,
                                          guild_id, author)
        await ctx.response.send_message(content=status)
    cursor.close()
    db.close()
...
async def destroy(ctx: commands.Context, kingdom: str, password: str):
    ...
    if result is not None and result[1] == password:
        ...
        await EventCommand.destroy_kingdom(kingdom, guild_id, author)
        await ctx.response.send_message(content=status)
    else:
...
@kingdom.command()
async def modify(ctx: commands.Context, old_kingdom: str, new_kingdom: str, old_password: str, new_password: str,
                 new_government: str, new_alignment: str):
    ...
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    cursor.execute(f"""SELECT Kingdom, Password FROM Kingdoms where Kingdom = '{old_kingdom}'""",
                   {'Kingdom': old_kingdom})
    result = cursor.fetchone()
    cursor.execute(
        f"""SELECT Alignment, Economy, Loyalty, Stability FROM AA_Alignment WHERE Alignment = '{new_alignment}'""",
        {'Alignment': new_alignment})
    alignment_validity = cursor.fetchone()
    cursor.execute(f"""SELECT Government FROM AA_Government WHERE Government = '{new_government}'""",
    ...
    if result is None:
        ...
        await EventCommand.modify_kingdom(old_kingdom, new_kingdom, new_password, new_government, new_alignment,
                                          guild_id, author)
...
async def display(ctx: commands.Context, current_page: int = 1):
    ...
    if decay[0]:  # IF THE SERVER HAS DECAY ON
        cursor.execute(
            f"""SELECT Kingdom, Government, Alignment, Control_DC, Build_Points, Stabilization_Points, Size, Population, Economy, Loyalty, Stability, Fame, Unrest, Consumption from Kingdoms WHERE ROWID BETWEEN {low} and {high}""")
...
async def detail(ctx: commands.Context, kingdom: str, custom_stats: bool = False):
    ...
    if decay[0]:
        if not custom_stats:
            cursor.execute(
                f"""SELECT Government, Alignment, Control_DC, Build_Points, Stabilization_Points, Size, Population, Economy, Loyalty, Stability, Fame, Unrest, Consumption from Kingdoms where Kingdom = '{kingdom}'""",
                {'Kingdom': kingdom})
            kingdom_info = cursor.fetchone()
            if kingdom_info is None:
...
async def bp(interaction: discord.Interaction, kingdom: str, password: str, character_name: str, amount: int):
    ...
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    cursor.execute(f"""SELECT Kingdom, Password, Build_Points FROM Kingdoms where Kingdom = '{kingdom}'""",
                   {'Kingdom': kingdom})
    result = cursor.fetchone()
    ...
    if character_info is None:
        ...
        if gold_value >= 0:
            ...
            if result is not None and result[1] == password:
                ...
                if build_points >= 0:
                    await EventCommand.adjust_build_points(kingdom, amount, guild_id, character_info[0], author)
                    await interaction.response.send_message(
...
async def sp(interaction: discord.Interaction, kingdom: str, password: str, character_name: str, amount: int):
    ...
    decay = cursor.fetchone()
    if decay[0]:  # IF THE SERVER HAS DECAY ON
        cursor.execute(f"""SELECT Kingdom, Password, Stabilization_Points FROM Kingdoms where Kingdom = '{kingdom}'""",
                       {'Kingdom': kingdom})
        result = cursor.fetchone()
        sql = f"""Select True_Character_Name, Gold from Player_Characters where Player_Name = ? and  Character_Name = ? or  Player_Name = ? and Nickname = ?"""
        val = (author, character_name, author, character_name)
        ...
        if character_info is None:
            ...
            if gold_value < 0:
                ...
                if result is not None and result[1] == password:
                    ...
                    if stabilization_points >= 0:
                        await EventCommand.adjust_stabilization_points(kingdom, amount, guild_id, author,
                                                                       character_info[0])
                        await interaction.response.send_message(
...
async def help(ctx: commands.Context):
    ...
    embed = discord.Embed(title=f"Overseer Help", description=f'This is a list of Overseer help commands',
                          colour=discord.Colour.blurple())
    embed.add_field(name=f'**blueprint_add**',
                    value=f'The command for an overseer to create a new blueprint for players to use..', inline=False)
    embed.add_field(name=f'**blueprint_remove**', value=f'This command removes blueprints from player usage.',
                    inline=False)
    embed.add_field(name=f'**blueprint_modify**', value=f'This command modifies a blueprint that is already in use.',
                    inline=False)
    embed.add_field(name=f'**kingdom_modifiers**',
                    value=f'This command adjusts the custom modifiers associated with a kingdom.', inline=False)
    embed.add_field(name=f'**settlement_modifiers**',
                    value=f'This command adjusts the custom modifiers associated with a settlement.', inline=False)
    embed.add_field(name=f'**settlement_decay**',
                    value=f'This command modifies the multiplier for stabilization points a settlement requires in order to build.',
                    inline=False)
    embed.add_field(name=f'**improvement_add**', value=f'This command adds a new hex improvement for players to build',
                    inline=False)
    embed.add_field(name=f'**improvement_remove**',
                    value=f'This command removes hex improvements from options players can build.', inline=False)
    embed.add_field(name=f'**improvement_modify**',
                    value=f'This command modifies hex improvements that are available to build, or have been built',
                    inline=False)
    embed.add_field(name=f'**kingdom_tables_rebalance**',
                    value=f'Forced the kingdom and settlement tables to rebalance.', inline=False)
    await ctx.response.send_message(embed=embed)


@overseer.command()
async def blueprint_add(ctx: commands.Context, building: str, build_points: int, lots: int, economy: int, loyalty: int,
                        stability: int, fame: int, unrest: int, corruption: int, crime: int, productivity: int,
                        law: int, lore: int, society: int, danger: int, defence: int, base_value: int,
                        spell_casting: int, supply: int, settlement_limit: int, district_limit: int, description: str):
    ...
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    cursor.execute(f"""SELECT building FROM Buildings_Blueprints where building = '{building}' LIMIT 1;""",
                   {'building': building})
    result = cursor.fetchone()
    cursor.close()
    db.close()
    if result is None:
        ...
        await EventCommand.create_blueprint(building, build_points, lots, economy, loyalty, stability, fame,
                                            unrest, corruption, crime, productivity, law, lore, society, danger,
                                            defence, base_value, spell_casting, supply, settlement_limit,
                                            district_limit, description, guild_id, author)
        await ctx.response.send_message(status)
...
async def blueprint_remove(ctx: commands.Context, building: str):
    ...
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    cursor.execute(f"""SELECT Building FROM Buildings_Blueprints WHERE Building = '{building}'""",
                   {'building': building})
    result = cursor.fetchone()
    cursor.close()
    db.close()
    ...
    if result is not None:
        ...
        await EventCommand.remove_blueprint(building, guild_id, author)
        await ctx.response.send_message(status)


@overseer.command()
async def blueprint_modify(ctx: commands.Context, building: str, build_points: int, lots: int, economy: int,
                           loyalty: int, stability: int, fame: int, unrest: int, corruption: int, crime: int,
                           productivity: int, law: int, lore: int, society: int, danger: int, defence: int,
                           base_value: int, spellcasting: int, supply: int, settlement_limit: int, district_limit: int,
                           description: str):
    ...
    if result is not None:
        ...
        await EventCommand.modify_blueprint(building, build_points, lots, economy, loyalty, stability, fame,
                                            unrest, corruption, crime, productivity, law, lore, society, danger,
                                            defence, base_value, spellcasting, supply, settlement_limit, district_limit,
                                            description, guild_id, author)
        await ctx.response.send_message(status)


@overseer.command()
async def kingdom_modifiers(ctx: commands.Context, kingdom: str, control_dc: int, economy: int, loyalty: int,
                            stability: int, fame: int, unrest: int, consumption: int):
    ...
    if result is not None:
        ...
        await EventCommand.customize_kingdom_modifiers(kingdom, control_dc, economy, loyalty, stability, fame,
                                                       unrest, consumption, guild_id, author)
        await ctx.response.send_message(status)


@overseer.command()
async def settlement_modifiers(ctx: commands.Context, kingdom: str, settlement: str, corruption: int, crime: int,
                               productivity: int, law: int, lore: int, society: int, danger: int, defence: int,
                               base_value: int, spellcasting: int, supply: int):
    ...
    cursor = db.cursor()
    cursor.execute(
        f"""SELECT Settlement FROM Settlements WHERE Settlement = '{settlement}' AND Kingdom = '{kingdom}'""")
    result = cursor.fetchone()
    cursor.close()
    db.close()
    if result is None:
    ...
    if result is not None:
        ...
        await ctx.response.send_message(status)
        await EventCommand.custom_settlement_modifiers(kingdom, settlement, corruption, crime, productivity, law,
                                                       lore, society, danger, defence, base_value, spellcasting, supply,
                                                       guild_id, author)
...
async def settlement_decay(ctx: commands.Context, kingdom: str, settlement: str, decay: int):
    ...
    if server_decay[0]:  # IF THE SERVER HAS DECAY ON
        cursor = db.cursor()
        cursor.execute(
            f"""SELECT Kingdom FROM Settlements where Kingdom = '{kingdom}' AND Settlement = '{settlement}'""",
        ...
        if result is None:
            ...
            return
        if result is not None:
            await EventCommand.settlement_decay_set(kingdom, settlement, decay, guild_id, author)
            await ctx.response.send_message(
                f"The settlement of {settlement} within the kingdom of {kingdom} has had it's decay set to {decay}!")
    if not server_decay[0]:
        await ctx.response.send_message(f"Decay is not enabled in this server!")


@overseer.command()
async def improvement_add(ctx: commands.Context, improvement: str, build_points: int, road_multiplier: int,
                          economy: int, loyalty: int, stability: int, unrest: int, consumption: int, defence: int,
                          taxation: int, cavernous: bool, coastline: bool, desert: bool, forest: bool, hills: bool,
                          jungle: bool, marsh: bool, mountain: bool, plains: bool, water: bool):
    ...
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    cursor.execute(f"""SELECT Improvement from Hexes_Improvements where Improvement = '{improvement}'""",
                   {'Improvement': improvement})
    result = cursor.fetchone()
    cursor.close()
    db.close()
    if result is None:
        await EventCommand.add_hex_improvements(improvement, build_points, road_multiplier, economy, loyalty,
                                                stability, unrest, consumption, defence, taxation, cavernous, coastline,
                                                desert, forest, hills, jungle, marsh, mountain, plains, water, guild_id,
                                                author)
...
async def improvement_remove(ctx: commands.Context, improvement: str):
    ...
    db.close()
    if result is not None:
        await EventCommand.remove_hex_improvement(improvement, guild_id, author)
...
@overseer.command()
async def improvement_modify(ctx: commands.Context, old_improvement: str, new_improvement: str, new_build_points: int,
                             new_road_multiplier: int, new_economy: int, new_loyalty: int, new_stability: int,
                             new_unrest: int, new_consumption: bool, new_defence: int, new_taxation: int,
                             new_cavernous: bool, new_coastline: bool, new_desert: bool, new_forest: bool,
                             new_hills: bool, new_jungle: bool, new_marsh: bool, new_mountains: bool, new_plains: bool,
                             new_water: bool):
    ...
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    cursor.execute(f"""SELECT Improvement from Hexes_Improvements where Improvement = '{new_improvement}'""",
                   {'Improvement': new_improvement})
    result1 = cursor.fetchone()
    cursor.execute(f"""SELECT Improvement from Hexes_Improvements where Improvement = '{old_improvement}'""",
                   {'Improvement': old_improvement})
    ...
    if result1 is not None:
        if result2 is None:
            ...
            await ctx.response.send_message(status)
            await EventCommand.modify_hex_improvement(old_improvement, new_improvement, new_build_points,
                                                      new_road_multiplier, new_economy, new_loyalty, new_stability,
                                                      new_unrest, new_consumption, new_defence, new_taxation,
                                                      new_cavernous, new_coastline, new_desert, new_forest, new_hills,
                                                      new_jungle, new_marsh, new_mountains, new_plains, new_water,
                                                      guild_id, author)
...
async def kingdom_tables_rebalance(ctx: commands.Context):
    ...
    await EventCommand.balance_tables(guild_id, author)
...
async def character_milestones(ctx: commands.Context, character_name: str, amount: int,
    ...
    if job == 1:
        ...
        job_name = 'None'
    cursor.execute(
        f"SELECT Player_Name, Player_ID, True_Character_Name, Character_Name, Titles, Description, Oath, Level, Tier, Milestones, Milestones_Required, Trials, Trials_Required, Gold, Gold_Value, Gold_value_Max, Flux, Color, Mythweavers, Image_Link, Tradition_Name, Tradition_Link, Template_Name, Template_Link, Message_ID, Logging_ID, Thread_ID, Fame, Title, Personal_Cap, Prestige, Article_Link FROM Player_Characters where Character_Name = ? OR Nickname = ?",
    ...
    if player_info is not None:
        ...
        if job_name == 'None' and misc_milestones == 0:
            ...
            if current_level[0] < int_max_level:
                ...
                cursor.execute(
                    f"SELECT Level, Minimum_Milestones, Milestones_to_level from AA_Milestones  WHERE level = '{character_level}'")
                current_level = cursor.fetchone()
                remaining = current_level[1] + current_level[2] - milestone_total
            if current_level is None:
                await ctx.response.send_message(
                    f"Comrade, one cannot degrade this character: {character_name} past level 3, please train them to  best level up in the future!")
            else:
                await EventCommand.adjust_milestones(character_name, milestone_total, remaining, character_level,
                                                     guild_id, author)
                if character_level != player_info[1]:
                    cursor.execute(
                        f"SELECT Level, Role_name, Role_ID FROM Level_Range WHERE level = '{player_info[7]}'")
                    ...
                    cursor.execute(
                        f"Select True_Character_Name from Player_Characters WHERE Player_Name = '{player_info[0]}' AND level >= {level_range_min[0]} AND level <= {level_range_max[0]}")
                    ...
                    bio_embed = character_embed(player_info[0], player_info[1], player_info[2], player_info[4],
                                                player_info[5], player_info[6], character_level, player_info[8],
                                                milestone_total, remaining, player_info[11], player_info[12],
                    ...
                    logging_embed = log_embed(player_info[2], author, character_level, adjust_milestones,
                                              milestone_total, remaining, None, None, None, None, None, None, None,
                    ...
                    if level_range_characters is None:
                        ...
                        await member.add_roles(role2)
                    await ctx.response.send_message(
                        f"{ctx.user.name} has adjusted {character_name}'s milestones by {adjust_milestones}, they are now level {current_level[0]} and require {remaining} milestones to reach their next level up!")
                if player_info[1] == current_level[0]:
                    await ctx.response.send_message(
                        f"{ctx.user.name} has adjusted {character_name}'s milestones by {adjust_milestones}, they require {remaining} milestones to level up!")
    cursor.close()
    db.close()
...
async def character_trials(ctx: commands.Context, character_name: str, amount: int):
    ...
    if amount == 0:
        ...
        if player_info is None:
            await ctx.response.send_message(
                f"{ctx.user.name} does not have {character_name} registered to their account.")
        else:
            cursor.execute(f"SELECT Search from Admin WHERE Identifier = 'Tier_Cap'")
            max_tier = cursor.fetchone()
            cursor.execute(f"SELECT Search from Admin WHERE Identifier = 'Tier_Rate_Limit_Breakpoint'")
            ...
            cursor.execute(
                f"SELECT Tier, Trials, Trials_Required FROM AA_Trials where Trials <= {total_trials} ORDER BY Trials Desc LIMIT 1")
            trial_info = cursor.fetchone()
            if trial_info is None:
                await ctx.response.send_message(
                    f"{character_name} cannot be made any more menial! Please comrade, encourage them to elevate themselves in the future!")
                ...
                logging_embed = log_embed(player_info[2], author, None, None, None, None, trial_info[0], amount,
                                          total_trials, trial_info[1] + trial_info[2] - total_trials, None, None, None,
                                          None, None, None, None, None, None, None, None, None, None, None, None,
                                          source)
                logging_thread = guild.get_thread(player_info[25])
                await logging_thread.send(embed=logging_embed)
                await EventCommand.adjust_trials(character_name, total_trials, guild_id, author)
                if player_info[8] != trial_info[0]:
                    remaining = trial_info[1] + trial_info[2] - total_trials
                    await ctx.response.send_message(
                        f"{ctx.user.name} has adjusted {character_name}'s trials by {amount}, reaching a tier of {trial_info[0]} with {remaining} trials to increase their tier.")
                if player_info[8] == trial_info[0]:
                    remaining = trial_info[1] + trial_info[2] - total_trials
                    await ctx.response.send_message(
                        f"{ctx.user.name} has adjusted {character_name}'s trials by {amount} with {remaining} trials to increase their tier")
            else:
...
@admin.command()
@app_commands.autocomplete(character_name=character_select_autocompletion)
async def gold_adjust(ctx: commands.Context, character_name: str, amount: typing.Optional[float],
                      effective_gold: typing.Optional[float], lifetime_gold: typing.Optional[float], reason: str):
    ...
    if amount is None and effective_gold is None and lifetime_gold is None:
        await ctx.response.send_message(f"BRUH, if you don't intend to change anything why change at all?",
                                        ephemeral=True)
    else:
        cursor.execute(
            f"SELECT Player_Name, Player_ID, True_Character_Name, Character_Name, Titles, Description, Oath, Level, Tier, Milestones, Milestones_Required, Trials, Trials_Required, Gold, Gold_Value, Gold_value_Max, Flux, Color, Mythweavers, Image_Link, Tradition_Name, Tradition_Link, Template_Name, Template_Link, Message_ID, Logging_ID, Thread_ID, Fame, Title, Personal_Cap, Prestige, Article_Link FROM Player_Characters where Character_Name = ? OR Nickname = ?",
        ...
        if player_info is None:
            ...
            await EventCommand.gold_change(guild_id, author, author_id, player_info[3], gold_info[3],
                                           effective_gold_change, lifetime_gold_change, reason, 'Admin Gold Adjust')
            ...
            bio_embed = character_embed(player_info[0], player_info[1], player_info[2], player_info[4], player_info[5],
                                        ...
                                        player_info[11], player_info[12], player_info[13] + gold_info[3],
                                        player_info[14] + effective_gold_change, player_info[16], player_info[17],
                                        player_info[18], player_info[19], player_info[20], player_info[21],
                                        player_info[22], player_info[23], player_info[27], player_info[28],
                                        player_info[30], player_info[31])
            bio_channel = await bot.fetch_channel(accepted_bio_channel[0])
            ...
            logging_embed = log_embed(player_info[2], author, None, None, None, None, None, None, None, None,
                                      ...
                                      player_info[14] + effective_gold_change, transaction_id[0], None, None, None,
                                      None, None, None, None, None, None, None, None, source)
            logging_thread = guild.get_thread(player_info[25])
            await logging_thread.send(embed=logging_embed)
            embed = discord.Embed(title=f"Admin Gold Change", description=f'Gold Adjustment Transaction',
                                  colour=discord.Colour.blurple())
            embed.add_field(name=f'**Adjustments**',
                            value=f'Gold change: {gold_info[3]}, Effective Gold Change: {effective_gold_change}, Lifetime Wealth Change: {lifetime_gold_change}',
                            inline=False)
            embed.add_field(name=f"**Totals**",
                            value=f"Gold Total: {player_info[13] + gold_info[3]}, Effective Gold Total: {player_info[14] + effective_gold_change}, Lifetime Wealth Total: {player_info[15] + lifetime_gold_change}",
                            inline=False)
...
async def undo_transaction(ctx: commands.Context, transaction_id: int):
    ...
    author = ctx.user.name
    cursor.execute(
        f"Select Author_Name, Author_ID, Character_Name, Gold_Value, Effective_Gold_Value, Effective_Gold_value_max FROM A_Audit_Gold WHERE Transaction_ID = {transaction_id}")
    transaction_info = cursor.fetchone()
    if transaction_info is not None:
        embed = discord.Embed(title=f"Undoing Transaction: {transaction_id}", description=f'Undoing a transaction',
        ...
        embed.add_field(name=f"**{transaction_info[2]}'s Transaction Info:**",
                        value=f'**Gold:** {transaction_info[3]} GP, **Effective Gold**: {transaction_info[4]} GP, **Lifetime Gold**: {transaction_info[5]}.',
                        inline=False)
        cursor.execute(
            f"Select Author_Name, Author_ID, Character_Name, Gold_Value, Effective_Gold_Value, Effective_Gold_value_max, Transaction_ID FROM A_Audit_Gold WHERE Related_Transaction_ID = {transaction_id}")
        related_transaction_info = cursor.fetchone()
        await EventCommand.undo_transaction(guild_id, transaction_id, gold, effective_gold, max_effective_gold,
                                            transaction_info[2], transaction_info[0])
        cursor.execute(f"Select Search FROM Admin WHERE Identifier = 'Accepted_Bio_Channel'")
        accepted_bio_channel = cursor.fetchone()
        cursor.execute(f"Select Search FROM Admin WHERE Identifier = 'Char_Eventlog_Channel'")
        character_log_channel_id = cursor.fetchone()
        cursor.execute(
            f"SELECT Player_Name, Player_ID, True_Character_Name, Character_Name, Titles, Description, Oath, Level, Tier, Milestones, Milestones_Required, Trials, Trials_Required, Gold, Gold_Value, Gold_value_Max, Flux, Color, Mythweavers, Image_Link, Tradition_Name, Tradition_Link, Template_Name, Template_Link, Message_ID, Logging_ID, Thread_ID, Fame, Title, Personal_Cap, Prestige, Article_Link FROM Player_Characters where Character_Name = ?",
            (transaction_info[2],))
        player_info = cursor.fetchone()
        bio_channel = await bot.fetch_channel(accepted_bio_channel[0])
        bio_embed = character_embed(player_info[0], player_info[1], player_info[2], player_info[4], player_info[5],
                                    player_info[6], player_info[7], player_info[8], player_info[9], player_info[10],
                                    player_info[11], player_info[12], player_info[13] + gold,
                                    player_info[14] + effective_gold, player_info[16], player_info[17], player_info[18],
                                    player_info[19], player_info[20], player_info[21], player_info[22], player_info[23],
                                    player_info[27], player_info[28], player_info[30], player_info[31])
        bio_message = await bio_channel.fetch_message(player_info[24])
        await bio_message.edit(content=bio_embed[1], embed=bio_embed[0])
        source = f"admin undid transaction {transaction_id}"
        logging_embed = log_embed(player_info[2], author, None, None, None, None, None, None, None, None,
                                  player_info[13] + gold, gold, player_info[14] + effective_gold, transaction_id, None,
        ...
        if related_transaction_info is not None:
            ...
            cursor.execute(
                f"SELECT Player_Name, Player_ID, True_Character_Name, Character_Name, Titles, Description, Oath, Level, Tier, Milestones, Milestones_Required, Trials, Trials_Required, Gold, Gold_Value, Gold_value_Max, Flux, Color, Mythweavers, Image_Link, Tradition_Name, Tradition_Link, Template_Name, Template_Link, Message_ID, Logging_ID, Thread_ID, Fame, Title, Personal_Cap, Prestige, Article_Link FROM Player_Characters where Character_Name = ?",
                (related_transaction_info[2],))
            player_info = cursor.fetchone()
            embed.add_field(name=f"**{related_transaction_info[2]}'s Transaction Info:**",
                            value=f'**Gold:** {related_transaction_info[3]} GP, **Effective Gold**: {related_transaction_info[4]} GP, **Lifetime Gold**: {related_transaction_info[5]}.',
                            inline=False)
            await EventCommand.undo_transaction(guild_id, related_transaction_info[6], gold, effective_gold,
                                                max_effective_gold, related_transaction_info[0],
                                                related_transaction_info[2])
            bio_embed = character_embed(player_info[0], player_info[1], player_info[2], player_info[4], player_info[5],
                                        player_info[6], player_info[7], player_info[8], player_info[9], player_info[10],
                                        player_info[11], player_info[12], player_info[13] + gold,
                                        player_info[14] + effective_gold, player_info[16], player_info[17],
                                        player_info[18], player_info[19], player_info[20], player_info[21],
                                        player_info[22], player_info[23], player_info[27], player_info[28],
                                        player_info[30], player_info[31])
            bio_message = await bio_channel.fetch_message(player_info[24])
            ...
            source = f"admin undid transaction {transaction_id}"
            logging_embed = log_embed(player_info[2], author, None, None, None, None, None, None, None, None,
                                      player_info[13] + gold, gold, player_info[14] + effective_gold, transaction_id,
                                      None, None, None, None, None, None, None, None, None, None, None, source)
            logging_thread = guild.get_thread(player_info[25])
            await logging_thread.send(embed=logging_embed)
    else:
        mentions = f"This Transaction was not a valid transaction to undo!!"
        embed = discord.Embed(title=f"Command Failed! Undo Transaction: {transaction_id}",
                              description=f'This Command Failed', colour=discord.Colour.red())
...
# noinspection PyUnresolvedReferences
@admin.command()
async def session_management(interaction: discord.Interaction, session_id: int, gold: typing.Optional[int],
                             easy: typing.Optional[int], medium: typing.Optional[int], hard: typing.Optional[int],
                             deadly: typing.Optional[int], flux: typing.Optional[int], trials: typing.Optional[int],
    ...
    cursor = db.cursor()
    cursor.execute(
        f"""SELECT GM_Name, Session_Name, Play_Time, Session_Range, Gold, Flux, Easy, Medium, Hard, Deadly, Trials, Alt_Reward_All, Alt_Reward_Party, Session_Thread, Message, Rewards_Message, Rewards_Thread, Fame, Prestige FROM Sessions WHERE Session_ID = {session_id} and IsActive = 0 LIMIT 1""")
    session_simple = cursor.fetchone()
    cursor.execute(
        f"""SELECT Player_Name, Character_Name, Level, Received_Milestones, Effective_Gold, Received_Gold, Player_ID, Received_Fame, Forego, Received_Prestige FROM Sessions_Archive WHERE Session_ID = {session_id}""")
    ...
    if session_simple is None:
        ...
        if gold is not None and gold < 0 or easy is not None and easy < 0 or medium is not None and medium < 0 or hard is not None and hard < 0 or deadly is not None and deadly < 0 or flux is not None and flux < 0 or trials is not None and trials < 0:
            ...
            embed = discord.Embed(title="Session Report", description=f"a report of the session: {session_simple[1]}",
                                  color=discord.Color.blue())
            embed.set_author(name=f'{session_simple[0]}')
            embed.add_field(name="Session Info",
                            value=f'**GM:** {session_simple[0]} \n **Level Range**: {session_simple[3]}, **Gold**: {session_simple[4]}, **Trials**: {session_simple[10]}, **Fame**: {session_simple[17]} **Flux**:{session_simple[5]}',
                            inline=False)
            embed.add_field(name="Job Info",
                            value=f'**Easy**: {session_simple[6]}, **Medium**:{session_simple[7]}, **Hard**: {session_simple[8]}, **Deadly**: {session_simple[9]}',
                            inline=False)
            for player in session_complex:
                embed.add_field(name="Character Info",
                                value=f'Player: {player[0]} Character:{player[1]} \r\n **Level**: {player[2]} \r\n **Milestones Received**: {player[3]} **Gold Received**: {player[5]} \r\n ***Fame Received***: {player[[7]]}',
                                inline=False)
            ...
            embed.set_author(name=f'{session_simple[0]}')
            embed.add_field(name="Session Info",
                            value=f'**GM:** {session_simple[0]} \n **Level Range**: {session_simple[3]}, **Gold**: {gold}, **Trials**: {trials}, **Fame**: {fame}, **Flux**:{flux}',
                            inline=False)
            embed.add_field(name="Job Info",
                            value=f'**Easy**: {easy}, **Medium**:{medium}, **Hard**: {hard}, **Deadly**: {deadly}',
                            inline=False)
            x = 0
            if party_reward is not None:
                party_reward_embed = discord.Embed(title="Party Reward",
                                                   description=f"Party Reward for {session_simple[1]}",
                                                   color=discord.Color.blue())
                ...
                if session_simple[16] is None:
                    ...
                    reward_thread = await reward_msg.create_thread(
                        name=f"Session name: {session_simple[1]} Party Rewards, Session ID: {session_simple[0]}",
                        auto_archive_duration=60, reason=f"{party_reward}")
            ...
            for player in session_complex:
                ...
                cursor.execute(
                    f"SELECT Milestones, Tier, Trials, Gold, Gold_Value, Gold_Value_Max, Flux, Oath FROM Player_Characters WHERE Character_Name = ?",
                    (player[1],))
                ...
                current_level = cursor.fetchone()
                if int_max_level < current_level[0]:
                    int_max_level = player_info[29] if player_info[29] is not None and int_max_level > player_info[
                        29] else int_max_level
                    cursor.execute(
                        f"SELECT Level, Minimum_Milestones, Milestones_to_level from AA_Milestones  WHERE Level = {int_max_level} ORDER BY Minimum_Milestones DESC  LIMIT 1")
                    current_level = cursor.fetchone()
                ...
                if true_level != player[2]:
                    ...
                    level_range_min = cursor.fetchone()
                    cursor.execute(
                        f"Select True_Character_Name from Player_Characters WHERE Player_Name = '{player[0]}' AND level >= {level_range_min[0]} AND level <= {level_range_max[0]}")
                    level_range_characters = cursor.fetchone()
                    member = await guild.fetch_member(player_info[1])
                    if level_range_characters is None:
                ...
                cursor.execute(
                    f"SELECT Tier, Trials, Trials_Required from AA_Trials  WHERE Trials <= {trials_total} ORDER BY Trials DESC  LIMIT 1")
                current_mythic_information = cursor.fetchone()
                cursor.execute(f"SELECT Search from Admin WHERE Identifier = 'Tier_Cap'")
                max_tier = cursor.fetchone()
                cursor.execute(f"SELECT Search from Admin WHERE Identifier = 'Tier_Rate_Limit_Breakpoint'")
                ...
                if true_tier == rate_limited_tier or true_tier == max_tier:
                    cursor.execute(
                        f"SELECT Tier, Trials, Trials_Required from AA_Trials  WHERE Tier = {true_tier} LIMIT 1")
                    current_mythic_information = cursor.fetchone()
                ...
                #                    DONE WITH GOLD
                difference = round(difference, 2)
                await EventCommand.session_rewards(player[0], guild_id, player[1], true_level,
                                                   new_milestone_total, remaining, flux_total, true_tier, trials_total,
                                                   trials_required, fame_total, prestige_total,
                                                   f"Adjusting Session {session_id} reward")
                await EventCommand.gold_change(guild_id, player[0], player[6], player[1], difference, difference,
                                               gold, 'Session Reward', 'Session Reward')
                await EventCommand.update_session_log_player(guild_id, session_id, player[1], rewarded, trials,
                                                             difference, fame, prestige)
                embed.add_field(name="Character Info",
                                value=f'Player: {player[0]} Character:{player[1]} \n **Level**: {true_level} \n **Milestone change**: {rewarded} **Gold change**: {difference}',
                                inline=False)
                cursor.execute(f'SELECT MAX(Transaction_ID) FROM A_Audit_Gold Order By Transaction_ID DESC LIMIT 1')
                transaction_id = cursor.fetchone()
                cursor.execute(f"Select Search FROM Admin WHERE Identifier = 'Accepted_Bio_Channel'")
                ...
                character_log_channel_id = cursor.fetchone()
                bio_embed = character_embed(player_info[0], player_info[1], player_info[2], player_info[4],
                                            player_info[5], player_info[6], true_level, true_tier, new_milestone_total,
                                            remaining, trials_total, trials_required, player_info[13] + difference,
                                            player_info[14] + difference, flux_total, player_info[17], player_info[18],
                                            player_info[19], player_info[20], player_info[21], player_info[22],
                                            player_info[23], player_info[27], fame_total, prestige_total,
                                            player_info[31])
                bio_channel = await bot.fetch_channel(accepted_bio_channel[0])
                bio_message = await bio_channel.fetch_message(player_info[24])
                await bio_message.edit(content=bio_embed[1], embed=bio_embed[0])
                source = f"admin adjusted session {session_id}"
                logging_embed = log_embed(player_info[2], player_info[2], true_level, rewarded, new_milestone_total,
                                          remaining, true_tier, trials - session_simple[10], trials_total,
                                          trials_required, player_info[13] + difference, difference,
                                          player_info[14] + difference, transaction_id[0], flux_total,
                                          flux - session_simple[5], None, None, None, None, reward_all, fame_total,
                                          fame, prestige_total, prestige, source)
                logging_thread = guild.get_thread(player_info[25])
                await logging_thread.send(embed=logging_embed)
            await EventCommand.update_session_log(guild_id, session_id, gold, flux, easy, medium, hard, deadly,
                                                  trials, reward_all, party_reward, reward_message_id, reward_thread_id,
                                                  fame, prestige)
...
async def settings_display(ctx: commands.Context, current_page: int = 1):
    ...
    while True:
        try:
            ...
            if current_page != previous_page:
                cursor.execute(
                    f"""SELECT Search, Type, Identifier, Description from Admin WHERE ROWID BETWEEN {low} and {high}""")
                pull = cursor.fetchall()
                embed = discord.Embed(title=f"Admin Settings Page {current_page}",
                                      description=f'This is a list of the administrative defined settings',
                                      colour=discord.Colour.blurple())
...
async def settings_define(interaction: discord.Interaction, identifier: str, new_search: str):
    ...
    if new_search is None:
        ...
        if information is not None:
            await EventCommand.update_settings(guild_id, author, new_search, identifier)
...
async def level_cap(interaction: discord.Interaction, new_level: int):
    ...
    if new_level > 20:
        ...
        await interaction.response.defer(thinking=True)
        await EventCommand.update_level_cap(guild_id, author, new_level)
        ...
        cursor.execute(
            f"SELECT Player_Name, Player_ID, Character_Name, Level, Personal_Cap FROM Player_Characters WHERE Milestones >= {minimum_milestones} LIMIT 20")
        ...
        if count_of_characters is not None:
            ...
            for characters in characters_info:
                ...
                if personal_cap >= new_level:
                    ...
                    level_range_min = cursor.fetchone()
                    cursor.execute(
                        f"Select True_Character_Name from Player_Characters WHERE Player_Name = '{characters[2]}' AND level >= {level_range_min[0]} AND level <= {level_range_max[0]}")
                    level_range_characters = cursor.fetchone()
...
async def tier_cap(interaction: discord.Interaction, new_tier: int):
    ...
    if new_tier < 1:
        ...
        minimum_milestones = level_info[0]
        cursor.execute(
            f"SELECT COUNT(Character_Name) FROM Player_Characters Trials WHERE level >= {minimum_milestones} and level >= {minimum_level}")
        count_of_characters = cursor.fetchone()
        cursor.execute(
            f"SELECT Player_Name, Player_ID, Character_Name FROM Player_Characters WHERE Trials >= {minimum_milestones} LIMIT 20")
        characters_info = cursor.fetchall()
        await EventCommand.update_tier_cap(guild_id, author, new_tier, minimum_level)
...
async def flux_adjust(ctx: commands.Context, character_name: str, amount: int):
    ...
    if amount == 0:
        ...
        if player_info is None:
            ...
            await EventCommand.flux(guild_id, true_name, amount, new_flux, author)
            ...
            logging_embed = log_embed(player_info[2], author, None, None, None, None, None, None, None, None, None,
                                      None, None, None, new_flux, amount, None, None, None, None, None, None, None,
                                      None, None, source)
            logging_thread = guild.get_thread(player_info[25])
            await logging_thread.send(embed=logging_embed)
    cursor.close()
...
async def level_range(ctx: commands.Context, range: discord.Role, minimum_level: int, maximum_level: int):
    ...
    if maximum_level < minimum_level:
        ...
        await ctx.response.send_message(f"Your maximum level of {maximum_level} is either below 3 or above 20!",
                                        ephemeral=True)
    elif minimum_level < 3 or minimum_level > 20:
        await ctx.response.send_message(f"Your minimum level of {minimum_level} is either below 3 or above 20!",
                                        ephemeral=True)
    else:
        guild_id = ctx.guild_id
        author = ctx.user.name
        ...
        role_id = range.id
        await EventCommand.set_range(guild_id, author, role_name, role_id, minimum_level, maximum_level)
...
async def clean_playerbase(ctx: commands.Context, player: typing.Optional[discord.Member],
    ...
    if player_wipe == 1 and player_id is None and player is None:
        ...
        if player_id is not None:
            cursor.execute(
                f"Select distinct(Player_Name), count(Character_Name) from Player_Characters where Player_ID = {player_id}")
        ...
        if player is not None:
            cursor.execute(
                f"Select distinct(Player_Name), count(Character_Name) from Player_Characters where Player_ID = {player.id}")
            player_id_info = cursor.fetchone()
            if player_id_info is not None:
            ...
            try:
                ...
                if reaction.emoji == u"\u2705":
                    ...
                    for wiped in overall_wipe_list:
                        await EventCommand.wipe_unapproved(wiped[0], guild_id, author)
...
async def manage(ctx: commands.Context, character_name: str, player_id: typing.Optional[int],
    ...
    if character_name is None and cleanse is None:
        ...
        if status == 2:
            ...
            if character_name is not None:
                cursor.execute(
                    f"Select distinct(Player_Name), count(Character_Name) from A_STG_Player_Characters where character_name = ?",
                    (character_name,))
                player_id_info = cursor.fetchone()
                if player_id_info is not None:
            ...
            if cleanse is not None:
                if cleanse.endswith('D'):
                    ...
                    cursor.execute(
                        f"Select distinct(Player_Name), count(Character_Name) from A_STG_Player_Characters where Created_Date <= date('now', '-{cleanse} days')")
                    player_id_info = cursor.fetchall()
                    x = 0
                    for inactive_player in player_id_info:
                        x += 1
                    ...
                    cursor.execute(
                        f"Select distinct(Player_Name), count(Character_Name) from A_STG_Player_Characters where Created_Date <= date('now', '-{cleanse} weeks')")
                    player_id_info = cursor.fetchall()
                    x = 0
                    for inactive_player in player_id_info:
                        x += 1
            ...
            while True:
                try:
                    ...
                    if reaction.emoji == u"\u2705":
                        ...
                        for wiped in overall_wipe_list:
                            await EventCommand.wipe_unapproved(wiped, guild_id, author)
    else:
        e = None
        try:
        ...
        character_name = str.replace(str.replace(unidecode(str.title(character_name)), ";", ""), ")", "")
        cursor.execute(
            f"Select True_Character_Name, tmp_bio, mythweavers from A_STG_Player_Characters where character_name = ?",
            (character_name,))
        player_id_info = cursor.fetchone()
        if player_id_info is not None:
            if player_id_info[1] is not None or 'worldanvil' in player_id_info[2]:
                try:
                    e = None
                    await EventCommand.create_bio(guild_id, player_id_info[0], player_id_info[1],
                                                  player_id_info[2])
                except Exception as e:
                    print(e)
                    pass
                ...
                e = None
            await EventCommand.create_character(guild_id, author, player_id_info[0])
            ...
            character_log_channel_id = cursor.fetchone()
            cursor.execute(
                f"SELECT Player_Name, True_Character_Name, Titles, Description, Level, Tier, Milestones, Milestones_Required, Trials, Trials_Required, Gold, Gold_Value, Gold_Value_Max, Mythweavers, Image_Link, Color, Flux, Player_ID, Oath, Article_Link FROM Player_Characters WHERE Character_Name = ?",
                (character_name,))
            character_info = cursor.fetchone()
            ...
            await EventCommand.log_character(guild_id, character_name, bio_message.id, character_log_message.id,
                                             thread.id)
            if e is None:
...
async def customize(ctx: commands.Context, character_name: str, destination: discord.app_commands.Choice[int],
                    customized_name: str, link: str, flux_cost: int = 0):
    ...
    if player_info is None:
        await ctx.response.send_message(f"no character with the Name or Nickname of {character_name} could be found!",
                                        ephemeral=True)
    else:
        ...
        await EventCommand.customize_characters(guild_id, author, player_info[3], destination_name,
                                                destination_link, customized_name, link, flux_remaining, flux_cost)
        embed = discord.Embed(title=f"{destination_name_pretty} change for {player_info[3]}",
                              description=f"<@{player_info[1]}>'s {player_info[3]} has spent {flux_cost} flux leaving them with {player_info[16] - flux_cost} flux!",
                              colour=discord.Colour.blurple())
        ...
        bio_embed = character_embed(player_info[0], player_info[1], player_info[2], player_info[4], player_info[5],
                                    player_info[6], player_info[7], player_info[8], player_info[9], player_info[10],
                                    player_info[11], player_info[12], player_info[13], player_info[14],
                                    player_info[16] + flux_cost, player_info[17], player_info[18], player_info[19],
        ...
        logging_embed = log_embed(player_info[2], author, None, None, None, None, None, None, None, None, None, None,
                                  None, None, player_info[16] - flux_cost, flux_cost, tradition_name, tradition_link,
...
async def fame_store(ctx: commands.Context, name: str, fame_required: typing.Optional[int],
    ...
    if item_info is None and modify == 2:
        await EventCommand.add_fame_store(guild_id, author, fame_required, prestige_cost, name, effect, limit)
        ...
        await EventCommand.remove_fame_store(guild_id, author, name)
        ...
        await EventCommand.edit_fame_store(guild_id, author, fame_required, prestige_cost, name, effect, limit)
        ...
        cursor = db.cursor()
        cursor.execute(
            f"""SELECT Fame_Required, Prestige_Cost, Name, Effect, Use_Limit FROM Store_Fame where Name = ?""", (name,))
        item_info = cursor.fetchone()
        if item_info is not None:
            embed = discord.Embed(
                title=f'**Fame Required**: {item_info[0]} **Prestige Cost**: {item_info[1]}, **Limit**: {item_info[4]} \r\n **Effect**: {item_info[3]}',
                colour=discord.Colour.blurple())
            await ctx.response.send_message(embed=embed)
        else:
            buttons = ["⏪", "⬅", "➡", "⏩"]  # skip to start, left, right, skip to end
...
async def title_store(ctx: commands.Context, masculine_name: typing.Optional[str], feminine_name: typing.Optional[str],
    ...
    author = ctx.user.name
    cursor.execute(
        f"SELECT ID, Effect, Fame, Masculine_Name, Feminine_Name FROM Store_Title where Masculine_Name = ? OR Feminine_Name = ?",
        (masculine_name, feminine_name))
    item_info = cursor.fetchone()
    cursor.close()
    db.close()
    if item_info is None and modify == 1:
        await EventCommand.add_title_store(guild_id, author, ubb_id, effect, fame, masculine_name, feminine_name)
        ...
        await EventCommand.remove_title_store(guild_id, author, item_info[2], item_info[3], item_info[4])
        ...
        await EventCommand.edit_title_store(guild_id, author, ubb_id, effect, fame, masculine_name, feminine_name)
        ...
        await ctx.response.send_message(
            f"you were trying to do the following {modify} modification in the title store, but {masculine_name} was incorrect!",
            ephemeral=True)
...
async def help(ctx: commands.Context):
    """Help commands for the associated tree"""
    embed = discord.Embed(title=f"Settlement Help", description=f'This is a list of Settlement help commands',
                          colour=discord.Colour.blurple())
    ...
    embed.add_field(name=f'**Modify**', value=f'The command for a player to modify the properties of a settlement.',
                    inline=False)
...
async def claim(ctx: commands.Context, kingdom: str, password: str, settlement: str):
    ...
    if settlement_result is None and kingdom_result[1] == password:
        ...
        await ctx.response.send_message(status)
        await EventCommand.claim_settlement(kingdom, settlement, guild_id, author)
...
async def destroy(ctx: commands.Context, kingdom: str, password: str, settlement: str):
    ...
    if kingdom_result is None:
        ...
        await EventCommand.destroy_settlement(kingdom, settlement, guild_id, author)
        await ctx.response.send_message(status)
...
async def modify(ctx: commands.Context, kingdom: str, password: str, old_settlement: str, new_settlement: str):
    ...
    cursor = db.cursor()
    cursor.execute(
        f"""SELECT Settlement FROM Settlements WHERE Settlement = '{old_settlement}' AND Kingdom = '{kingdom}'""",
        {'Settlement': old_settlement, 'Kingdom': kingdom})
    result = cursor.fetchone()
    cursor.execute(f"""SELECT Kingdom, Password FROM kingdoms WHERE Kingdom = '{kingdom}'""", {'kingdom': kingdom})
    kingdom_result = cursor.fetchone()
    cursor.close()
    ...
    if result is not None and password == kingdom_result[1]:
        ...
        await ctx.response.send_message(status)
        await EventCommand.modify_settlement(kingdom, old_settlement, new_settlement, guild_id, author)
...
async def detail(ctx: commands.Context, kingdom: str, settlement: str, current_page: int = 1):
    ...
    buttons = ["⏪", "⬅", "➡", "⏩"]  # skip to start, left, right, skip to end
    cursor.execute(
        f"""SELECT COUNT(building) FROM Buildings WHERE Kingdom = '{kingdom}' AND Settlement = '{settlement}'""",
        {'Kingdom': kingdom, 'Settlement': settlement})
    ...
    high = 5 + ((current_page - 1) * 5)
    cursor.execute(
        f"""SELECT Building, Constructed, Lots, Economy, Loyalty, Stability, Fame, Unrest, Corruption, Crime, Productivity, Law, Lore, Society, Danger, Defence, Base_value, Spellcasting, Supply from Buildings WHERE Kingdom = '{kingdom}' AND Settlement = '{settlement}' AND ROWID BETWEEN {low} and {high}""",
        {'Kingdom': kingdom, 'Settlement': settlement})
    pull = cursor.fetchall()
    embed = discord.Embed(title=f"Buildings page {current_page}",
                          description=f'This is list of constructed buildings in the settlement of {settlement} in kingdom of {kingdom}',
                          colour=discord.Colour.blurple())
    for result in pull:
        embed.add_field(name=f'Buildings Info',
                        value=f'***__Building__***: {result[0]}, **Constructed**: {result[1]}, **Lots occupied**: {result[2]}, **Supply**: {result[18]}',
                        inline=False)
        embed.add_field(name=f'Kingdom Benefits',
                        value=f'**Economy**: {result[3]}, **Loyalty**: {result[4]}, **Stability**: {result[5]}, **Fame:**: {result[6]}, **Unrest**: {result[7]}',
                        inline=False)
        embed.add_field(name=f'Settlement Benefits',
                        value=f'**Corruption:**: {result[8]}, **Crime**: {result[9]}, **Productivity**: {result[10]}, **Law**: {result[11]}, **Lore**: {result[12]}, **Society**: {result[13]}',
                        inline=False)
        embed.add_field(name=f'Settlement Risks', value=f'**Danger**: {result[14]} **Defence**: {result[15]}')
        embed.add_field(name=f'Settlement Economy',
                        value=f'**Base Value**: {result[16]}, **Spellcasting**: {result[17]}')
        embed.add_field(name=f'Settlement Limitation',
                        value=f'**Settlement Limit**: {result[19]}, **District Limit**: {result[20]}')
            ...
            if current_page != previous_page:
                ...
                for result in edit_pull:
                    ...
                    embed.add_field(name=f'Settlement Benefits',
                                    value=f'**Corruption:**: {result[8]}, **Crime**: {result[9]}, **Productivity**: {result[10]}, **Law**: {result[11]}, **Lore**: {result[12]}, **Society**: {result[13]}',
                                    inline=False)
                    embed.add_field(name=f'Settlement Risks',
                                    value=f'**Danger**: {result[14]} **Defence**: {result[15]}')
                    embed.add_field(name=f'Settlement Economy',
                                    value=f'**Base Value**: {result[16]}, **Spellcasting**: {result[17]}')
                    embed.add_field(name=f'Settlement Limitation',
                                    value=f'**Settlement Limit**: {result[19]}, **District Limit**: {result[20]}')
...
async def displayall(ctx: commands.Context, kingdom: str, settlement: str, current_page: int = 1):
    ...
    high = 5 + ((current_page - 1) * 5)
    cursor.execute(
        f"""SELECT Settlement, size, Population, Corruption, Crime, Productivity, Law, Lore, Society, Danger, Defence, Base_Value, Spellcasting, Supply, Decay FROM Settlements WHERE Kingdom = '{kingdom}' AND ROWID BETWEEN {low} and {high}""",
        {'Kingdom': kingdom})
    pull = cursor.fetchall()
    embed = discord.Embed(title=f"Settlements page {current_page}",
                          description=f'This is list of settlements in {kingdom}', colour=discord.Colour.blurple())
    cursor.execute(f"SELECT Search from Admin where identifier = 'Decay'")
    decay = cursor.fetchone()
    if decay[0]:  # IF THE SERVER HAS DECAY ON
        for result in pull:
            embed.add_field(name=f'Settlement info',
                            value=f'**Settlement**: {result[0]}, **Districts**: {result[1]} **Population**: {result[2]}, **Supply**: {result[13]}, **Decay**: {result[14]}',
                            inline=False)
            embed.add_field(name=f'Settlement Benefits',
                            value=f'**Corruption**: {result[3]}, **Crime**: {result[4]}, **Productivity**: {result[5]}, **Law**: {result[6]}, **Lore**: {result[7]}, **Society**: {result[8]}',
                            inline=False)
            embed.add_field(name=f'Settlement Misc',
                            value=f'**Danger**: {result[9]}, **Defence**: {result[10]} **Base_Value**: {result[11]} **Spellcasting**: {result[12]}, ',
                            inline=False)
    else:
        for result in pull:
            embed.add_field(name=f'Settlement info',
                            value=f'**Settlement**: {result[0]}, **Districts**: {result[1]} **Population**: {result[2]}, **Supply**: {result[13]}',
                            inline=False)
            embed.add_field(name=f'Settlement Benefits',
                            value=f'**Corruption**: {result[3]}, **Crime**: {result[4]}, **Productivity**: {result[5]}, **Law**: {result[6]}, **Lore**: {result[7]}, **Society**: {result[8]}',
                            inline=False)
    ...
    while True:
        try:
            ...
            if current_page != previous_page:
                ...
                edit_pull = cursor.fetchall()
                embed = discord.Embed(title=f"Settlements page {current_page}",
                                      description=f'This is list of settlements in {kingdom}',
                                      colour=discord.Colour.blurple())
                if decay[0]:  # IF THE SERVER HAS DECAY ON
                    for result in pull:
                        embed.add_field(name=f'Settlement info',
...
async def displayone(ctx: commands.Context, kingdom: str, settlement: str, custom_stats: bool = False):
    ...
    if custom_stats is False:
        cursor.execute(
            f"""SELECT Size, Population, Corruption, Crime, Productivity, Law, Lore, Society, Danger, Defence, Base_Value, Spellcasting, Supply, Decay from Settlements where Kingdom = '{kingdom}' and Settlement = '{settlement}'""",
            {'Kingdom': kingdom, 'Settlement': settlement})
        settlement_info = cursor.fetchone()
        cursor.execute(f"""SELECT Government, Alignment from Kingdoms where Kingdom = '{kingdom}'""",
                       {'Kingdom': kingdom})
        kingdom_info = cursor.fetchone()
        if kingdom_info is None:
            await ctx.response.send_message(f'The kingdom of {kingdom} could not be found.')
        if settlement_info is None:
            await ctx.response.send_message(
                f'The settlement of {settlement} could not be found underneath the kingdom of {kingdom}.')
        if kingdom_info is not None and settlement_info is not None:
            embed = discord.Embed(title=f"Settlement {Settlement} of kingdom {kingdom}",
                                  description=f'Here is the full view of this Settlement',
                                  colour=discord.Colour.blurple())
            embed.add_field(name=f'Government', value=f'{kingdom_info[0]}')
            embed.add_field(name=f'Alignment', value=f'{kingdom_info[1]}')
    ...
    if custom_stats is True:
        cursor.execute(
            f"""SELECT Corruption, Crime, Productivity, Law, Lore, Society, Danger, Defence, Base_Value, Spellcasting, Supply from Settlements_Custom where Kingdom = '{kingdom}' and Settlement = '{settlement}'""",
            {'Kingdom': kingdom, 'Settlement': settlement})
        kingdom_info = cursor.fetchone()
        if kingdom_info is None:
            await ctx.response.send_message(
                f'The settlement of {settlement} underneath kingdom of {kingdom_info} could not be found.')
        if kingdom_info is not None:
            embed = discord.Embed(title=f"Settlement of {settlement} underneath kingdom of {kingdom}",
                                  description=f'Here is the full view of this Custom Information for this Settlement',
                                  colour=discord.Colour.blurple())
...
async def help(ctx: commands.Context):
    """Help commands for the associated tree"""
    embed = discord.Embed(title=f"Building Help", description=f'This is a list of Settlement help commands',
                          colour=discord.Colour.blurple())
...
async def shop(ctx: commands.Context, current_page: int = 1, building: str = 'All'):
    ...
    if building == 'All':
        ...
        cursor.execute(
            f"""SELECT Building, Build_Points, Lots, Economy, Loyalty, Stability, Fame, Unrest, Corruption, Crime, Productivity, Law, Lore, Society, Danger, Defence, Base_value, Spellcasting, Supply, Settlement_limit, District_Limit, Description from Buildings_Blueprints WHERE ROWID BETWEEN {low} and {high}""")
        pull = cursor.fetchall()
        embed = discord.Embed(title=f"Blueprints page {current_page}", description=f'This is list of blueprints',
                              colour=discord.Colour.blurple())
        for result in pull:
            embed.add_field(name=f'Blueprint info',
                            value=f'***__Blueprint__***: {result[0]}, **build_points**: {result[1]}, **Lots**: {result[2]}, **Supply**: {result[18]}',
                            inline=False)
            embed.add_field(name=f'Kingdom Benefits',
                            value=f'**Economy**: {result[3]}, **Loyalty**: {result[4]}, **Stability**: {result[5]}, **Fame:**: {result[6]}, **Unrest**: {result[7]}',
                            inline=False)
            embed.add_field(name=f'Settlement Benefits',
                            ...
                            inline=False)
            embed.add_field(name=f'Settlement Misc',
                            value=f'**Danger**: {result[14]}, **Defence**: {result[15]}, **Base Value**: {result[16]}, **Spellcasting**: {result[17]}')
            embed.add_field(name=f'Settlement Limitation',
                            value=f'**Settlement Limit**: {result[19]}, **District Limit**: {result[20]}')
            embed.add_field(name=f'Blueprint Description', value=f'{result[21]}', inline=False)
        await ctx.response.send_message(embed=embed)
            ...
            try:
                ...
                if current_page != previous_page:
                    cursor.execute(
                        f"""SELECT Building, Build_Points, Lots, Economy, Loyalty, Stability, Fame, Unrest, Corruption, Crime, Productivity, Law, Lore, Society, Danger, Defence, Base_value, Spellcasting, Supply, Settlement_limit, District_Limit, Description from Buildings_Blueprints WHERE ROWID BETWEEN {low} and {high}""")
                    edit_pull = cursor.fetchall()
                    embed = discord.Embed(title=f"Blueprints page {current_page}",
                                          description=f'This is list of blueprints', colour=discord.Colour.blurple())
                    for result in edit_pull:
                        embed.add_field(name=f'Blueprint info',
                                        value=f'***__Blueprint__***: {result[0]}, **build_points**: {result[1]}, **Lots**: {result[2]}, **Supply**: {result[18]}',
                                        inline=False)
    ...
    if building != 'All':
        cursor.execute(
            f"""SELECT Build_Points, Lots, Economy, Loyalty, Stability, Fame, unrest, Corruption, Crime, Productivity, law, Lore, Society, Danger, Defence, Base_Value, Spellcasting, Supply, Settlement_Limit, District_Limit, Description FROM Buildings_Blueprints WHERE Building = '{building}'""",
            {'Building': building})
        result = cursor.fetchone()
        if result is None:
            ...
            embed.add_field(name=f'Settlement Benefits',
                            value=f'**Corruption:**: {result[8]} **Crime**: {result[9]} **Productivity**: {result[10]}, **Law**: {result[11]}, **Lore**: {result[12]}, **Society**: {result[13]}',
                            inline=False)
            embed.add_field(name=f'Alternate Modifier',
                            value=f'**Danger**:{result[14]}, **Defence**: {result[15]}, **Base_Value**: {result[16]}, **Spellcasting**: {result[17]}',
                            inline=False)
            embed.add_field(name=f'Settlement Limitation',
                            value=f'**Settlement Limit**: {result[19]}, **District Limit**: {result[20]}', inline=False)
            embed.add_field(name=f'Blueprint Description', value=f'{result[21]}', inline=False)
            await ctx.response.send_message(embed=embed)
    cursor.close()
    db.close()
...
async def build(ctx: commands.Context, kingdom: str, password: str, settlement: str, building: str, amount: int):
    ...
    kingdom_result = cursor.fetchone()
    cursor.execute(
        f"""SELECT Settlement FROM Settlements WHERE Kingdom = '{kingdom}' AND settlement = '{settlement}'""",
        {'kingdom': kingdom, 'settlement': settlement})
    settlement_result = cursor.fetchone()
    cursor.close()
    db.close()
    ...
    if blueprint_result is not None and settlement_result is not None and kingdom_result[0] == password:
        ...
        await ctx.response.send_message(status)
        await EventCommand.construct_building(kingdom, settlement, building, amount, guild_id, author)
...
async def destroy(ctx: commands.Context, kingdom: str, password: str, settlement: str, building: str, amount: int):
    ...
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    cursor.execute(f"""SELECT Building, Constructed FROM Buildings WHERE Building = '{building}'""",
                   {'building': building})
    ...
    if kingdom_result[0] != password:
        ...
        await ctx.response.send_message(
            f"Brother! You cannot destroy this {amount} of {building}s! you only have {building_result[1]} built!")
    else:
        status = f"You have destroyed {amount} of {building} within your settlement of {settlement}!"
        await ctx.response.send_message(status)
        await EventCommand.destroy_building(kingdom, settlement, building, amount, guild_id, author)


@leadership.command()
async def help(ctx: commands.Context):
    """Help commands for the associated tree"""
    embed = discord.Embed(title=f"Leadership Help", description=f'This is a list of Leadership commands',
                          colour=discord.Colour.blurple())
    embed.add_field(name=f'**Modify**', value=f'Update a leader, shift one leader for another, or fill or vacant spot.',
                    inline=False)
...
async def modify(ctx: commands.Context, kingdom: str, password: str, leader: str, title: str, modifier: int):
    ...
    if kingdom_results[0] != password:
        await ctx.response.send_message(
            f"BEHOLD, THE ULTIMATE POWER OF SUBMITTING THE WRONG PASSWORD FOR THE KINGDOM OF {kingdom}!")
        cursor.close()
        db.close()
        return
    if kingdom_results[0] is None:
        ...
        db.close()
        return
    statistics = ([':red_circle: Strength', 'Strength', '🔴'], [':blue_circle: Dexterity', 'Dexterity', '🔵'],
                  [':green_circle: Constitution', 'Constitution', '🟢'],
                  [':purple_circle: Intelligence', 'Intelligence', '🟣'], [':yellow_circle: Wisdom', 'Wisdom', '🟡'],
                  [':orange_circle: Charisma', 'Charisma', '🟠'])
    cursor.execute(
        f"""SELECT Title, Ability, Description, Economy, Loyalty, Stability FROM AA_Leadership_Roles WHERE Title = '{title}'""",
        {'title': title})
    result = cursor.fetchone()
    if result is None:
    ...
    while kingdom_modifier is None:
        try:
            reaction, user = await bot.wait_for('reaction_add', check=lambda reaction,
                                                                             user: user.id == ctx.user.id and reaction.emoji in buttons2,
                                                timeout=60.0)
        except asyncio.TimeoutError:
            embed.set_footer(text="Request has timed out.")
            await msg.edit(embed=embed)
        ...
        await EventCommand.modify_leader(kingdom, leader, title, modifier, column, economy_modifier,
                                         loyalty_modifier, stability_modifier, guild_id, author)
    cursor.close()
    db.close()
...
async def remove(ctx: commands.Context, kingdom: str, password: str, title: str):
    ...
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    cursor.execute(f"""SELECT Name, Title FROM Leadership WHERE Title = '{title}' AND Kingdom = '{kingdom}'""",
    ...
    if leadership_results[0] is not None and kingdom_results[0] == password:
        await EventCommand.remove_leader(kingdom, title, guild_id, author)
        await ctx.response.send_message(
            f"You have removed {leadership_results[0]} from the position of {leadership_results[1]} for {kingdom}")
...
async def display(ctx: commands.Context, kingdom: str, current_page: int = 1, leader: str = 'All'):
    ...
    if leader == 'All':
        ...
        offset = 10
        cursor.execute(
            f"""SELECT Name, Title, Stat, Modifier, Economy, Loyalty, Stability, Unrest from Leadership WHERE Kingdom = '{kingdom}' LIMIT {low}, {offset}""",
            {'Kingdom': kingdom})
        pull = cursor.fetchall()
        embed = discord.Embed(title=f"{kingdom} Leadership page {current_page}",
                              description=f"This is list of {kingdom}'s leaders", colour=discord.Colour.blurple())
        for result in pull:
            embed.add_field(name=f'Leader info',
                            value=f'**Name**: {result[0]}, **Title**: {result[1]}, **Stat**: {result[2]}, **Modifier**: {result[3]}',
                            inline=False)
            embed.add_field(name=f'Kingdom Benefits',
                            value=f'**Economy**: {result[4]}, **Loyalty**: {result[5]}, **Stability**: {result[5]}, **Unrest**: {result[7]}',
                            inline=False)
        await ctx.response.send_message(embed=embed)
        msg = await ctx.original_response()
        for button in buttons:
        ...
        while True:
            try:
                ...
                if current_page != previous_page:
                    ...
                    edit_pull = cursor.fetchall()
                    embed = discord.Embed(title=f"{kingdom} Leadership page {current_page}",
                                          description=f"This is list of {kingdom}'s leaders",
                                          colour=discord.Colour.blurple())
                    for result in edit_pull:
                        embed.add_field(name=f'Leader info',
                                        value=f'**Name**: {result[0]}, **Title**: {result[1]}, **Stat**: {result[2]}, **Modifier**: {result[3]}',
        ...
        cursor.execute(
            f"""SELECT Name, Title, Stat, Modifier, Economy, Loyalty, Stability, Unrest from Leadership WHERE Kingdom = '{kingdom}' AND Title = '{leader}'""",
            {'Kingdom': kingdom, 'Leader': leader})
        pull = cursor.fetchone()
        if pull is None:
            cursor.execute(
                f"""SELECT Name, Title, Stat, Modifier, Economy, Loyalty, Stability, Unrest from Leadership WHERE Kingdom = '{kingdom}' AND Name = '{leader}'""",
                {'Kingdom': kingdom, 'Leader': leader})
            pull = cursor.fetchone()
            if pull is None:
                ...
                embed = discord.Embed(title=f"{kingdom}'s {leader}",
                                      description=f"This is {kingdom}'s {leader}: {pull[0]}",
                                      colour=discord.Colour.blurple())
                embed.add_field(name=f'Leader info',
                                value=f'**Name**: {pull[0]}, **Title**: {pull[1]}, **Stat**: {pull[2]}, **Modifier**: {pull[3]}',
                                inline=False)
                embed.add_field(name=f'Kingdom Benefits',
                                value=f'**Economy**: {pull[4]}, **Loyalty**: {pull[5]}, **Stability**: {pull[5]}, **Unrest**: {pull[6]}',
                                inline=False)
                await ctx.response.send_message(embed=embed)
        else:
            embed = discord.Embed(title=f"{kingdom}'s {leader}", description=f"This is {kingdom}'s {leader}: {pull[0]}",
                                  colour=discord.Colour.blurple())
            embed.add_field(name=f'Leader info',
                            value=f'**Name**: {pull[0]}, **Title**: {pull[1]}, **Stat**: {pull[2]}, **Modifier**: {pull[3]}',
                            inline=False)
...
async def help(ctx: commands.Context):
    """Help commands for the associated tree"""
    embed = discord.Embed(title=f"Hex Help", description=f'This is a list of Hex administration commands',
                          colour=discord.Colour.blurple())
    embed.add_field(name=f'**Claim**', value=f'Claim an unclaimed hex to be able to improvement.', inline=False)
    embed.add_field(name=f'**Remove**', value=f'Remove a claimed not improved hex from your kingdoms possession.',
                    inline=False)
    ...
    embed.add_field(name=f'**Improvements**', value=f'Display the improved hexes that a kingdom has claimed.',
                    inline=False)
...
@app_commands.choices(hex_terrain=[discord.app_commands.Choice(name='Cavernous', value=1),
                                   discord.app_commands.Choice(name='Coastline', value=2),
                                   discord.app_commands.Choice(name='Desert', value=3),
                                   discord.app_commands.Choice(name='Forest', value=4),
                                   discord.app_commands.Choice(name='Hills', value=5),
                                   discord.app_commands.Choice(name='jungle', value=6),
                                   discord.app_commands.Choice(name='Marsh', value=7),
                                   discord.app_commands.Choice(name='Mountains', value=8),
                                   discord.app_commands.Choice(name='Plains', value=9),
                                   discord.app_commands.Choice(name='Water', value=10)])
async def claim(ctx: commands.Context, kingdom: str, password: str, hex_terrain: discord.app_commands.Choice[int]):
    ...
    cursor.execute(f"""SELECT Kingdom, Password FROM Kingdoms where Kingdom = '{kingdom}'""", {'kingdom': kingdom})
    kingdom_results = cursor.fetchone()
    cursor.execute(f"""SELECT Hex_Terrain FROM AA_Hex_Terrains WHERE Hex_Terrain = '{hex_terrain}'""",
    ...
    if kingdom_results is not None and password == kingdom_results[1] and hex_results is not None:
        ...
        await EventCommand.claim_hex(kingdom, hex_terrain, guild_id, author)
        await ctx.response.send_message(status)
...
async def remove(ctx: commands.Context, kingdom: str, password: str, hex_terrain: discord.app_commands.Choice[int]):
    ...
    kingdom_results = cursor.fetchone()
    cursor.execute(
        f"""SELECT Kingdom, Hex_Terrain FROM Hexes WHERE Kingdom = '{kingdom}' AND Hex_Terrain = '{hex_terrain}' AND Improvement = 'None'""",
        {'kingdom': kingdom, 'Hex_Terrain': hex_terrain})
    hex_results = cursor.fetchone()
    cursor.close()
    db.close()
    if kingdom_results is None:
    ...
    if hex_results is None:
        await ctx.response.send_message(
            f"you do not have any unimproved hexes of the {hex_terrain} hex terrain to release!")
    if kingdom_results is not None and password == kingdom_results[1] and hex_results is not None:
        status = f"You have released a hex from the kingdom of {kingdom}"
        await EventCommand.relinquish_hex(kingdom, hex_terrain, guild_id, author)
        await ctx.response.send_message(status)
...
@app_commands.choices(hex_terrain=[discord.app_commands.Choice(name='Cavernous', value=1),
                                   ...
                                   discord.app_commands.Choice(name='Plains', value=9),
                                   discord.app_commands.Choice(name='Water', value=10)])
async def improve(ctx: commands.Context, kingdom: str, password: str, hex_terrain: discord.app_commands.Choice[int],
                  improvement: str):
    ...
    kingdom_results = cursor.fetchone()
    cursor.execute(f"""SELECT {hex_terrain} FROM Hexes_Improvements where Improvement = '{improvement}'""",
                   {'Improvement': improvement})
    improvement_result = cursor.fetchone()
    cursor.execute(
        f"""SELECT Amount FROM Hexes where Improvement = 'None' AND Hex_Terrain = '{hex_terrain}' AND kingdom = '{kingdom}'""")
    hex_results = cursor.fetchone()
    cursor.close()
    db.close()
    if improvement_result is not None:
        ...
        if kingdom_results is not None and kingdom_results[1] == password:
            if hex_results is not None:
                ...
                await EventCommand.improve_hex(kingdom, hex_terrain, improvement, guild_id, author)
                await ctx.response.send_message(status)
...
@app_commands.choices(hex_terrain=[discord.app_commands.Choice(name='Cavernous', value=1),
                                   ...
                                   discord.app_commands.Choice(name='Plains', value=9),
                                   discord.app_commands.Choice(name='Water', value=10)])
async def diminish(ctx: commands.Context, kingdom: str, password: str, hex_terrain: discord.app_commands.Choice[int],
                   improvement: str):
    ...
    kingdom_results = cursor.fetchone()
    cursor.execute(
        f"""SELECT Kingdom FROM Hexes where Kingdom = '{kingdom}' AND Hex_Terrain = '{hex_terrain}' AND Improvement = '{improvement}'""",
        {'Kingdom': kingdom, 'Hex_Terrain': hex_terrain, 'Improvement': improvement})
    ...
    if kingdom_results is None:
        ...
        await ctx.response.send_message(
            f"You are lacking a hex that matches the {hex_terrain} hex_terrain or {improvement} improvement")
        ...
        await EventCommand.diminish_hex(kingdom, hex_terrain, improvement, guild_id, author)
        await ctx.response.send_message(status)
...
async def improvements(ctx: commands.Context, kingdom: str, current_page: int = 1):
    ...
    offset = 10
    cursor.execute(
        f"""SELECT Hex_Terrain, Amount, Improvement, Economy, Loyalty, Stability, Unrest, Consumption, Defence, Taxation from Hexes WHERE Kingdom = '{kingdom}' LIMIT {low}, {offset}""",
        {'Kingdom': kingdom})
    pull = cursor.fetchall()
    embed = discord.Embed(title=f"{kingdom} Hex page {current_page}",
                          description=f"This is list of the hexes owned by {kingdom}", colour=discord.Colour.blurple())
    for result in pull:
        embed.add_field(name=f'Hex Information',
                        value=f'**Terrain**: {result[0]}, **Improvement*: {result[2]}, **Amount**: {result[1]}',
                        inline=False)
        embed.add_field(name=f'Hex Benefits',
                        value=f'**Economy**: {result[3]}, **Loyalty**: {result[4]}, **Stability**: {result[5]}, **Unrest**: {result[6]} **Consumption**: {result[7]} **Defence**: {result[8]}, **Taxation**: {result[9]}',
                        inline=False)
    await ctx.response.send_message(embed=embed)
    msg = await ctx.original_response()
    for button in buttons:
    ...
    while True:
        try:
            ...
            if current_page != previous_page:
                ...
                pull = cursor.fetchall()
                embed = discord.Embed(title=f"{kingdom} Hex page {current_page}",
                                      description=f"This is list of the hexes owned by {kingdom}",
                                      colour=discord.Colour.blurple())
                for result in pull:
                    embed.add_field(name=f'Hex Information',
                                    value=f'**Terrain**: {result[0]}, **Improvement*: {result[2]}, **Amount**: {result[1]}',
...
async def store(ctx: commands.Context, current_page: int = 1):
    ...
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    cursor.execute(f"""SELECT COUNT(Improvement) FROM Hexes_Improvements where Kingdom = '{kingdom}'""",
                   {'Kingdom': kingdom})
    leaders = cursor.fetchone()
    max_page = math.ceil(leaders[0] / 5)
    if current_page >= max_page:
    ...
    cursor.execute(
        ...
        {'Kingdom': kingdom})
    pull = cursor.fetchall()
    embed = discord.Embed(title=f"Available Improvements Page {current_page}",
                          description=f"This is list of the available hex improvements.",
                          colour=discord.Colour.blurple())
    for result in pull:
        embed.add_field(name=f'Hex Information', value=f'**Improvement*: {result[2]}, **Cost**: {result[2]} BP',
                        inline=False)
        embed.add_field(name=f'Hex Benefits',
                        value=f'**Economy**: {result[3]}, **Loyalty**: {result[4]}, **Stability**: {result[5]}, **Unrest**: {result[6]} **Consumption**: {result[7]} **Defence**: {result[8]}, **Taxation**: {result[9]}',
                        inline=False)
        embed.add_field(name=f"Buildable Terrains",
                        value=f'**Cavernous**: {result[10]}, **Coastline**: {result[11]}, **Desert**: {result[12]}, **Forest**: {result[13]}, **Hills**: {result[14]}, **Jungle**: {result[15]}, **Marsh**: {result[16]}, **Mountains**: {result[17]}, **Plains**: {result[18]}, **Water**: {result[19]}')
    await ctx.response.send_message(embed=embed)
    msg = await ctx.original_response()
    for button in buttons:
        await msg.add_reaction(button)
    while True:
        try:
            ...
            if current_page != previous_page:
                ...
                for result in pull:
                    embed.add_field(name=f'Hex Information',
                                    value=f'**Improvement*: {result[2]}, **Cost**: {result[2]} BP', inline=False)
                    embed.add_field(name=f'Hex Benefits',
                                    value=f'**Economy**: {result[3]}, **Loyalty**: {result[4]}, **Stability**: {result[5]}, **Unrest**: {result[6]} **Consumption**: {result[7]} **Defence**: {result[8]}, **Taxation**: {result[9]}',
                                    inline=False)
                    embed.add_field(name=f"Buildable Terrains",
...
async def register(ctx: commands.Context, character_name: str, mythweavers: str, image_link: str, nickname: str = None,
    ...
    if len(color) == 7 and match:
        ...
        if results is None and results2 is None:
            int_color = int(color[1:], 16)
            await EventCommand.stage_character(true_character_name, character_name, author, author_id, guild_id,
                                               nickname, titles, description, oath_name, mythweavers, image_link, color,
                                               backstory)
            await EventCommand.stg_gold_change(guild_id, author, author_id, character_name, starting_gold,
            ...
            try:
                ...
                await ctx.response.send_message(embed=embed)
                await EventCommand.fix_character(guild_id, character_name)
        else:
    ...
    if results is None:
        ...
        if results is None:
            ...
            if new_character_name is not None:
                new_character_name = str.replace(str.replace(str.replace(str.replace(str.replace(str.title(
                    new_character_name),
                    ";", ""), ")", ""), "(", ""), "[", ""),"]", "")
            ...
            if len(color) == 7 and match:
                ...
                if results is not None:
                    ...
                    if oath_name != results[6]:
                        await EventCommand.gold_set(guild_id, author, author_id, name, gold, gold, gold,
                                                    'Oath Change', 'Character Edit', 2)
                    await EventCommand.edit_stg_character(true_name, true_character_name, new_character_name,
        ...
        if len(color) == 7 and match:
            if results is not None:
                ...
                if oath_name != results[6] and results[8] < 7:
                    if oath == 2:
                        await EventCommand.gold_set(guild_id, author, author_id, name, results[14] / 2,
                                                    results[15] - results[14] / 2, results[15], 'Oath Change',
                                                    'Character Edit', 1)
                    else:
                        await EventCommand.gold_set(guild_id, author, author_id, name, results[14], results[15],
                                                    results[15], 'Oath Change', 'Character Edit', 1)
                print(f"printing {oath_name}")
                await EventCommand.edit_character(true_name, true_character_name, new_character_name, guild_id,
                                                  new_nickname, titles, description, oath_name, mythweavers, image_link,
                                                  color, author)
                if results[23] is not None:
                    await EventCommand.edit_bio(guild_id, new_character_name, None, results[22])
                cursor.execute(
        ...
        while True:
            try:
                ...
                if reaction.emoji == u"\u2705":
                    ...
                    await msg.edit(embed=embed)
                    await msg.clear_reactions()
                    await EventCommand.retire_character(guild_id, true_character_name, author)
                        ...
                        for x in range(amount):
                            if character_level < int_max_level:
                                ...
                                if x + 1 == amount or character_level == int_max_level or character_level == personal_cap:
                                    await EventCommand.adjust_milestones(true_character_name, milestone_total,
                        ...
                        for x in range(amount):
                            if tier < tier_max:
                                ...
                                if x + 1 == amount or tier == tier_max:
                                    ...
                                    await EventCommand.adjust_trials(character_name, trial_total, guild_id,
                                                                     author)
                                    await client.delete_inventory_item(guild_id, author_id, item_id[0], used)
                                    cursor.execute(
                                        f"Select Search FROM Admin WHERE Identifier = 'Accepted_Bio_Channel'")
        ...
        if inventory is not None:
            ...
            if player_info is not None:
                ...
                if wpl_info[0] <= player_info[15]:
                    ...
                    gold_info = gold_calculation(player_info[7], player_info[6], player_info[13], player_info[14],
                                                 player_info[15], gold)
                    await EventCommand.gold_change(guild_id, author, author_id, true_character_name, gold_info[3],
    ...
    if usage != 1 and usage != 4 and item_id is not None:
            ...
            if inventory is not None:
                ...
                if player_info is not None:
                    ...
                    if item_id[1] <= title_fame:
                        ...
                        await EventCommand.title_change(guild_id, author, author_id, true_character_name,
        ...
        if player_info is not None:
            ...
            if title_information is not None:
                ...
                await EventCommand.title_change(guild_id, author, author_id, true_character_name, title_name,
    ...
    if modify == 2 and approver != "N/A" and item_id is not None:
        ...
        if player_info is not None:
            ...
            if title_information[0] < item_id[4] and player_info[27] >= item_id[0] and player_info[30] >= item_id[1]:
                await EventCommand.proposition_open(guild_id, author, author_id, player_info[3], item_id[2],
...
async def cap(ctx: commands.Context, character_name: str, level_cap: int):
    ...
    if character_info is not None:
        await EventCommand.adjust_personal_cap(guild_id, author, character_name, level_cap)
    ...
    if character_info is not None:
        if modify == 1:
            if character_info[1] is not None:
                ...
                await EventCommand.create_bio(guild_id, character_info[0], backstory, character_info[2])
                cursor.execute(f"Select Search FROM Admin WHERE Identifier = 'Accepted_Bio_Channel'")
                accepted_bio_channel = cursor.fetchone()
                cursor.execute(f"Select Search FROM Admin WHERE Identifier = 'Char_Eventlog_Channel'")
                character_log_channel_id = cursor.fetchone()
            ...
            if character_info[1] is None:
                ...
                await EventCommand.edit_bio(guild_id, character_info[0], backstory, character_info[1])
                cursor.execute(f"Select Search FROM Admin WHERE Identifier = 'Accepted_Bio_Channel'")
                accepted_bio_channel = cursor.fetchone()
                cursor.execute(f"Select Search FROM Admin WHERE Identifier = 'Char_Eventlog_Channel'")
                character_log_channel_id = cursor.fetchone()
        ...
        character_info = cursor.fetchone()
        if character_info is not None:
            await EventCommand.edit_stage_bio(guild_id, character_info[0], backstory)
        else:
            await ctx.response.send_message(
                f"{author} does not have a {character_name} registered under this character name or nickname.",
                ephemeral=True)
...
async def help(ctx: commands.Context):
    """Help commands for the associated tree"""
    embed = discord.Embed(title=f"Gold Help", description=f'This is a list of Gold management commands',
                          colour=discord.Colour.blurple())
...
async def claim(ctx: commands.Context, character_name: str, amount: float, reason: str):
    ...
    if amount <= 0:
        await ctx.response.send_message(
            f"Little comrade! Please give yourself some credit! {amount} is too small to claim!")
    elif amount > 0:
        cursor.execute(
            f"SELECT Player_Name, Player_ID, True_Character_Name, Character_Name, Titles, Description, Oath, Level, Tier, Milestones, Milestones_Required, Trials, Trials_Required, Gold, Gold_Value, Gold_value_Max, Flux, Color, Mythweavers, Image_Link, Tradition_Name, Tradition_Link, Template_Name, Template_Link, Message_ID, Logging_ID, Thread_ID, Fame, Title, Personal_Cap, Prestige, Article_Link FROM Player_Characters where Player_Name = ? AND Character_Name = ? or Nickname = ?",
            (author, character_name, character_name))
        ...
        if player_info is None:
            ...
            gold_info = gold_calculation(player_info[7], player_info[6], player_info[13], player_info[14],
                                         player_info[15], amount)
            await EventCommand.gold_change(guild_id, author, author_id, character_name, gold_info[3],
                                           gold_info[3], amount, reason, 'Gold_Claim')
            cursor.execute(f'SELECT MAX(Transaction_ID) FROM A_Audit_Gold Order By Transaction_ID DESC LIMIT 1')
            transaction_id = cursor.fetchone()
            cursor.execute(f"Select Search FROM Admin WHERE Identifier = 'Accepted_Bio_Channel'")
            accepted_bio_channel = cursor.fetchone()
            bio_embed = character_embed(player_info[0], player_info[1], player_info[2], player_info[4], player_info[5],
                                        player_info[6], player_info[7], player_info[8], player_info[9], player_info[10],
                                        player_info[11], player_info[12], player_info[13] + gold_info[3],
                                        player_info[14] + gold_info[3], player_info[16], player_info[17],
                                        player_info[18], player_info[19], player_info[20], player_info[21],
                                        player_info[22], player_info[23], player_info[27], player_info[28],
                                        player_info[30], player_info[31])
            bio_channel = await bot.fetch_channel(accepted_bio_channel[0])
            ...
            source = f"Character has increased their wealth by {gold_info[3]} GP using a gold pouch from the shop, transaction_id: {transaction_id[0]}!"
            logging_embed = log_embed(player_info[2], author, None, None, None, None, None, None, None, None,
                                      player_info[13] + gold_info[3], gold_info[3], player_info[14] + gold_info[3],
                                      transaction_id[0], None, None, None, None, None, None, None, None, None, None,
                                      None, source)
            logging_thread = guild.get_thread(player_info[25])
            cursor.close()
            db.close()
            await logging_thread.send(embed=logging_embed)
            await ctx.response.send_message(
                f"{player_info[2]} has claimed {amount} gold, receiving {gold_info[3]} gold and now has {gold_info[0]} gold!.")
            ...
            if player_info[13] >= abs(expenditure):
                ...
                if player_info[6] == 'Poverty':
                    ...
                    if gold_value > max_wealth:
                        await ctx.response.send_message(
                            f"{player_info[2]} has too much money and needs to give some to charitable causes by using the 'buy' command where they receive nothing in return!")
                        return
                elif player_info[6] == 'Absolute':
                ...
                await EventCommand.gold_change(guild_id, author, author_id, character_name, expenditure,
                                               market_value_adjusted, market_value_adjusted, reason, 'Gold_Buy')
                cursor.execute(f'SELECT MAX(Transaction_ID) FROM A_Audit_Gold Order By Transaction_ID DESC LIMIT 1')
                transaction_id = cursor.fetchone()
                cursor.execute(f"Select Search FROM Admin WHERE Identifier = 'Accepted_Bio_Channel'")
                accepted_bio_channel = cursor.fetchone()
                bio_embed = character_embed(player_info[0], player_info[1], player_info[2], player_info[4],
                                            player_info[5], player_info[6], player_info[7], player_info[8],
                                            player_info[9], player_info[10], player_info[11], player_info[12],
                                            player_info[13] + expenditure, player_info[14] + market_value_adjusted,
                                            player_info[16], player_info[17], player_info[18], player_info[19],
                                            player_info[20], player_info[21], player_info[22], player_info[23],
                                            player_info[27], player_info[28], player_info[30], player_info[31])
                bio_channel = await bot.fetch_channel(accepted_bio_channel[0])
                ...
                logging_embed = log_embed(player_info[2], author, None, None, None, None, None, None, None, None,
                                          player_info[13] + expenditure, expenditure,
                                          round(player_info[14] + market_value_adjusted, 2), transaction_id[0], None,
                                          None, None, None, None, None, None, None, None, None, None, source)
                logging_thread = guild.get_thread(player_info[25])
                print(expenditure)
                await logging_thread.send(embed=logging_embed)
                await ctx.response.send_message(
                    f"{player_info[2]} has spent {abs(expenditure)} gold and has received {market_value_adjusted} in expected value return. Leaving them with {remaining} gold.")
            else:
...
async def consume(ctx: commands.Context, character_name: str, consumption: float, reason: str):
    ...
    if consumption <= 0:
        await ctx.response.send_message(
            f"MY FREN, YOU MUST BE CONSUMING GOLD. {consumption} IS FAR TOO LOW A NUMBER. REMEMBER, IF IT ISN'T TOIGHT, IT ISN'T ROIGHT.")
    else:
        cursor.execute(
            f"SELECT Player_Name, Player_ID, True_Character_Name, Character_Name, Titles, Description, Oath, Level, Tier, Milestones, Milestones_Required, Trials, Trials_Required, Gold, Gold_Value, Gold_value_Max, Flux, Color, Mythweavers, Image_Link, Tradition_Name, Tradition_Link, Template_Name, Template_Link, Message_ID, Logging_ID, Thread_ID, Fame, Title, Personal_Cap, Prestige, Article_Link FROM Player_Characters where Player_Name = ? AND Character_Name = ? or  Player_Name = ? AND Nickname = ?",
            (author, character_name, author, character_name))
        ...
        if player_info[0] is None:
            ...
            if player_info[14] - player_info[13] >= abs(consumption):
                ...
                await EventCommand.gold_change(guild_id, author, author_id, character_name, player_info[13],
                ...
                bio_embed = character_embed(player_info[0], player_info[1], player_info[2], player_info[4],
                                            player_info[5], player_info[6], player_info[7], player_info[8],
                                            player_info[9], player_info[10], player_info[11], player_info[12],
                                            player_info[13], player_info[14] + consumption, player_info[16],
                                            player_info[17], player_info[18], player_info[19], player_info[20],
                                            player_info[21], player_info[22], player_info[23], player_info[27],
                                            player_info[28], player_info[30], player_info[31])
                bio_channel = await bot.fetch_channel(accepted_bio_channel[0])
                bio_message = await bio_channel.fetch_message(player_info[24])
                await bio_message.edit(content=bio_embed[1], embed=bio_embed[0])
                ...
                await logging_thread.send(embed=logging_embed)
                await ctx.response.send_message(
                    f"{player_info[2]} has consumed {abs(consumption)} gold by using {reason}. Leaving them with {remaining} illiquid wealth.")
            else:
...
async def send(ctx: commands.Context, character_from: str, character_to: str, amount: float, expected_value: float,
               reason: str):
    ...
    if amount <= 0:
        await ctx.response.send_message(
            f"Little comrade! Please SEND something of actual value! {amount} is too small to claim!")
    elif expected_value < 0:
        await ctx.response.send_message(f"Expected Value cannot be less than 0!")
    elif expected_value < amount:
        await ctx.response.send_message(
            f"If they're charging you higher than the market value, go buy it from an NPC...")
    else:
        cursor.execute(
            f"SELECT Player_Name, Player_ID, True_Character_Name, Character_Name, Titles, Description, Oath, Level, Tier, Milestones, Milestones_Required, Trials, Trials_Required, Gold, Gold_Value, Gold_value_Max, Flux, Color, Mythweavers, Image_Link, Tradition_Name, Tradition_Link, Template_Name, Template_Link, Message_ID, Logging_ID, Thread_ID, Fame, Title, Personal_Cap, Prestige, Article_Link FROM Player_Characters where Player_Name = ? AND Character_Name = ? or Player_Name = ? AND Nickname = ?",
        ...
        if player_info[0] is None:
            ...
            if player_info[13] < amount:
                await ctx.response.send_message(
                    f"Unlike America, you can't go into debt to resolve your debt. {player_info[1] - amount} leaves you too in debt.")
            else:
                cursor.execute(
                    f"SELECT Player_Name, Player_ID, True_Character_Name, Character_Name, Titles, Description, Oath, Level, Tier, Milestones, Milestones_Required, Trials, Trials_Required, Gold, Gold_Value, Gold_value_Max, Flux, Color, Mythweavers, Image_Link, Tradition_Name, Tradition_Link, Template_Name, Template_Link, Message_ID, Logging_ID, Thread_ID, Fame, Title, Personal_Cap, Prestige, Article_Link FROM Player_Characters where Character_Name = ? or Nickname = ? ",
                    (character_to, character_to))
                ...
                if send_info is None:
                    ...
                    buttons = ["✅", "❌"]  # checkmark X symbol
                    embed = discord.Embed(
                        title=f"Are you sure you want {character_from} to send {amount} GP to {character_to}?",
                        description=f"hit the checkmark to confirm", colour=discord.Colour.blurple())
                    await ctx.response.send_message(embed=embed)
                    msg = await ctx.original_response()
                    for button in buttons:
                    ...
                    while True:
                        try:
                            ...
                            if reaction.emoji == u"\u2705":
                                embed = discord.Embed(
                                    title=f"{character_from} has given {amount} in GP to {character_to}",
                                    description=f"Hope it was worth it.", colour=discord.Colour.red())
                                await msg.clear_reactions()
                                ...
                                await EventCommand.gold_change(guild_id, author, author_id, character_from,
                                                               -abs(amount), expected_value_adjusted,
                                                               expected_value_adjusted, reason, 'gold send')
                                cursor.execute(
                                    f"Select MAX(Transaction_ID) from A_Audit_Gold order by Transaction_ID desc limit 1")
                                transaction_id_from = cursor.fetchone()
                                await EventCommand.gold_change(guild_id, send_info[0], send_info[1], send_info[2],
                                                               amount, amount, amount, reason, 'Gold_Buy')
                                cursor.execute(
                                    f"Select MAX(Transaction_ID) from A_Audit_Gold order by Transaction_ID desc limit 1")
                                ...
                                bio_embed = character_embed(player_info[0], player_info[1], player_info[2],
                                                            player_info[4], player_info[5], player_info[6],
                                                            player_info[7], player_info[8], player_info[9],
                                                            player_info[10], player_info[11], player_info[12],
                                                            player_info[13] + -abs(amount),
                                                            player_info[14] + expected_value_adjusted, player_info[16],
                                                            player_info[17], player_info[18], player_info[19],
                                                            player_info[20], player_info[21], player_info[22],
                                                            player_info[23], player_info[27], player_info[28],
                                                            player_info[30], player_info[31])
                                ...
                                logging_embed = log_embed(player_info[2], author, None, None, None, None, None, None,
                                                          None, None, player_info[13] + -abs(amount), -abs(amount),
                                                          player_info[14] + expected_value_adjusted,
                                                          transaction_id_from[0], None, None, None, None, None, None,
                                                          None, None, None, None, None, source)
                                logging_thread = guild.get_thread(player_info[25])
                                await logging_thread.send(embed=logging_embed)
                                to_bio_embed = character_embed(send_info[0], send_info[1], send_info[2], send_info[4],
                                                               send_info[5], send_info[6], send_info[7], send_info[8],
                                                               send_info[9], send_info[10], send_info[11],
                                                               send_info[12], send_info[13] + amount,
                                                               send_info[14] + expected_value_adjusted, send_info[16],
                                                               send_info[17], send_info[18], send_info[19],
                                                               send_info[20], send_info[21], send_info[22],
                                                               send_info[23], send_info[27], send_info[28],
                                                               send_info[30], send_info[31])
                                ...
                                to_logging_embed = log_embed(send_info[0], author, None, None, None, None, None, None,
                                                             None, None, send_info[13] + amount, amount,
                                                             send_info[14] + amount, transaction_id_to[0], None, None,
                                                             None, None, None, None, None, None, None, None, None,
                                                             to_source)
                                to_logging_thread = guild.get_thread(send_info[25])
                                await to_logging_thread.send(embed=to_logging_embed)
                                await EventCommand.gold_transact(transaction_id_from[0], transaction_id_to[0],
                                                                 guild_id)
                                await EventCommand.gold_transact(transaction_id_to[0], transaction_id_from[0],
                                                                 guild_id)
...
async def history(ctx: commands.Context, character_name: str, current_page: int = 1):
    ...
    offset = 8
    cursor.execute(
        f"""Select Transaction_ID, Author_Name, Character_Name, Gold_Value, Effective_Gold_Value, Effective_Gold_Value_Max, Reason, Source_Command, Time from A_Audit_Gold WHERE Character_Name = ? LIMIT {low}, {offset}""",
        (character,))
    pull = cursor.fetchall()
    embed = discord.Embed(title=f"{character} character page {current_page}",
                          description=f"This is list of {character}'s transactions", colour=discord.Colour.red())
    for result in pull:
        embed.add_field(name=f'Transaction Information', value=f'**Date**: {result[8]}, **Source**: {result[7]}',
                        inline=False)
        embed.add_field(name=f'Changes:',
                        value=f'{result[3]} Liquid GP {result[4]} Effective GP, {result[5]} Life Time GP')
    ...
    while True:
        try:
            ...
            if current_page != previous_page:
                ...
                pull = cursor.fetchall()
                embed = discord.Embed(title=f"{character} character page {current_page}",
                                      description=f"This is list of {character}'s transactions",
                                      colour=discord.Colour.red())
                for result in pull:
                    embed.add_field(name=f'Transaction Information',
                                    value=f'**Date**: {result[8]}, **Source**: {result[7]}', inline=False)
                    embed.add_field(name=f'Changes:',
                                    value=f'{result[3]} Liquid GP {result[4]} Effective GP, {result[5]} Life Time GP')
                    embed.add_field(name=f'Transaction:', value=f'Transaction_ID: {result[0]}, Reason: {result[6]}',
                                    inline=False)
                await msg.edit(embed=embed)
...
async def help(ctx: commands.Context):
    ...
    embed.add_field(name=f'**Proposition**', value=f'**GAMEMASTER**: Accept or Reject A Proposition based on the ID!',
                    inline=False)
    embed.add_field(name=f'**Glorify**', value=f'**GAMEMASTER**: Increase or decrease characters fame and prestige!',
                    inline=False)
    embed.add_field(name=f'**Plot**', value='**GAMEMASTER**: Create or Modify a plot for your session series!',
                    inline=False)
    embed.add_field(name=f'**Questify**', value='**GAMEMASTER**: Remove a request!', inline=False)
...
async def help(ctx: commands.Context):
    """Help commands for the associated tree"""
    embed = discord.Embed(title=f"Player Help", description=f'This is a list of Playerside commands',
                          colour=discord.Colour.blurple())
    ...
    embed.add_field(name=f'**Request**', value=f'Mark a session request using a character and create a session group.',
                    inline=False)
    ...
    embed.add_field(name=f'**Availability**',
                    value=f'Display Availability of a Specific Player on a specific day of the week.', inline=False)
    await ctx.response.send_message(embed=embed)
...
async def proposition(ctx: commands.Context, proposition_id: int, reason: typing.Optional[str],
    ...
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    cursor.execute(f"SELECT Character_Name, Item_Name from A_Audit_Prestige Where Transaction_ID = ?",
                   (proposition_id,))
    item_id = cursor.fetchone()
    ...
    if item_id is not None:
        if acceptance == 1:
            ...
            player_info = cursor.fetchone()
            logging_embed = discord.Embed(
                title=f"{author.capitalize()} has accepted the proposition of {item_id[0]} using {item_id[1]}!",
                description=f"Proposition ID: {proposition_id}", color=discord.Colour.red())
            ...
            cursor.execute(f"Select Thread_ID from Player_Characters where Character_Name = ?", (item_id[0],))
            player_info = cursor.fetchone()
            logging_embed = discord.Embed(title=f"{author} has rejected the proposition!",
                                          description=f"Proposition ID: {proposition_id}", color=discord.Colour.red())
            logging_embed.add_field(name=f'Reason:', value=f'{reason}', inline=False)
            logging_thread = guild.get_thread(player_info[0])
            source = "proposition"
            await logging_thread.send(embed=logging_embed)
            await EventCommand.proposition_reject(guild_id, author, proposition_id, reason, source)
...
async def glorify(ctx: commands.Context, character: str, fame: int, prestige: int, reason: typing.Optional[str]):
    ...
    if player_info is not None:
        ...
        logging_embed.add_field(name=f'New Totals:',
                                value=f'**fame**: {fame + player_info[1]}, **prestige**: {prestige + player_info[2]} ',
                                inline=False)
        logging_thread = guild.get_thread(player_info[0])
        ctx.response.send_message(embed=logging_embed)
        await logging_thread.send(embed=logging_embed)
        await EventCommand.glorify(guild_id, author, character, fame + player_info[1], prestige + player_info[2],
                                   reason)
    else:
...
@app_commands.choices(overflow=[discord.app_commands.Choice(name='current range only!', value=1),
...
async def create(interaction: discord.Interaction, session_name: str, session_range: discord.Role, player_limit: int,
                 play_location: str, game_link: typing.Optional[str], group_id: typing.Optional[int], hammer_time: str,
                 overview: str, description: str, plot: str = '9762aebb-43ae-47d5-8c7b-30c34a55b9e5',
    ...
    if hammer_timing == "<t:":
        ...
        hour = "<t:" + timing + ":t>"
        arrival = "<t:" + timing + ":R>"
    await EventCommand.create_session(author, session_name, session_range_name, session_range_id, play_location,
                                      timing, game_link, guild_id, author, overview, description, player_limit,
                                      overflow, plot)
    ...
    try:
        ...
        if group_id is not None:
            ...
            if group_base is not None:
                ...
                await EventCommand.clear_group(guild_id, group_id)
                for player in group_base:
                    session_text += f'<@{player[3]}>'
        msg = await session_channel.send(content=session_text, embed=embed,
                                         allowed_mentions=discord.AllowedMentions(roles=True))
        message = msg.id
        thread = await msg.create_thread(name=f"{info[0]}: {session_name}", auto_archive_duration=60,
                                         reason=f"{description}")
        await EventCommand.create_session_message(info[0], message, thread.id, guild_id)
        ...
        thread = await msg.create_thread(name=f"Session name: {session_name} Session ID: {info[0]}",
                                         auto_archive_duration=60, reason=f"{description}")
        message = msg.id
        await interaction.response.send_message(f"Session created! Session ID: {info[0]}.", ephemeral=True)
        await EventCommand.create_session_message({info[0]}, message, thread.id, guild_id)
        cursor.close()
...
@app_commands.choices(overflow=[discord.app_commands.Choice(name='current range only!', value=1),
                                ...
                                discord.app_commands.Choice(name='include lower level bracket!', value=3),
                                discord.app_commands.Choice(name='ignore role requirements!', value=4)])
async def edit(interaction: discord.Interaction, session_id: int, session_range: typing.Optional[discord.Role],
               session_name: typing.Optional[str], player_limit: typing.Optional[int],
               play_location: typing.Optional[str], game_link: typing.Optional[str], hammer_time: typing.Optional[str],
               overview: typing.Optional[str], description: typing.Optional[str],
    ...
    if info is not None:
        ...
        if game_link is not None:
            ...
            if str.lower(game_link_valid) == 'http':
                embed = discord.Embed(title=f"{session_name}", url=f'{game_link}',
                                      description=f"Play Location: {play_location}", color=discord.Colour.blue())
            else:
                game_link = 'HTTPS://' + game_link
                embed = discord.Embed(title=f"{session_name}", url=f'{game_link}',
                                      description=f"Play Location: {play_location}", color=discord.Colour.blue())
            ...
            embed = discord.Embed(title=f"{session_name}", description=f"Play Location: {play_location}",
        ...
        print(footer_text)
        await EventCommand.edit_session(guild_id, author, session_id, session_name, session_range_name,
                                        session_range_id, play_location, timing, game_link, overflow)
        embed.set_author(name=f'{author}')
        embed.add_field(name="Session Range", value=session_ranges)
        embed.add_field(name="Play Location", value=f'{play_location}')
        embed.add_field(name="Player Limit", value=f'{player_limit}')
        ...
        await interaction.response.send_message(
            content=f"The following session of {session_name} located at {msg.jump_url} has been updated.",
            allowed_mentions=discord.AllowedMentions(roles=True, ))
    if info is None:
...
async def delete(interaction: discord.Interaction, session_id: int):
    ...
    if info is not None:
        embed = discord.Embed(title=f"{info[1]}", description=f"This session has been cancelled.",
                              color=discord.Colour.red())
        await EventCommand.delete_session(session_id, guild_id, author)
        embed.set_author(name=f'{author}')
        ...
        await msg.edit(embed=embed)
        await interaction.response.send_message(
            content=f"the following session of {info[2]} located at {msg.jump_url} has been cancelled.", ephemeral=True)
...
async def accept(interaction: discord.Interaction, session_id: int, player_1: typing.Optional[discord.Member],
    ...
    cursor = db.cursor()
    cursor.execute(
        f"SELECT Session_Name, Play_location, Play_Time, game_link FROM Sessions WHERE Session_ID = '{session_id}' AND GM_Name = '{author}'")
    ...
    if session_info is None:
        ...
        await interaction.response.send_message(
            f"Cannot have a negative number of randomized players! {randomizer} is not acceptable!")
    else:
        ...
        if accepted_players[0] < 20:
            ...
            if players[0] > 0:
                ...
                if randomizer == 0 and player_1 is None and player_2 is None and player_3 is None and player_4 and player_5 is None and player_6 is None and specific_character is None:
                    ...
                    if player_1 is not None:
                        ...
                        if player_info is None and signups_information is None:
                            embed.add_field(name=f'**Not Accepted!**:',
                                            value=f"<@{player_1.id}> had no characters signed up for this session!")
                        elif signups_information is not None:
                            embed.add_field(name=f'**No Duplicates!**:',
                                            value=f" <@{player_1.id}> already has {signups_information[0]} signed up!")
                        else:
                            accepted_characters += 1
                            await EventCommand.accept_player(guild_id, session_info[0], session_id,
                                                             player_info[0], player_info[1], player_info[2],
                                                             player_1.name, player_1.id, author, player_info[3])
                            embed.add_field(name=f'{player_info[0]}',
                                            value=f"has been accepted with Player: <@{player_1.id}>!")
                    if player_2 is not None and player_2 != player_1:
                        ...
                        if player_info is None and signups_information is None:
                            embed.add_field(name=f'**Not Accepted!**:',
                                            value=f"<@{player_2.id}> had no characters signed up for this session!")
                        elif signups_information is not None:
                            embed.add_field(name=f'**No Duplicates!**:',
                                            value=f" <@{player_2.id}> already has {signups_information[0]} signed up!")
                        else:
                            accepted_characters += 1
                            await EventCommand.accept_player(guild_id, session_info[0], session_id,
                                                             player_info[0], player_info[1], player_info[2],
                                                             player_2.name, player_2.id, author, player_info[3])
                            embed.add_field(name=f'{player_info[0]}',
                                            value=f"has been accepted with Player: <@{player_2.id}>!")
                    if player_3 is not None and player_3 != player_1 and player_3 != player_2:
                        ...
                        if player_info is None:
                            embed.add_field(name=f'**Not Accepted!**:',
                                            value=f"<@{player_3.id}> had no characters signed up for this session!")
                        elif signups_information is not None and signups_information is None:
                            embed.add_field(name=f'**No Duplicates!**:',
                                            value=f" <@{player_3.id}> already has {signups_information[0]} signed up!")
                        else:
                            accepted_characters += 1
                            await EventCommand.accept_player(guild_id, session_info[0], session_id,
                                                             player_info[0], player_info[1], player_info[2],
                                                             player_3.name, player_3.id, author, player_info[3])
                            embed.add_field(name=f'{player_info[0]}',
                                            value=f"has been accepted with Player: <@{player_3.id}>!")
                    if player_4 is not None and player_4 != player_1 and player_4 != player_2 and player_4 != player_3:
                        ...
                        if player_info is None:
                            ...
                            await EventCommand.accept_player(guild_id, session_info[0], session_id,
                                                             player_info[0],
                                                             player_info[1], player_info[2], player_4.name, player_4.id,
                                                             author,
                                                             player_info[3])
                            embed.add_field(name=f'{player_info[0]}',
                    ...
                    if player_5 is not None and player_5 != player_1 and player_5 != player_2 and player_5 != player_3 and player_5 != player_4:
                        ...
                        if player_info is None:
                            ...
                            await EventCommand.accept_player(guild_id, session_info[0], session_id,
                                                             player_info[0],
                                                             player_info[1], player_info[2], player_5.name, player_5.id,
                                                             author,
                                                             player_info[3])
                            embed.add_field(name=f'{player_info[0]}',
                    ...
                    if player_6 is not None and player_6 != player_1 and player_6 != player_2 and player_6 != player_3 and player_6 != player_4 and player_6 != player_5:
                        ...
                        if player_info is None:
                            ...
                            await EventCommand.accept_player(guild_id, session_info[0], session_id,
                                                             player_info[0],
                                                             player_info[1], player_info[2], player_6.name, player_6.id,
                                                             author,
                                                             player_info[3])
                            embed.add_field(name=f'{player_info[0]}',
                    ...
                    if specific_character is not None:
                        ...
                        if player_info is not None:
                            ...
                            await EventCommand.accept_player(guild_id, session_info[0], session_id,
                                                             player_info[0], player_info[1], player_info[2],
                                                             player_info[3], player_info[4], author, player_info[5])
                            embed.add_field(name=f'{player_info[0]}',
                                            value=f"has been accepted with player: @{player_info[4]}!")
                    ...
                    if randomizer > 0:
                        ...
                        for x in range(randomizer):
                            ...
                            accepted_characters += 1
                            cursor.execute(
                                f"Select Character_Name, Level, Effective_Wealth, Player_Name, Player_ID, Tier FROM Sessions_Signups WHERE Session_ID = '{session_id}' LIMIT 1 OFFSET {random_number}")
                            player_info = cursor.fetchone()
                            print(x, player_info)
                            if accepted_characters <= 20:
                                await EventCommand.accept_player(guild_id, session_info[0], session_id,
                                                                 ...
                                                                 player_info[3], player_info[4], author, player_info[5])
                                embed.add_field(name=f'{player_info[0]}',
                                                value=f"has been accepted with player: @{player_info[3]}!")
                                mentions += f'<@{player_info[4]}> '
                            else:
                                break
                ...
                if specific_character is not None:
                    ...
                    if player_info is not None:
                        ...
                        embed.set_author(name=f'{author}')
                        accepted_characters += 1
                        await EventCommand.accept_player(guild_id, session_info[0], session_id, player_info[0],
                                                         player_info[1], player_info[2], player_info[3], player_info[4],
                                                         author, player_info[5])
                        embed.add_field(name=f'{player_info[0]}',
                                        value=f"has been accepted with player: @{player_info[4]}!")
                        mentions += f'<@{player_info[4]}> '
                        embed.set_footer(text=f"Session ID: {session_id}")
                        ...
                        embed = discord.Embed(title=f"{session_info[0]} signups failed!",
                                              description=f"This character was not found.",
                                              color=discord.Colour.green())
                if session_info[3] is not None:
                    embed = discord.Embed(title=f"{session_info[0]} signups failed!", url=f'{session_info[3]}',
            ...
            if session_info[3] is not None:
                embed = discord.Embed(title=f"{session_info[0]} signups failed!", url=f'{session_info[3]}',
                                      description=f"20 IS THE HARD CAP. GO MAKE A SEPARATE SESSION FOR HANDLING REWARDS.",
                                      color=discord.Colour.green())
            else:
                embed = discord.Embed(title=f"{session_info[0]} signups failed!",
                                      description=f"20 IS THE HARD CAP. GO MAKE A SEPARATE SESSION FOR HANDLING REWARDS.",
...
async def remove(interaction: discord.Interaction, session_id: int, player: discord.Member):
    ...
    guild = interaction.guild
    cursor.execute(
        f"SELECT Session_Name, Play_location, Play_Time, game_link, IsActive, Gold, Flux, Alt_Reward_All FROM Sessions WHERE Session_ID = '{session_id}'")
    session_info = cursor.fetchone()
    if session_info is None:
        await interaction.response.send_message(f"Invalid Session ID of {session_id}")
    else:
        if session_info[4] == 1:
            cursor.execute(
                f"SELECT Player_Name, Character_Name FROM Sessions_Participants WHERE Session_ID = '{session_id}' and Player_Name = '{player.name}'")
            player_info = cursor.fetchone()
            if player_info is None:
                await interaction.response.send_message(
                ...
                await EventCommand.remove_player(guild_id, session_id, player_name, character_name, author)
                await interaction.response.send_message(
                    f"{player.name} has been removed from Session {session_info[0]} with ID: {session_id}")
        else:
            cursor.execute(
                f"SELECT Player_Name, Character_Name, Level, Effective_Gold, Received_Milestones, Received_Trials, Received_Gold, Alt_Reward_Personal, Received_Fame FROM Sessions_Archive WHERE Session_ID = '{session_id}' and Player_Name = '{player.name}'")
            ...
            if reward_info is None:
                ...
                buttons = ["✅", "❌"]  # checkmark X symbol
                embed = discord.Embed(title=f"are you sure you want to revoke rewards from {reward_info[1]}?",
                                      description=f"hit the checkmark to confirm", colour=discord.Colour.blurple())
                await interaction.response.send_message(embed=embed)
                ...
                while True:
                    try:
                        ...
                        if reaction.emoji == u"\u264C":
                            embed = discord.Embed(
                                title=f"You have thought better of revoking rewards from {reward_info[1]}",
                                description=f"Savings!", colour=discord.Colour.blurple())
                            await msg.edit(embed=embed)
                            await msg.clear_reactions()
                        if reaction.emoji == u"\u2705":
                            embed = discord.Embed(title=f"Reward Change has occurred!",
                                                  description=f"Rewards Revoked from {reward_info[1]}.",
                                                  colour=discord.Colour.red())
                            await msg.clear_reactions()
                            await msg.edit(embed=embed)
                            ...
                            level_information = level_calculation(guild_id, player_info[9], -abs(reward_info[4]),
                                                                  player_info[29])
                            mythic_information = mythic_calculation(guild_id, player_info[7], player_info[11],
                                                                    -abs(reward_info[5]))
                            await EventCommand.session_rewards(author, guild_id, player_info[2],
                                                               level_information[0], milestone_total,
                                                               level_information[1], player_info[16] - session_info[6],
                                                               mythic_information[0], player_info[11] - reward_info[5],
                                                               mythic_information[1], player_info[27] - reward_info[8],
                                                               player[30] - reward_info[8], session_id)
                            await EventCommand.gold_change(guild_id, player_info[0], player_info[1],
                                                           player_info[2], reward_info[6] * -1, reward_info[6] * -1,
                                                           session_info[5] * -1, 'Session removing session_reward',
                                                           'removing session_reward')
                            bio_embed = character_embed(player_info[0], player_info[1], player_info[2], player_info[4],
                                                        player_info[5], player_info[6], level_information[0],
                                                        mythic_information[0], player_info[9] + -abs(reward_info[4]),
                                                        level_information[1], player_info[11] + -abs(reward_info[5]),
                                                        mythic_information[1], player_info[13] - reward_info[6],
                                                        player_info[14] - reward_info[6],
                                                        player_info[16] - session_info[6], player_info[17],
                                                        player_info[18], player_info[19], player_info[20],
                                                        player_info[21], player_info[22], player_info[23],
                                                        player_info[27], player_info[28], player_info[30],
                                                        player_info[31])
                            ...
                            reward_all = None if session_info[
                                                     7] is None else f"Removed the following: {session_info[7]}"
                            logging_embed = log_embed(player_info[2], author, level_information[0],
                                                      -abs(reward_info[4]), player_info[9] + -abs(reward_info[4]),
                                                      level_information[1], mythic_information[0], reward_info[5],
                                                      player_info[11] + -abs(reward_info[5]), mythic_information[1],
                                                      player_info[13] - reward_info[6], reward_info[5],
                                                      player_info[14] - reward_info[6], transaction_id[0],
                                                      player_info[16] - session_info[6], -abs(session_info[6]), None,
                                                      None, None, None, reward_all, None, None, None, None, source)
                            logging_thread = guild.get_thread(player_info[25])
                            await logging_thread.send(embed=logging_embed)
                            cursor.close()
                            db.close()
                            await EventCommand.remove_player(guild_id, session_id, reward_info[0], reward_info[1],
                                                             author)
    cursor.close()
    db.close()
...
@app_commands.describe(
    overview="This accepts either a google docs link or an overview. Use Double Spacing with the Docs link.")
async def reward(interaction: discord.Interaction, session_id: int, gold: float, easy: int = 0, medium: int = 0,
                 hard: int = 0, deadly: int = 0, trials: int = 0, reward_all: str = None, fame: int = 2,
                 prestige: int = 2, party_reward: str = None, overview: str = None):
    ...
    if gold < 0 or easy < 0 or medium < 0 or hard < 0 or deadly < 0 or awarded_flux < 0 or trials < 0:
        ...
        await interaction.response.send_message(
            f"Your players have been rewarded wi-- wait a minute, what the fuck? No Rewards?! No! At least give them a silver or a milestone!")
    guild_id = interaction.guild_id
    guild = interaction.guild
    author = interaction.user.name
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    cursor.execute(
        f"""SELECT GM_Name, Session_Name, Session_Range, Play_Location, Play_Time, Message, Session_Thread, IsActive, Related_Plot FROM Sessions WHERE Session_ID = {session_id} LIMIT 1""")
    session_info = cursor.fetchone()
    if session_info is not None:
        if session_info[7] == 1:
            mentions = f"Session Rewards for {session_info[1]}: "
            cursor.execute(
                f"""SELECT Player_Name, Player_ID, Character_Name, Level, Tier, Effective_Wealth  FROM Sessions_Participants WHERE Session_ID = {session_id}""")
            session_players = cursor.fetchall()

            if session_players == []:
                await interaction.response.send_message(
                    f"No players could be found participating in session with {session_id} can be found!")
                ...
                if overview is not None:
                    ...
                    await EventCommand.report(guild_id, 2, plot, overview, session_id, author, significance)
                    cursor.execute(f"""SELECT Article_Link from Sessions where Session_ID = ?""", (session_id,))
                    session_link = cursor.fetchone()
                    embed = discord.Embed(title=f"{session_info[1]}", description=f"Session Report",
                                          url=session_link[0], color=discord.Colour.green())
                else:
                    embed = discord.Embed(title=f"{session_info[1]}", description=f"Session Report",
                                          color=discord.Colour.green())
                ...
                for player in session_players:
                    ...
                    #                Setting Job Rewards
                    cursor.execute(
                        f"""SELECT Easy, Medium, Hard, Deadly from AA_Milestones WHERE level = {player[3]}""")
                    job_info = cursor.fetchone()
                    ...
                    cursor.execute(
                        f"SELECT Player_Name, Player_ID, True_Character_Name, Character_Name, Titles, Description, Oath, Level, Tier, Milestones, Milestones_Required, Trials, Trials_Required, Gold, Gold_Value, Gold_value_Max, Flux, Color, Mythweavers, Image_Link, Tradition_Name, Tradition_Link, Template_Name, Template_Link, Message_ID, Logging_ID, Thread_ID, Fame, Title, Personal_Cap, Prestige, Article_Link, Accepted_Date FROM Player_Characters where Character_Name = ? OR Nickname = ?",
                        (character_name, character_name))
                    player_info = cursor.fetchone()
                    # The specific date you want to compare
                    end_date = datetime.strptime(player_info[32], '%Y-%m-%d %H:%M')
                    ...
                    gold_info = gold_calculation(level_info[0], player_info[6], player_info[13], player_info[14],
                                                 player_info[15], gold)
                    ...
                    if player[3] != level_info[0]:
                        ...
                        if level_range_characters is None:
                            cursor.execute(
                                f"SELECT Level, Role_name, Role_ID FROM Level_Range WHERE level = {level_info[0]}")
                            new_level_range = cursor.fetchone()
                            role1 = guild.get_role(level_range[2])
                            role2 = guild.get_role(new_level_range[2])
                            await member.remove_roles(role1)
                            ...
                            role2 = guild.get_role(new_level_range[2])
                            await member.add_roles(role2)
                    prestige = prestige if player_info[30] + prestige <= player_info[27] + fame else player_info[
                                                                                                         27] + fame - \
                                                                                                     player_info[30]
                    if trials != 0:
                    ...
                    await EventCommand.session_rewards(author, guild_id, player[2], level_info[0],
                                                       player_info[9] + rewarded, level_info[1], flux_total,
                                                       mythic_info[0], player_info[11] + trials, mythic_info[1],
                                                       player_info[27] + fame, player_info[30] + prestige,
                                                       f"Session {session_id} reward")
                    await EventCommand.gold_change(guild_id, player[0], player[1], player[2], gold_info[3],
                                                   gold_info[3], gold, 'Session Reward', 'Session Reward')
                    await EventCommand.session_log_player(guild_id, session_id, player_info[0], player_info[1],
                                                          player_info[2], player[3], player[4], player[5], rewarded,
                                                          trials, gold_info[3], fame, prestige, awarded_flux)
                    cursor.execute(f'SELECT MAX(Transaction_ID) FROM A_Audit_Gold Order By Transaction_ID DESC LIMIT 1')
                    transaction_id = cursor.fetchone()
                    cursor.execute(f"Select Search FROM Admin WHERE Identifier = 'Accepted_Bio_Channel'")
                    accepted_bio_channel = cursor.fetchone()
                    ...
                    bio_embed = character_embed(player_info[0], player_info[1], player_info[2], player_info[4],
                                                player_info[5], player_info[6], level_info[0], mythic_info[0],
                                                player_info[9] + rewarded, level_info[1], player_info[11] + trials,
                                                mythic_info[1], player_info[13] + gold_info[3],
                                                player_info[14] + gold_info[3], player_info[16], player_info[17],
                                                player_info[18], player_info[19], player_info[20], player_info[21],
                                                player_info[22], player_info[23], player_info[27] + fame,
                                                player_info[28], player_info[30] + prestige, player_info[31])
                    bio_channel = await bot.fetch_channel(accepted_bio_channel[0])
                    bio_message = await bio_channel.fetch_message(player_info[24])
                    await bio_message.edit(content=bio_embed[1], embed=bio_embed[0])
                    source = f"Session Reward with Session ID: {session_id} and transaction ID: {transaction_id[0]}"
                    logging_embed = log_embed(player_info[2], author, level_info[0], rewarded,
                                              player_info[9] + rewarded, level_info[1], mythic_info[0], trials,
                                              player_info[11] + trials, mythic_info[1], player_info[13] + gold_info[3],
                                              gold_info[3], player_info[14] + gold_info[3], transaction_id[0],
                                              flux_total, awarded_flux, None, None, None, None, reward_all,
                ...
                rewards_channel = cursor.fetchone()
                reward_channel = await bot.fetch_channel(rewards_channel[0])
                reward_msg = await reward_channel.send(content=mentions, embed=embed,
                                                       allowed_mentions=discord.AllowedMentions(users=True))
                if party_reward is not None:
                    reward_thread = await reward_msg.create_thread(
                        name=f"Session name: {session_info[1]} Party Rewards, Session ID: {session_id}",
                        auto_archive_duration=60, reason=f"{party_reward}")
                    reward_thread_id = reward_thread.id
                    party_reward_embed = discord.Embed(title=f"{session_info[1]}", description=f"Party Reward Display",
                                                       color=discord.Colour.green())
                ...
                await EventCommand.session_log(guild_id, session_id, gold, 10, easy, medium, hard, deadly, trials,
                                               reward_all, party_reward, reward_msg.id, reward_thread_id, fame,
                                               prestige)
            ...
            await interaction.response.send_message(
                f"Session found with {session_id} but it is archived! Please submit a request to your admin to address!")
    if session_info is None:
        await interaction.response.send_message(f"No active session with {session_id} can be found!")


@gamemaster.command()
async def endow(interaction: discord.Interaction, session_id: int, player_1: typing.Optional[discord.Member],
                player_1_reward: typing.Optional[str], player_2: typing.Optional[discord.Member],
                player_2_reward: typing.Optional[str], player_3: typing.Optional[discord.Member],
                player_3_reward: typing.Optional[str], player_4: typing.Optional[discord.Member],
                player_4_reward: typing.Optional[str], player_5: typing.Optional[discord.Member],
                player_5_reward: typing.Optional[str], player_6: typing.Optional[discord.Member],
                player_6_reward: typing.Optional[str]):
    """GM: Reward Players for Participating in your session."""
    guild_id = interaction.guild_id
    guild = interaction.guild
    author = interaction.user.name
    ...
    cursor = db.cursor()
    cursor.execute(
        f"""SELECT GM_Name, Session_Name, Session_Range, Play_Location, Play_Time, Message FROM Sessions WHERE Session_ID = {session_id} LIMIT 1""")
    session_info = cursor.fetchone()
    await interaction.response.defer(thinking=True)
    if session_info is not None:
        embed = discord.Embed(title=f"{session_info[1]}", description=f"Personal Reward Display",
                              color=discord.Colour.green())
        embed.set_footer(text=f"Session ID is {session_id}")
        if player_1 is not None and player_1_reward is not None:
            cursor.execute(
                f"""SELECT Character_Name FROM Sessions_Archive WHERE Player_Name = '{player_1.name}' AND Session_ID = {session_id}""")
            session_player_info = cursor.fetchone()
            response = f"<@{player_1.id}> "
            if session_player_info is not None:
                cursor.execute(f"""SELECT Character_Name, Thread_ID FROM Player_Characters WHERE Character_Name = ?""",
                               (session_player_info[0],))
                character_info = cursor.fetchone()
                await EventCommand.session_endowment(author, guild_id, session_id, player_1.name, player_1_reward,
                                                     character_info[0])
                source = f"Personal reward for Session ID: {session_id}"
                logging_embed = log_embed(session_player_info[0], author, None, None, None, None, None, None, None,
                                          None, None, None, None, None, None, None, None, None, None, None,
                                          player_1_reward, None, None, None, None, source)
        ...
        if player_2 is not None and player_2_reward is not None:
            cursor.execute(
                f"""SELECT Character_Name FROM Sessions_Archive WHERE Player_Name = '{player_2.name}' AND Session_ID = {session_id}""")
            session_player_info = cursor.fetchone()
            ...
            if session_player_info is not None:
                cursor.execute(f"""SELECT Character_Name, Thread_ID FROM Player_Characters WHERE Character_Name = ?""",
                               (session_player_info[0],))
                character_info = cursor.fetchone()
                await EventCommand.session_endowment(author, guild_id, session_id, player_2.name, player_2_reward,
                                                     character_info[0])
                source = f"Personal reward for Session ID: {session_id}"
                logging_embed = log_embed(session_player_info[0], author, None, None, None, None, None, None, None,
                                          None, None, None, None, None, None, None, None, None, None, None,
                                          player_2_reward, None, None, None, None, source)
                logging_thread = guild.get_thread(character_info[1])
                await logging_thread.send(embed=logging_embed)
        ...
        if player_3 is not None and player_3_reward is not None:
            cursor.execute(
                f"""SELECT Character_Name FROM Sessions_Archive WHERE Player_Name = '{player_3.name}' AND Session_ID = {session_id}""")
            session_player_info = cursor.fetchone()
            ...
            if session_player_info is not None:
                cursor.execute(f"""SELECT Character_Name, Thread_ID FROM Player_Characters WHERE Character_Name = ?""",
                               (session_player_info[0],))
                character_info = cursor.fetchone()
                await EventCommand.session_endowment(author, guild_id, session_id, player_3.name, player_3_reward,
                                                     character_info[0])
                source = f"Personal reward for Session ID: {session_id}"
                logging_embed = log_embed(session_player_info[0], author, None, None, None, None, None, None, None,
                                          None, None, None, None, None, None, None, None, None, None, None,
                                          player_3_reward, None, None, None, None, source)
                logging_thread = guild.get_thread(character_info[1])
                await logging_thread.send(embed=logging_embed)
        ...
        if player_4 is not None and player_4_reward is not None:
            cursor.execute(
                f"""SELECT Character_Name FROM Sessions_Archive WHERE Player_Name = '{player_4.name}' AND Session_ID = {session_id}""")
            session_player_info = cursor.fetchone()
            ...
            if session_player_info is not None:
                cursor.execute(f"""SELECT Character_Name, Thread_ID FROM Player_Characters WHERE Character_Name = ?""",
                               (session_player_info[0],))
                character_info = cursor.fetchone()
                await EventCommand.session_endowment(author, guild_id, session_id, player_4.name, player_4_reward,
                                                     character_info[0])
                source = f"Personal reward for Session ID: {session_id}"
                logging_embed = log_embed(session_player_info[0], author, None, None, None, None, None, None, None,
                                          None, None, None, None, None, None, None, None, None, None, None,
                                          player_4_reward, None, None, None, None, source)
                logging_thread = guild.get_thread(character_info[1])
                await logging_thread.send(embed=logging_embed)
        ...
        if player_5 is not None and player_5_reward is not None:
            cursor.execute(
                f"""SELECT Character_Name FROM Sessions_Archive WHERE Player_Name = '{player_5.name}' AND Session_ID = {session_id}""")
            session_player_info = cursor.fetchone()
            ...
            if session_player_info is not None:
                cursor.execute(f"""SELECT Character_Name, Thread_ID FROM Player_Characters WHERE Character_Name = ?""",
                               (session_player_info[0],))
                character_info = cursor.fetchone()
                await EventCommand.session_endowment(author, guild_id, session_id, player_5.name, player_5_reward,
                                                     character_info[0])
                source = f"Personal reward for Session ID: {session_id}"
                logging_embed = log_embed(session_player_info[0], author, None, None, None, None, None, None, None,
                                          None, None, None, None, None, None, None, None, None, None, None,
                                          player_5_reward, None, None, None, None, source)
                logging_thread = guild.get_thread(character_info[1])
                await logging_thread.send(embed=logging_embed)
        ...
        if player_6 is not None and player_6_reward is not None:
            cursor.execute(
                f"""SELECT Character_Name, Thread_ID FROM Sessions_Archive WHERE Player_Name = '{player_6.name}' AND Session_ID = {session_id}""")
            session_player_info = cursor.fetchone()
            ...
            if session_player_info is not None:
                cursor.execute(f"""SELECT Character_Name, Thread_ID FROM Player_Characters WHERE Character_Name = ?""",
                               (session_player_info[0],))
                character_info = cursor.fetchone()
                await EventCommand.session_endowment(author, guild_id, session_id, player_6.name, player_6_reward,
                                                     character_info[0])
                source = f"Personal reward for Session ID: {session_id}"
                logging_embed = log_embed(session_player_info[0], author, None, None, None, None, None, None, None,
                                          None, None, None, None, None, None, None, None, None, None, None,
                                          player_6_reward, None, None, None, None, source)
                logging_thread = guild.get_thread(character_info[1])
                await logging_thread.send(embed=logging_embed)
...
@app_commands.choices(forego=[discord.app_commands.Choice(name='all!', value=1),
                              discord.app_commands.Choice(name='Forego Milestones!', value=2),
                              discord.app_commands.Choice(name='Foregold!', value=3)])
@app_commands.autocomplete(character_name=own_character_select_autocompletion)
async def claim(interaction: discord.Interaction, session_id: int, character_name: str,
                forego: discord.app_commands.Choice[int] = 1):
    guild_id = interaction.guild_id
    guild = interaction.guild
    author = interaction.user.name
    author_id = interaction.user.id
    ...
    cursor.execute(
        f"SELECT IsActive, GM_Name, Session_Name, Gold, Flux, Easy, Medium, Hard, Deadly, Trials, Fame, Prestige FROM Sessions WHERE Session_ID = '{session_id}' limit 1")
    session_info = cursor.fetchone()
    if session_info is None:
        ...
        if validate_recipient is not None:
            ...
            if previous_rewards is not None:
                ...
                if previous_rewards[1] == character_name:
                    ...
                    buttons = ["✅", "❌"]  # checkmark X symbol
                    embed = discord.Embed(
                        title=f"are you sure you want to revoke rewards from {previous_recipient[2]} and claim them for {validate_recipient[2]}?",
                        description=f"hit the checkmark to confirm", colour=discord.Colour.blurple())
                    await interaction.response.send_message(embed=embed)
                    msg = await interaction.original_response()
                    for button in buttons:
                    ...
                    while True:
                        try:
                            ...
                            if reaction.emoji == u"\u264C":
                                embed = discord.Embed(title=f"You have thought better of swapping the rewards",
                                                      description=f"Savings!", colour=discord.Colour.blurple())
                                await msg.edit(embed=embed)
                                await msg.clear_reactions()
                            if reaction.emoji == u"\u2705":
                                embed = discord.Embed(title=f"Reward Change has occurred!",
                                                      description=f"Rewards Revoked from {previous_recipient[2]} and claimed for {validate_recipient[2]}.",
                                                      colour=discord.Colour.red())
                                await msg.clear_reactions()
                                await msg.edit(embed=embed)
                                ...
                                level_information = level_calculation(guild_id, player_info[9],
                                                                      -abs(previous_rewards[4]), player_info[29])
                                mythic_information = mythic_calculation(guild_id, player_info[7], player_info[11],
                                                                        -abs(previous_rewards[5]))
                                await EventCommand.session_rewards(author, guild_id, player_info[2],
                                                                   level_information[0], milestone_total,
                                                                   level_information[1],
                                                                   player_info[16] - session_info[6],
                                                                   mythic_information[0],
                                                                   player_info[11] - previous_rewards[5],
                                                                   mythic_information[1],
                                                                   player_info[28] - previous_rewards[8], session_id)
                                await EventCommand.gold_change(guild_id, player_info[0], player_info[1],
                                                               player_info[2], previous_rewards[6] * -1,
                                                               previous_rewards[6] * -1, session_info[5] * -1,
                                                               'Session removing session_reward',
                                                               'removing session_reward')
                                bio_embed = character_embed(player_info[0], player_info[1], player_info[2],
                                                            player_info[4], player_info[5], player_info[6],
                                                            level_information[0], mythic_information[0],
                                                            player_info[9] + -abs(previous_rewards[4]),
                                                            level_information[1],
                                                            player_info[11] + -abs(previous_rewards[5]),
                                                            mythic_information[1],
                                                            player_info[13] - previous_rewards[6],
                                                            player_info[14] - previous_rewards[6],
                                                            player_info[16] - session_info[6], player_info[17],
                                                            player_info[18], player_info[19], player_info[20],
                                                            player_info[21], player_info[22], player_info[23],
                                                            player_info[27], player_info[28], player_info[30],
                                ...
                                reward_all = None if session_info[
                                                         7] is None else f"Removed the following: {session_info[7]}"
                                logging_embed = log_embed(player_info[2], author, level_information[0],
                                                          -abs(previous_rewards[4]),
                                                          player_info[9] + -abs(previous_rewards[4]),
                                                          level_information[1], mythic_information[0],
                                                          previous_rewards[5],
                                                          player_info[11] + -abs(previous_rewards[5]),
                                                          mythic_information[1], player_info[13] - previous_rewards[6],
                                                          previous_rewards[5], player_info[14] - previous_rewards[6],
                                                          transaction_id[0], player_info[16] - session_info[6],
                                                          -abs(session_info[6]), None, None, None, None, reward_all,
                                                          None, None, None, None, source)
                                logging_thread = guild.get_thread(player_info[25])
                                await logging_thread.send(embed=logging_embed)
                                cursor.close()
                                db.close()
                                await EventCommand.remove_player(guild_id, session_id, previous_rewards[0],
                                                                 previous_rewards[1], author)
                                db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
                                cursor = db.cursor()
                                cursor.execute(
                                ...
                                mythic_information = mythic_calculation(guild_id, level_information[0],
                                                                        validate_recipient[10], session_info[9])
                                print(mythic_information)
                                gold_information = gold_calculation(level_information[0], validate_recipient[5],
                                                                    validate_recipient[12], validate_recipient[13],
                                                                    validate_recipient[14], session_info[3])
                                gold_received = 0 if forego == 3 else gold_information[3]
                                gold_rewarded = 0 if forego == 3 else session_info[3]
                                await EventCommand.session_rewards(validate_recipient[0], guild_id,
                                                                   character_name, level_information[0], new_milestones,
                                                                   level_information[1],
                                                                   validate_recipient[15] + session_info[4],
                                                                   mythic_information[0],
                                                                   validate_recipient[10] + session_info[9],
                                                                   mythic_information[1],
                                                                   validate_recipient[26] + session_info[10],
                                                                   session_id)
                                await EventCommand.gold_change(guild_id, validate_recipient[0],
                                                               validate_recipient[1], character_name, gold_received,
                                                               gold_received, gold_rewarded, 'Session Added new Claim',
                                                               'Session Claim')
                                ...
                                cursor.close()
                                db.close()
                                await EventCommand.session_log_player(guild_id, session_id, validate_recipient[0],
                                                                      validate_recipient[1], validate_recipient[2],
                                                                      validate_recipient[6], validate_recipient[7],
                                                                      validate_recipient[13], rewarded, session_info[9],
                                                                      gold_received, session_info[10])
                                bio_embed = character_embed(validate_recipient[0], validate_recipient[1],
                                                            validate_recipient[2], validate_recipient[3],
                                                            validate_recipient[4], validate_recipient[5],
                                                            level_information[0], mythic_information[0], new_milestones,
                                                            level_information[1],
                                                            validate_recipient[10] + session_info[9],
                                                            mythic_information[1],
                                                            validate_recipient[12] + gold_received,
                                                            validate_recipient[13] + gold_received,
                                                            validate_recipient[15] + session_info[4],
                                                            validate_recipient[16], validate_recipient[17],
                                                            validate_recipient[18], validate_recipient[19],
                                                            validate_recipient[20], validate_recipient[21],
                                                            validate_recipient[22],
                                                            validate_recipient[26] + session_info[10],
                                                            validate_recipient[27], )
                                bio_channel = await bot.fetch_channel(accepted_bio_channel[0])
                                bio_message = await bio_channel.fetch_message(validate_recipient[23])
                                await bio_message.edit(content=bio_embed[1], embed=bio_embed[0])
                                session_name = session_info[2]
                                embed_log = log_embed(validate_recipient[2], author, level_information[0], rewarded,
                                                      new_milestones, level_information[1], mythic_information[0],
                                                      session_info[9], validate_recipient[10] + session_info[9],
                                                      mythic_information[1], validate_recipient[12] + gold_received,
                                                      gold_received, validate_recipient[13] + gold_received,
                                                      gold_transaction_id[0], validate_recipient[15] + session_info[4],
                                                      session_info[4], None, None, None, None, None,
                                                      validate_recipient[26] + session_info[10], session_info[10], None,
                                                      None, f"Session {session_name} with ID: {session_id} claimed")
                                logging_thread = guild.get_thread(validate_recipient[25])
                                await logging_thread.send(embed=embed_log)
                                await interaction.response.send_message(
                                    f"Rewards have been claimed for {character_name}!")
            else:
                cursor.execute(
                    f"""SELECT Easy, Medium, Hard, Deadly from AA_Milestones WHERE level = {validate_recipient[6]}""")
                job_info = cursor.fetchone()
                ...
                mythic_information = mythic_calculation(guild_id, level_information[0], validate_recipient[10],
                                                        session_info[9])
                print(mythic_information)
                gold_information = gold_calculation(level_information[0], validate_recipient[5], validate_recipient[12],
                                                    validate_recipient[13], validate_recipient[14], session_info[3])
                gold_received = 0 if forego == 3 else gold_information[3]
                gold_rewarded = 0 if forego == 3 else session_info[3]
                prestige = session_info[11]
                prestige = prestige if validate_recipient[29] + prestige <= validate_recipient[26] + session_info[
                    10] else validate_recipient[26] + session_info[10] - validate_recipient[29]
                # The specific date you want to compare
                end_date = datetime.strptime(validate_recipient[31], '%Y-%m-%d %H:%M')
                # Current date and time
                current_date = datetime.now()
                # Calculate the difference between dates
                difference = current_date - end_date
                ...
                await EventCommand.session_rewards(validate_recipient[0], guild_id, character_name,
                                                   level_information[0], new_milestones, level_information[1],
                                                   validate_recipient[15] + awarded_flux, mythic_information[0],
                                                   validate_recipient[10] + session_info[9], mythic_information[1],
                                                   validate_recipient[26] + session_info[10],
                                                   validate_recipient[29] + prestige, session_id)
                await EventCommand.gold_change(guild_id, validate_recipient[0], validate_recipient[1],
                                               character_name, gold_received, gold_received, gold_rewarded,
                                               'Session Added new Claim', 'Session Claim')
                cursor.execute(f"Select MAX(transaction_id) from A_Audit_Gold")
                gold_transaction_id = cursor.fetchone()
                cursor.execute(f"Select Search FROM Admin WHERE Identifier = 'Accepted_Bio_Channel'")
                accepted_bio_channel = cursor.fetchone()
                ...
                await EventCommand.session_log_player(guild_id, session_id, validate_recipient[0],
                                                      validate_recipient[1], validate_recipient[2],
                                                      validate_recipient[6], validate_recipient[7],
                                                      validate_recipient[13], rewarded, session_info[9], gold_received,
                                                      session_info[10], prestige, awarded_flux)
                bio_embed = character_embed(validate_recipient[0], validate_recipient[1], validate_recipient[2],
                                            validate_recipient[3], validate_recipient[4], validate_recipient[5],
                                            level_information[0], mythic_information[0], new_milestones,
                                            level_information[1], validate_recipient[10] + session_info[9],
                                            mythic_information[1], validate_recipient[12] + gold_received,
                                            validate_recipient[13] + gold_received,
                                            validate_recipient[15] + awarded_flux, validate_recipient[16],
                                            validate_recipient[17], validate_recipient[18], validate_recipient[19],
                                            validate_recipient[20], validate_recipient[21], validate_recipient[22],
                                            validate_recipient[26] + session_info[10], validate_recipient[27],
                                            validate_recipient[29] + prestige, validate_recipient[30])
                ...
                await bio_message.edit(content=bio_embed[1], embed=bio_embed[0])
                session_name = session_info[2]
                embed_log = log_embed(validate_recipient[2], author, level_information[0], rewarded, new_milestones,
                                      level_information[1], mythic_information[0], session_info[9],
                                      validate_recipient[10] + session_info[9], mythic_information[1],
                                      validate_recipient[12] + gold_received, gold_received,
                                      validate_recipient[13] + gold_received, gold_transaction_id[0],
                                      validate_recipient[15] + awarded_flux, awarded_flux, None, None, None, None, None,
                                      validate_recipient[26] + session_info[10], session_info[10],
                                      validate_recipient[29] + prestige, prestige,
                                      f"Session {session_name} with ID: {session_id} claimed")
                logging_thread = guild.get_thread(validate_recipient[25])
                await logging_thread.send(embed=embed_log)
...
async def plot(interaction: discord.Interaction, plot: str, summary: str):
    ...
    if summary is None:
        ...
        await EventCommand.plot(guild_id, type, plot_id, summary, author)
        ...
        await EventCommand.plot(guild_id, type, plot_name, summary, author)
...
async def report(interaction: discord.Interaction, session_id: int, summary: str, plot: str = None):
    ...
    cursor = db.cursor()
    cursor.execute(
        f"Select Session_ID, Session_Name, Article_Link, Article_ID, Related_Plot, easy, medium, hard, deadly, trials from Sessions where Session_ID = ? and GM_Name = ? and IsActive = 0",
        (session_id, author))
    session_info = cursor.fetchone()
    await interaction.response.defer(thinking=True)
    ...
    if session_info is None:
        ...
        if session_info[2] is None:
            ...
            await EventCommand.report(guild_id, event_type, plot, summary, session_id, author, significance)
            cursor.execute(
                f"Select Session_ID, Session_Name, Article_Link, Article_ID, Related_Plot from Sessions where Session_ID = ? and IsActive = 0",
                (session_id,))
            session_info = cursor.fetchone()
            wa_update_channel = await bot.fetch_channel(1032422764168106134)
            await wa_update_channel.send(
                f"Session Report for [{session_info[1]}](<{session_info[2]}>) has been created.")
            await interaction.followup.send(
                f"Session Report for [{session_info[1]}](<{session_info[2]}>) has been created.")
        else:
            event_type = 1
            ...
            print(significance)
            await interaction.followup.send(
                f"Session Report for [{session_info[1]}](<{session_info[2]}>) has been edited.")
            wa_update_channel = await bot.fetch_channel(1032422764168106134)
            await wa_update_channel.send(
                f"Session Report for [{session_info[1]}](<{session_info[2]}>) has been edited.")
            await wa_update_channel.send(
...
async def report(interaction: discord.Interaction, session_id: int, summary: str, character_name: str):
    ...
    if session_info is None:
        ...
        await EventCommand.session_report(guild_id, summary, session_id, character_name, author)
        cursor.execute(
            f"Select Session_ID, Session_Name, Article_Link, Article_ID, Related_Plot from Sessions where Session_ID = ? and IsActive = 0",
            (session_id,))
        session_info = cursor.fetchone()
        await interaction.response.send_message(
            f"Session Report for [{session_info[1]}](<{session_info[2]}>) has been added to!")
    cursor.close()
    db.close()
...
async def questify(ctx: commands.Context, group_id: int):
    ...
    if group_base is not None:
        await EventCommand.clear_group(guild_id, group_id)
        embed = discord.Embed(title=f"Group Request {group_id}", description=f'Group {group_id} has been deleted!',
                              colour=discord.Colour.green())
        await ctx.response.send_message(embed=embed)
    else:
...
async def join(interaction: discord.Interaction, session_id: int, character_name: str):
    ...
    await interaction.response.defer(thinking=True, ephemeral=True)
    cursor.execute(
        f"SELECT Session_Name, Play_location, Play_Time, game_link, Session_Range_ID, Session_Range, Session_Thread, overflow FROM Sessions WHERE Session_ID = '{session_id}' AND IsActive = 1")
    session_info = cursor.fetchone()
    if session_info is None:
        ...
        if role in user.roles or session_info[7] == 4 or session_info[7] == 3 or session_info[7] == 2:
            ...
            if character_info is not None:
                ...
                if level_range_info is None or session_info[7] == 4:
                    cursor.execute(
                        f"""Select Character_Name from Sessions_Participants where Player_name = '{author}' and Session_ID = {session_id}""")
                    participation = cursor.fetchone()
                    cursor.execute(
                        f"""Select Character_Name from Sessions_Signups where Player_name = '{author}' AND Session_ID = {session_id}""")
                    signups = cursor.fetchone()
                    if participation is None and signups is None:
                        await EventCommand.session_join(guild_id, session_info[0], session_id, author, author_id,
                        ...
                        if result is None:
                            await interaction.followup.send(
                                f"{character_name} is not a valid Character name or Nickname.")
                            cursor.close()
                            db.close()
                            return
                        else:
                            ...
                            embed.set_thumbnail(url=f'{result[13]}')
                            embed.add_field(name="Information",
                                            value=f'**Level**: {result[4]}, **Mythic Tier**: {result[5]}', inline=True)
                            embed.add_field(name="illiquid Wealth",
                                            value=f'**GP**: {round(result[19] - result[10], 2)}', inline=True)
                            linkage = f""
                            if result[15] is not None:
                                linkage += f"**Tradition**: [{result[15]}]({result[16]})"
                            if result[17] is not None:
                            ...
                            if result[20] == 'Offerings':
                                ...
                                embed.set_footer(text=f'{result[3]}')
                            await quest_thread.send(embed=embed, content=f"{interaction.user.mention}",
                                                    allowed_mentions=discord.AllowedMentions(users=True))
                            await interaction.followup.send(
                        ...
                        await interaction.followup.send(
                            f"{author} is already participating in {session_info[0]} with session ID: {session_id}")
                    ...
                    if session_info[7] == 3:
                        cursor.execute(
                            f"SELECT min(level), max(level) Role_Name from Level_Range WHERE Role_ID = {session_info[4]}")
                        ...
                        cursor.execute(f"SELECT min(level), max(level) Role_Name from Level_Range WHERE Role_Name = ?",
                                       (overflow_level_role[0],))
                        overflow_level_range_info = cursor.fetchone()
                        overflow_level_range_info = overflow_level_range_info if overflow_level_range_info[
                                                                                     0] is not None else new_level_range_info
                        level_range_validation = 1 if overflow_level_range_info is not None and \
                                                      overflow_level_range_info[0] <= character_info[1] <= \
                                                      new_level_range_info[1] else 0
                        ...
                        overflow_level_range_info = overflow_level_range_info if overflow_level_range_info[
                                                                                     0] is not None else new_level_range_info
                        level_range_validation = 1 if overflow_level_range_info is not None and new_level_range_info[
                            0] <= character_info[1] <= overflow_level_range_info[1] else 0
                    else:
                        cursor.execute(
                            f"SELECT min(level), max(level) Role_Name from Level_Range WHERE Role_ID = {session_info[4]} AND Level = {character_info[1]}")
                        new_level_range_info = cursor.fetchone()
                        level_range_validation = 1 if new_level_range_info is not None else 0
                    if level_range_validation != 1:
                        await interaction.followup.send(
                            f"{character_info[0]} is level {character_info[1]} which is not inside the level range of {level_range_info[1]}!",
                            ephemeral=True)
                    else:
                        cursor.execute(
                            f"""Select Character_Name from Sessions_Participants where Player_name = '{author}' and Session_ID = {session_id}""")
                        ...
                        signups = cursor.fetchone()
                        if participation is None and signups is None:
                            await EventCommand.session_join(guild_id, session_info[0], session_id, author,
                                                            author_id, character_info[0], character_info[1],
                                                            character_info[2], character_info[3])
            ...
            await interaction.followup.send(
                f"User does not have role: {session_info[5]}! If you wish to join, obtain this role! Ensure you have a character in the correct level bracket.",
                ephemeral=True)
...
async def leave(interaction: discord.Interaction, session_id: int):
    ...
    cursor = db.cursor()
    cursor.execute(
        f"SELECT Session_Name, Play_location, Play_Time, Game_Link FROM Sessions WHERE Session_ID = '{session_id}'")
    session_info = cursor.fetchone()
    if session_info is None:
        await interaction.response.send_message(f"No active session with {session_id} can be found!")
    if session_info is not None:
        cursor.execute(
            f"""Select Character_Name, Level, Effective_Wealth from Sessions_Signups where Player_Name = '{author}' and Session_ID = '{session_id}'""")
        character_info = cursor.fetchone()
        if character_info is None:
            cursor.execute(
                f"""Select Character_Name, Level, Effective_Wealth from Sessions_Participants where Player_Name = '{author}' and Session_ID = '{session_id}'""")
            character_info = cursor.fetchone()
            if character_info is None:
            ...
            if character_info is not None:
                ...
                cursor.close()
                db.close()
                await EventCommand.session_leave(guild_id, session_id, author, true_name)
                await interaction.response.send_message(
                    f"{author}'s {true_name} has decided against participating in the session of '{session_info[0]}!'")
        elif character_info is not None:
            true_name = character_info[0]
            cursor.close()
            db.close()
...
@app_commands.choices(
    group=[discord.app_commands.Choice(name='All', value=1), discord.app_commands.Choice(name='Participants', value=2),
...
async def display(ctx: commands.Context, session_id: int, group: discord.app_commands.Choice[int] = 1):
    ...
    if group == 1:
        ...
        group = group.value
    cursor.execute(
        f"SELECT GM_Name, Session_Name, Session_Range, Play_location, Play_Time, Overview, Description, Message, IsActive FROM Sessions WHERE Session_ID = {session_id}")
    session_info = cursor.fetchone()
    if session_info is not None:
        cursor.execute(f"Select Search FROM Admin WHERE Identifier = 'Sessions_Channel'")
        sessions_channel = cursor.fetchone()
        ...
        embed = discord.Embed(title=f"{session_info[1]}", description=f'[Session overview](<{msg.jump_url}>)!',
                              colour=discord.Colour.blurple())
        if session_info[8] == 1:
            embed.add_field(name=f"Active Session Info",
                            value=f"**GM**: {session_info[0]}, **Session Range**: {session_info[2]}, **Play_Location**: {session_info[3]}, **Play_Time**: <t:{session_info[4]}:D>",
                            inline=False)
            x = 0
            print(group)
            if group == 1 or group == 2:
                ...
                cursor.execute(
                    f"SELECT Player_Name, Character_Name, Level, Effective_Wealth, Tier, Player_ID FROM Sessions_Participants WHERE Session_ID = {session_id}")
                participants = cursor.fetchall()
                player_total = total_participants[0]
                embed.add_field(name=f"Participant List: {player_total} players", value=" ")
                for player in participants:
                    embed.add_field(name=f'**Character**: {player[1]}',
                                    value=f"**Player**: <@{player[5]}> \n **Level**: {player[2]}, **Tier** {player[4]} \n **Effective_Wealth**: {player[3]} GP",
                                    inline=False)
                    x += 1
                    if x >= 20:
                        embed.add_field(name=f"Field Limit reached",
                                        value=f'{total_participants[0] - 20} remaining Participants', inline=False)
                        break
            else:
                player_total = 0
            if group == 1 or group == 3:
                ...
                x = 0 + player_total
                cursor.execute(
                    f"SELECT Player_Name, Character_Name, Level, Effective_Wealth, Tier, Player_ID FROM Sessions_Signups WHERE Session_ID = {session_id}")
                participants = cursor.fetchall()
                player_total += total_participants[0]
                embed.add_field(name=f"Sign Up List: {total_participants[0]} players", value=' ', inline=False)
                for player in participants:
                    embed.add_field(name=f'**Character**: {player[1]}',
                                    value=f"Player: <@{player[5]}>, Level: {player[2]}, Tier: {player[4]}, Effective_Wealth: {player[3]}!",
                                    inline=False)
                    x += 1
                    if x >= 20:
                        embed.add_field(name=f"Field Limit reached",
                                        value=f'{total_participants[0] - 20} remaining Sign-ups')
                        break
                embed.set_footer(text=f"Session ID: {session_id}")
            await ctx.response.send_message(embed=embed)
        else:
            cursor.execute(
                f"SELECT Gold, Flux, Easy, Medium, Hard, Deadly, Trials FROM Sessions WHERE Session_ID = {session_id}")
            session_reward_info = cursor.fetchone()
            embed.add_field(name=f"Inactive Session Info",
                            value=f"**GM**: {session_info[0]}, **Session Range**: {session_info[2]}, **Play_Location**: {session_info[3]}, **Play_Time**: <t:{session_info[4]}:D>",
                            inline=False)
            embed.add_field(name=f"Milestone Rewards",
                            value=f"**Easy Jobs**: {session_reward_info[2]}, **Medium Jobs**: {session_reward_info[3]}, **Hard_jobs**: {session_reward_info[4]}, **Deadly_Jobs**: {session_reward_info[5]}, **Trials**: {session_reward_info[6]}",
                            inline=False)
            embed.add_field(name=f"Currency Rewards",
                            value=f"**Gold**: {session_reward_info[0]}, **Flux**: {session_reward_info[1]}",
                            inline=False)
            x = 0
            cursor.execute(f"SELECT COUNT(Player_Name) FROM Sessions_Archive WHERE Session_ID = {session_id}")
            total_participants = cursor.fetchone()
            cursor.execute(
                f"SELECT Player_Name, Character_Name, Level, Effective_Gold, Tier, Received_Milestones, Received_Gold, Player_ID FROM Sessions_Archive WHERE Session_ID = {session_id}")
            participants = cursor.fetchall()
            player_total = total_participants[0]
            embed.add_field(name=f"Participant List: {player_total} players", value=' ', inline=False)
            for player in participants:
                embed.add_field(name=f'**Character**: {player[1]}',
                                value=f"**Player**: <@{player[7]}> \n **Level**: {player[2]}, **Tier** {player[4]}, \n **Received Milestones**: {player[5]}, **Received Trials**: {session_info[12]} \n  **Session Effective Gold**: {player[3]}, **Received Gold**: {player[6]}",
                                inline=False)
                x += 1
                if x >= 20:
                    embed.add_field(name=f"Field Limit reached",
        ...
        embed = discord.Embed(title=f"Display Command Failed",
                              description=f'{session_id} could not be found in current or archived sessions!',
                              colour=discord.Colour.red())
        await ctx.response.send_message(embed=embed)
    cursor.close()
    db.close()
...
async def timesheet(ctx: commands.Context, day: discord.app_commands.Choice[int], timezone: str, start_time: str, end_time: str, change: discord.app_commands.Choice[int] = 1):
    #Update and Adjust your predicted Weekly Availability
    guild_id = ctx.guild.id
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    print(ctx.user.name)
    timezone_get = get_utc_offset(timezone)
    print(f"this is time get {timezone_get}")
    time_conversion = time_to_minutes(timezone_get)
    print(f"this is time conversion {time_conversion}")
    hour_difference = floor(time_conversion / 60)
    print(f"this is hour difference {hour_difference}")
    ...
    if change > 2:
        cursor.close()
        db.close()
        await EventCommand.clear_timesheet(guild_id, change, day_value, ctx.user.name)
        if change == 3:
            embed = discord.Embed(title=f"Time Sheet Update", description=f'{ctx.user.name} has cleared availability on {day_day}!', colour=discord.Colour.green())
            await ctx.followup.send(embed=embed)
        if change == 4:
    ...
    if start_hours > end_hours:
        ...
        return
    if change < 3:
        start_day = adjust_day(day_value, start_hours, hour_difference)
        end_day = adjust_day(day_value, end_hours, hour_difference)
        start_hours = int(start_hours) - hour_difference if int(start_hours) + hour_difference >= 0 else 24 + (int(start_hours) - hour_difference)
        print(f"I AM END HOURS {end_hours}")
        end_hours = int(end_hours) - hour_difference if int(end_hours) + hour_difference >= 0 else 24 + (int(end_hours) - hour_difference)
        print(f"I AM END HOURS {end_hours}")
        ...
        cursor.close()
        db.close()
        validation = await EventCommand.timesheet(guild_id, ctx.user.name, timezone, start_day, start_hours, start_minutes, end_day, end_hours, end_minutes, change)
        print(validation)
        if validation == 0:
            embed = discord.Embed(title=f"Time Sheet Error", description=f'This command failed!!', colour=discord.Colour.red())
            await ctx.followup.send(embed=embed)
        else:
            unix_start_time = convert_to_unix(start_time, timezone)
            unix_end_time = convert_to_unix(end_time, timezone)
            if change == 1:
                embed = discord.Embed(title=f"Time Sheet Update", description=f'{ctx.user.name} has added availability from {unix_start_time} to {unix_end_time} on {day_day}!', colour=discord.Colour.green())
                await ctx.followup.send(embed=embed)
            elif change == 2:
                embed = discord.Embed(title=f"Time Sheet Update", description=f'{ctx.user.name} has removed availability from {unix_start_time} to {unix_end_time} on {day_day}!', colour=discord.Colour.green())
                await ctx.followup.send(embed=embed)"""
...
@app_commands.describe(change="Add or remove availability")
@app_commands.describe(day="Which day of the week; if clearing all, ignore")
@app_commands.describe(start_time="The start time (e.g., '8:00 AM' or '20:00')")
@app_commands.describe(end_time="The end time (e.g., '12:30 PM' or '23:30')")
@app_commands.autocomplete(timezone=search_timezones)
@app_commands.choices(
    change=[
        discord.app_commands.Choice(name='Add', value=1),
        discord.app_commands.Choice(name='Remove', value=2),
        discord.app_commands.Choice(name='Clear', value=3),
        discord.app_commands.Choice(name='Clear All', value=4),
    ]
)
@app_commands.choices(
    day=[
        discord.app_commands.Choice(name='Monday', value=1),
        discord.app_commands.Choice(name='Tuesday', value=2),
        discord.app_commands.Choice(name='Wednesday', value=3),
        discord.app_commands.Choice(name='Thursday', value=4),
        discord.app_commands.Choice(name='Friday', value=5),
        discord.app_commands.Choice(name='Saturday', value=6),
        discord.app_commands.Choice(name='Sunday', value=7),
    ]
)
async def timesheet(ctx: commands.Context, day: discord.app_commands.Choice[int], timezone: str, start_time: str,
                    end_time: str, change: discord.app_commands.Choice[int] = 1):
    """Update and adjust your predicted weekly availability."""
    guild_id = ctx.guild.id
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    ...
    # Validate and parse the change value
    if change != 1:
        change_value = change.value
    else:
        change_value = 1  # Default to 'Add'

    # Validate and parse the day value
    if day and change_value < 4:
        day_value = day.value
        if not 1 <= day_value <= 7:
            embed = discord.Embed(
                title="Day Error",
                description=f"{day} is not a valid day of the week!",
                colour=discord.Colour.red(),
            )
            await ctx.followup.send(embed=embed)
            cursor.close()
        ...
        day_value = None  # Not needed for 'Clear All'

    # Validate the player's existence
    cursor.execute(
        "SELECT player_Name FROM player_characters WHERE player_name = ?", (ctx.user.name,)
    )
    player_info = cursor.fetchone()
    if player_info is None:
        embed = discord.Embed(
            title="Player Error",
            description=f"{ctx.user.name} is not a valid player!",
            colour=discord.Colour.red(),
        )
        await ctx.followup.send(embed=embed)
        cursor.close()
        db.close()
        return
        # Handle 'Clear' and 'Clear All' actions
    if change_value >= 3:
        cursor.close()
        db.close()
        await EventCommand.clear_timesheet(guild_id, change_value, day_value, ctx.user.name)
        if change_value == 3:
            embed = discord.Embed(
                title="Time Sheet Update",
                description=f"{ctx.user.name} has cleared availability on {day.name}!",
                colour=discord.Colour.green(),
            )
            await ctx.followup.send(embed=embed)
        elif change_value == 4:
            embed = discord.Embed(
                title="Time Sheet Update",
                description=f"{ctx.user.name} has cleared all availability!",
                colour=discord.Colour.green(),
            )
            await ctx.followup.send(embed=embed)
        return
    ...
    if not start_time_parsed or not end_time_parsed:
        embed = discord.Embed(
            title="Time Error",
            description=f"'{start_time}' or '{end_time}' is not a valid time!",
            colour=discord.Colour.red(),
        )
        await ctx.followup.send(embed=embed)
        cursor.close()
    ...
    try:
        ...
        embed = discord.Embed(
            ...
            description=f"'{timezone}' is not a valid time zone!",
            colour=discord.Colour.red(),
        )
        await ctx.followup.send(embed=embed)
        cursor.close()
    ...
    weekday = day_value - 1  # Adjust for Python's weekday (0=Monday)
    next_date = get_next_weekday(weekday)

    # Combine date with times
    start_datetime = datetime.combine(next_date, start_time_parsed, tzinfo=tzinfo)
    end_datetime = datetime.combine(next_date, end_time_parsed, tzinfo=tzinfo)

    # Adjust end time if it is before start time (assumes end time is on the next day)
    ...
    # Adjust day values based on UTC times
    start_day = (start_datetime_utc.weekday() + 1) % 7 or 7  # Adjust to 1-7 scale
    end_day = (end_datetime_utc.weekday() + 1) % 7 or 7

    # Close the cursor and database before calling external functions
    cursor.close()
    db.close()

    # Call your existing function to handle the timesheet update
    validation = await EventCommand.timesheet(
        self,
        guild_id,
        ctx.user.name,
        timezone,
    ...
    if validation == 0:
        ...
        start_time_unix = datetime.fromisoformat(str(start_datetime))
        end_time_unix = datetime.fromisoformat(str(end_datetime))
        if change_value == 1:
            embed = discord.Embed(
                title="Time Sheet Update",
                description=(
                    f"{ctx.user.name} has added availability from <t:{int(start_time_unix.timestamp())}:t> "
                    f"to <t:{int(end_time_unix.timestamp())}:t> on {day.name}!"
                ),
                colour=discord.Colour.green(),
            )
            await ctx.followup.send(embed=embed)
            ...
            embed = discord.Embed(
                title="Time Sheet Update",
                description=(
                    f"{ctx.user.name} has removed availability from <t:{int(start_time_unix.timestamp())}:t> "
                    f"to <t:{int(end_time_unix.timestamp())}:t> on {day.name}!"
                ),
                colour=discord.Colour.green(),
            )
...
@app_commands.choices(
    choice=[discord.app_commands.Choice(name='Add', value=1), discord.app_commands.Choice(name='Remove', value=2)])
async def request(ctx: commands.Context, character_name: str, group_name: str, description: str,
                  choice: discord.app_commands.Choice[int] = 1):
    ...
    if choice == 1:
        ...
        choice = choice.value
    cursor.execute(
        f"select character_name from player_characters where player_name = ? and character_name = ? OR Nickname = ? and player_name = ?",
        (ctx.user.name, character_name, character_name, ctx.user.name))
    player_info = cursor.fetchone()
    if player_info is not None and choice == 1:
        ...
        if group_info is not None:
            ...
            cursor.close()
            db.close()
            await EventCommand.group_request(guild_id, ctx.user.name, character_name, group_name, choice,
                                             description)
        ...
        if choice == 2:
            ...
            if group_info is not None:
                ...
                await EventCommand.group_request(guild_id, ctx.user.name, character_name, group_name, choice,
                                                 description)
                embed = discord.Embed(title=f"Group Request",
                                      description=f'{ctx.user.name} has removed their group request!',
                                      colour=discord.Colour.green())
            else:
                embed = discord.Embed(title=f"Group Request Error",
                                      description=f'{ctx.user.name} could not be found to be hosting a group!',
                                      colour=discord.Colour.red())
            await ctx.response.send_message(embed=embed)
        else:
...
@app_commands.autocomplete(group_id=group_id_autocompletion)
@app_commands.choices(
    choice=[discord.app_commands.Choice(name='Join', value=1), discord.app_commands.Choice(name='Leave', value=2)])
async def groupup(ctx: commands.Context, group_id: int, character_name: str,
                  choice: discord.app_commands.Choice[int] = 1):
    ...
    if choice == 1:
        ...
        if group is None:
            ...
            db.close()
            if player_info is not None:
                await EventCommand.group_join(guild_id, group_id, ctx.user.name, character_name, choice)
                embed = discord.Embed(title=f"Group Request",
                                      description=f'{ctx.user.name} has joined group {group_id}!',
                                      colour=discord.Colour.green())
                await ctx.response.send_message(embed=embed)
            else:
                embed = discord.Embed(title=f"No characters with that name under this player found.")
        ...
        if group is None:
            ...
            if player_info is not None:
                await EventCommand.group_join(guild_id, group_id, ctx.user.name, character_name, choice)
                embed = discord.Embed(title=f"Group Request", description=f'{ctx.user.name} has left group {group_id}!',
                                      colour=discord.Colour.green())
                await ctx.response.send_message(embed=embed)
            else:
                embed = discord.Embed(title=f"No characters with that name under this player found.")
...
@app_commands.choices(
    day=[discord.app_commands.Choice(name='Monday', value=1), discord.app_commands.Choice(name='Tuesday', value=2),
         ...
         discord.app_commands.Choice(name='Sunday', value=7)])
async def availability(ctx: commands.Context, player: typing.Optional[discord.Member],
    ...
    if day_value < 1 or day_value > 7:
        ...
        if player_utc_offset is not None:
            ...
            with open('C:\\Pathparser\\plots\\timecard_plot.png', 'rb') as f:
                ...
                await ctx.response.send_message(f"Here's the availability chart for {player} on {day.name}:",
                                                file=picture)
        else:
...
@app_commands.choices(
    day=[discord.app_commands.Choice(name='Monday', value=1), discord.app_commands.Choice(name='Tuesday', value=2),
         ...
         discord.app_commands.Choice(name='Friday', value=5), discord.app_commands.Choice(name='Saturday', value=6),
         discord.app_commands.Choice(name='Sunday', value=7)])
async def parties(ctx: commands.Context, day: discord.app_commands.Choice[int], group_id: int = 0):
    """Display historical Session Requests"""
    guild_id = ctx.guild.id
    db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
    cursor = db.cursor()
    ...
    if day_value < 1 or day_value > 7:
        ...
        if group_id == 0:
            ...
            if group_info is not None:
                ...
                with open('C:\\Pathparser\\plots\\timecard_plot.png', 'rb') as f:
                    picture = discord.File(f)
                await ctx.response.send_message(embed=embed, file=picture)
            else:
                embed = discord.Embed(title=f"Group Request Error", description=f'Group {group_id} could not be found!',
                                      colour=discord.Colour.red())
                await ctx.response.send_message(embed=embed)
...
class InputView(discord.ui.View):
    ...
    async def enter_text_button(interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
...
cProfile.run('bot.run(os.getenv("DISCORD_TOKEN"))', 'profile_output')
