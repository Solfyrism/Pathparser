...
def can_observe_dst(tz_name):
    tz = pytz.timezone(tz_name)

    # Get the current date and check for DST
    now = datetime.now(tz)
    current_offset = now.utcoffset()
    dst_offset = now.dst()

    # If the DST offset is not None and is greater than the standard offset, then DST is observed
    return dst_offset is not None and dst_offset != datetime.timedelta(0)
...
class Managed_Events(commands.Cog):
    ...
    async def create_kingdom(self, kingdom, password, government, alignment, economy, loyalty, stability, guild_id,
                             author):
        ...
        for leader in leadership_roles:
            ...
            val = (kingdom_info[0] + leader[1], kingdom_info[1] + leader[2], kingdom_info[2] + leader[3],
                   kingdom_info[3] + leader[4])
            cursor.execute(sql, val)
    ...
    async def modify_kingdom(self, old_kingdom, new_kingdom, new_password, new_government, new_alignment, guild_id,
                             author):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        cursor.execute(
            f"""SELECT Government, Alignment, Economy, Loyalty, Stability FROM Kingdoms WHERE Kingdom = '{old_kingdom}'""")
        kingdom_info = cursor.fetchone()
        cursor.execute(
            f"""SELECT Economy, Loyalty, Stability FROM AA_Alignment WHERE Alignment = '{kingdom_info[1]}'""")
        ...
        cursor.execute(
            f"""SELECT Corruption, Crime, Law, Lore, Productivity, Society FROM AA_Government WHERE Government = '{kingdom_info[0]}'""")
        old_government_info = cursor.fetchone()
        cursor.execute(
            f"""SELECT Corruption, Crime, Law, Lore, Productivity, Society FROM AA_Government WHERE Government = '{new_government}'""")
        ...
        val = (new_kingdom, new_password, new_government, new_alignment,
               kingdom_info[2] - old_alignment_info[0] + new_alignment_info[0],
               kingdom_info[3] - old_alignment_info[1] + new_alignment_info[1],
               kingdom_info[4] - old_alignment_info[2] + new_alignment_info[2])
        cursor.execute(sql, val)
        cursor.execute(f"""UPDATE Kingdoms_Custom SET Kingdom = '{new_kingdom}' WHERE Kingdom = '{old_kingdom}'""")
        cursor.execute(
            f"""SELECT Kingdom, Settlement, Corruption, Crime, Productivity, law, Lore, Society FROM Settlements WHERE Settlement = '{old_kingdom}'""")
        ...
        for settlement in settlement_info:
            ...
            val = (new_kingdom, settlement[3] - old_government_info[0] + new_government_info[0],
                   settlement[4] - old_government_info[1] + new_government_info[1],
                   settlement[5] - old_government_info[2] + new_government_info[2],
                   settlement[6] - old_government_info[3] + new_government_info[3],
                   settlement[7] - old_government_info[4] + new_government_info[4],
                   settlement[8] - old_government_info[5] + new_government_info[5])
            cursor.execute(sql, val)
    ...
    async def customize_kingdom_modifiers(self, kingdom, control_dc, economy, loyalty, stability, fame, unrest,
                                          consumption, guild_id, author):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        ...
        cursor.execute(sql, val)
        cursor.execute(
            f"""SELECT Control_DC,Economy,Loyalty,Stability, Fame,Unrest,Consumption from kingdoms where kingdom = '{kingdom}'""")
        ...
        val = (
        kingdoms[0] + control_dc - custom[1], kingdoms[1] + economy - custom[2], kingdoms[2] + loyalty - custom[3],
        kingdoms[3] + stability - custom[4], kingdoms[4] + fame - custom[5], kingdoms[5] + unrest - custom[6],
        kingdoms[6] + consumption - custom[7])
        cursor.execute(sql, val)
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def create_blueprint(self, building, build_points, lots, economy, loyalty, stability, fame, unrest,
                               corruption, crime, productivity, law, lore, society, danger, defence, base_value,
                               spellcasting, supply, settlement_limit, district_limit, description, guild_id, author):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        ...
        val = (
        building, build_points, lots, economy, loyalty, stability, fame, unrest, corruption, crime, productivity, law,
        lore, society, danger, defence, base_value, spellcasting, supply, settlement_limit, district_limit, description)
        cursor.execute(sql, val)
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def modify_blueprint(self, building, new_build_points, new_lots, new_economy, new_loyalty, new_stability,
                               new_fame, new_unrest, new_corruption, new_crime, new_productivity, new_law, new_lore,
                               new_society, new_danger, new_defence, new_base_value, new_spellcasting, new_supply,
                               new_settlement_limit, new_district_limit, new_description, guild_id, author):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        ...
        for held in holdings:
            ...
            val = (kingdoms[4] - (1 / 4 * holding_lots), kingdoms[5] + (holding_bp / 2),
                   kingdoms[6] + floor((holding_bp / 2) * settlements[14]), kingdoms[8] + (holding_lots * 50),
                   kingdoms[9] + holding_economy, kingdoms[10] + holding_loyalty, kingdoms[11] + holding_stability,
                   kingdoms[12] + holding_fame, kingdoms[13] + holding_unrest)
            cursor.execute(sql, val)
            ...
            val = (settlements[3] + (1 / 4 * holding_lots), settlements[4] + (holding_lots * 50),
                   settlements[5] + holding_corruption, settlements[6] + holding_crime,
                   settlements[7] + holding_productivity, settlements[8] + holding_law, settlements[9] + holding_lore,
                   settlements[10] + holding_society, settlements[11] + holding_danger,
                   settlements[12] + holding_defence, settlements[13] + holding_bv, settlements[14] + holding_sc,
                   settlements[15] + holding_supply)
            cursor.execute(sql, val)
            ...
            val = (
            held[4] + holding_lots, held[5] + holding_economy, held[6] + holding_loyalty, held[7] + holding_stability,
            held[8] + holding_fame, held[9] + holding_unrest, held[10] + holding_corruption, held[11] + holding_crime,
            held[12] + holding_productivity, held[13] + holding_law, held[14] + holding_lore,
            held[15] + holding_society, held[16] + holding_danger, held[17] + holding_defence, held[18] + holding_bv,
            held[19] + holding_sc, held[20] + holding_supply)
            cursor.execute(sql, val)
        ...
        val = (
        new_build_points, new_lots, new_economy, new_loyalty, new_stability, new_fame, new_unrest, new_corruption,
        new_crime, new_productivity, new_law, new_lore, new_society, new_danger, new_defence, new_base_value,
        new_spellcasting, new_supply, new_settlement_limit, new_district_limit, new_description)
        cursor.execute(sql, val)
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def remove_blueprint(self, building, guild_id, author):
        ...
        for held in holdings:
            ...
            val = (kingdoms[4] - (1 / 4 * held[5]), kingdoms[5] + floor((held[3] * building_info[2]) / 2),
                   kingdoms[6] + ((held[3] * building_info[2]) / 2) * settlements[14], kingdoms[8] - (held[4] * 50),
                   kingdoms[9] - held[5], kingdoms[10] - held[6], kingdoms[11] - held[7], kingdoms[12] - held[8],
                   kingdoms[13] - held[9])
            cursor.execute(sql, val)
            ...
            val = (settlements[3] - (1 / 4 * held[4]), settlements[4] - (held[4] * 50), settlements[5] - held[10],
                   settlements[6] - held[11], settlements[7] - held[12], settlements[8] - held[13],
                   settlements[9] - held[14], settlements[10] - held[15], settlements[11] - held[16],
                   settlements[12] - held[17], settlements[13] - held[18], settlements[14] - held[19])
            cursor.execute(sql, val)
            sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def destroy_settlement(self, kingdom, settlement, guild_id, author):
        ...
        cursor = db.cursor()
        cursor.execute(
            f"""Select Kingdom, Control_DC, Build_Points, Stabilization_Points, Population, Economy, Loyalty, Stability, Fame, Unrest FROM kingdoms WHERE kingdom = '{kingdom}'""")
        kingdoms = cursor.fetchone()
        cursor.execute(
            f"""SELECT Corruption FROM Settlements WHERE Kingdom = '{kingdom}' AND Settlement = '{settlement}'""")
        settlement_corruption = cursor.fetchone()
        cursor.execute(
            f"""SELECT Building, Constructed FROM Buildings WHERE kingdom = '{kingdom}' AND settlement = '{settlement}'""")
        ...
        cursor.execute(
            f"""SELECT Kingdom, SUM(Lots), SUM(Economy), SUM(Loyalty), SUM(Stability), SUM(Fame), SUM(Unrest) from Buildings where kingdom = '{kingdom}' and settlement = '{settlement}'""")
        ...
        cursor.execute(
            f"""DELETE from Settlements_Custom where Kingdom = '{kingdom}' AND Settlement = '{settlement}'""")
        if holding_sum[0] is not None:
            ...
            val = (kingdoms[1] - (holding_sum[1] / 4), kingdoms[2] + sum_bp, kingdoms[3] + sum_sp,
                   kingdoms[4] + (holding_sum[1] * 50), kingdoms[5] + holding_sum[2], kingdoms[6] + holding_sum[3],
                   kingdoms[7] + holding_sum[4], kingdoms[8] + holding_sum[5], kingdoms[9] + holding_sum[6])
            cursor.execute(sql, val)
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def modify_settlement(self, kingdom, old_settlement, new_settlement, guild_id, author):
        ...
        cursor = db.cursor()
        cursor.execute(
            f"""UPDATE Settlements SET Settlement = '{new_settlement}' where Kingdom = '{kingdom}' AND Settlement = '{old_settlement}'""")
        cursor.execute(
            f"""UPDATE Buildings SET Settlement = '{new_settlement}' where Kingdom = '{kingdom}' AND Settlement = '{old_settlement}'""")
        cursor.execute(
            f"""UPDATE Settlements_Custom SET Settlement = '{new_settlement}' where Kingdom = '{kingdom}' AND Settlement = '{old_settlement}'""")
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (
        author, kingdom, time, 'Settlements', f'change from {old_settlement} to {new_settlement} in {kingdom}', 0,
        'N/A')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
    ...
    async def custom_settlement_modifiers(self, kingdom, settlement, corruption, crime, productivity, law, lore,
                                          society, danger, defence, base_value, spellcasting, supply, guild_id, author):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        cursor.execute(
            f"""SELECT * FROM Settlements_Custom WHERE kingdom = '{kingdom}' AND settlement = '{settlement}'""")
        custom = cursor.fetchone()
        ...
        cursor.execute(sql, val)
        cursor.execute(
            f"""SELECT Corruption, Crime, Productivity, Law, Lore, Society, Danger, Defence, Base_Value, Spellcasting, Supply FROM Settlements WHERE kingdom = '{kingdom}' AND settlement = '{settlement}'""")
        settlements = cursor.fetchone()
        ...
        val = (settlements[0] + corruption - custom[2], settlements[1] + crime - custom[3],
               settlements[2] + productivity - custom[4], settlements[3] + law - custom[5],
               settlements[4] + lore - custom[6], settlements[5] + society - custom[7],
               settlements[6] + danger - custom[8], settlements[7] + defence - custom[9],
               settlements[8] + base_value - custom[10], settlements[9] + spellcasting - custom[11],
               settlements[10] + supply - custom[12])
        cursor.execute(sql, val)
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (
        author, kingdom, time, 'Settlement_Modifies', f'adjust the modifies of {settlement} in {kingdom}', 0, 'N/A')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
        db.close()
    ...
    async def construct_building(self, kingdom, settlement, building, amount, guild_id, author):
        ...
        cursor = db.cursor()
        cursor.execute(
            f"""SELECT Control_DC, Build_Points, Stabilization_Points, Population, Economy, Loyalty, Stability, Fame, Unrest from kingdoms where kingdom = '{kingdom}'""")
        kingdoms = cursor.fetchone()
        cursor.execute(
        ...
        settlements = cursor.fetchone()
        cursor.execute(
            f"""SELECT Constructed, Lots, Economy, Loyalty, Stability, Fame, Unrest, Corruption, Crime, Productivity, law, Lore, Society, Danger, Defence, Base_Value, Spellcasting, Supply from Buildings where kingdom = '{kingdom}' AND settlement = '{settlement}' AND building = '{building}'""")
        holdings = cursor.fetchone()
        cursor.execute(
            f"""SELECT Build_Points, Lots, Economy, Loyalty, Stability, Fame, Unrest, Corruption, Crime, Productivity, Law, Lore, Society, Danger, Defence, Base_Value, Spellcasting, Supply, Settlement_Limit, District_Limit from Buildings_Blueprints where building = '{building}'""")
        ...
        if holdings is None:
            ...
            val = (
            kingdom, settlement, building, amount, lots, economy, loyalty, stability, fame, unrest, corruption, crime,
            productivity, law, lore, society, danger, defence, base_value, spellcasting, supply)
            cursor.execute(sql, val)
        if holdings is not None:
            ...
            val = (holdings[0] + amount, holdings[1] + lots, holdings[2] + economy, holdings[3] + loyalty,
                   holdings[4] + stability, holdings[5] + fame, holdings[6] + unrest, holdings[7] + corruption,
                   holdings[8] + crime, holdings[9] + productivity, holdings[10] + law, holdings[11] + lore,
                   holdings[12] + society, holdings[13] + danger, holdings[14] + defence, holdings[15] + base_value,
                   holdings[16] + spellcasting, holdings[17] + supply)
            cursor.execute(sql, val)
        sql = f"""UPDATE kingdoms SET Control_DC = ?, Build_Points = ? , Stabilization_points = ? , Population = ?, Economy = ?, Loyalty = ?, Stability = ?, Fame = ?, Unrest = ? WHERE kingdom = '{kingdom}'"""
        val = (kingdoms[0] + (1 / 4 * lots), kingdoms[1] - build_points, kingdoms[2] - (build_points * settlements[12]),
               kingdoms[3] + (lots * 50), kingdoms[4] + economy, kingdoms[5] + loyalty, kingdoms[6] + stability,
               kingdoms[7] + fame, kingdoms[8] + unrest)
        cursor.execute(sql, val)
        ...
        val = (settlements[0] + (1 / 4 * lots), settlements[1] + (lots * 50), settlements[2] + corruption,
               settlements[3] + crime, settlements[4] + productivity, settlements[5] + law, settlements[6] + lore,
               settlements[7] + society, settlements[8] + danger, settlements[9] + defence,
               settlements[10] + base_value, settlements[11] + spellcasting, settlements[12] + supply)
        cursor.execute(sql, val)
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def destroy_building(self, kingdom, settlement, building, amount, guild_id, author):
        ...
        if holdings[0] - amount == 0:
            cursor.execute(
                f"""DELETE FROM Buildings where Kingdom = '{kingdom}' AND Settlement = '{settlement}' AND Building = '{building}'""")
        if holdings[0] - amount > 0:
            sql = f"""UPDATE Buildings SET Constructed = ?, Lots = ?, Economy = ?, Loyalty = ?, Stability = ?, Fame = ?, Unrest = ?, Corruption = ?, Crime = ?, Productivity = ?, Law = ?, Lore = ?, Society = ?, Danger = ?, Defence = ?, Base_Value = ?, Spellcasting = ?, Supply = ? WHERE Kingdom = '{kingdom}' AND Settlement = '{settlement}' AND Building = '{building}'"""
            val = (holdings[0] - amount, holdings[1] - lots, holdings[2] - economy, holdings[3] - loyalty,
                   holdings[4] - stability, holdings[5] - fame, holdings[6] - unrest, holdings[7] - corruption,
            ...
            cursor.execute(sql, val)
        sql = f"""UPDATE kingdoms SET Control_DC = ?, Build_Points = ? , Stabilization_points = ? , Population = ?, Economy = ?, Loyalty = ?, Stability = ?, Fame = ?, Unrest = ? WHERE kingdom = '{kingdom}'"""
        val = (kingdoms[0] - (1 / 4 * lots), kingdoms[1] + (build_points / 2),
               kingdoms[2] + (build_points / 2 * settlements[12]), kingdoms[3] - (lots * 50), kingdoms[4] - economy,
               kingdoms[5] - loyalty, kingdoms[6] - stability, kingdoms[7] - fame, kingdoms[8] - unrest)
        cursor.execute(sql, val)
        sql = f"""UPDATE settlements SET Size = ?, Population = ?, Corruption = ?, Crime = ?, Productivity = ?, Law = ?, Lore = ?, Society = ?, Danger = ?, Defence = ?, Base_Value = ?, Spellcasting = ?, Supply = ? WHERE kingdom = '{kingdom}' AND settlement = '{settlement}'"""
        val = (settlements[0] - (1 / 4 * lots), settlements[1] - (lots * 50), settlements[2] - corruption,
               settlements[3] - crime, settlements[4] - productivity, settlements[5] - law, settlements[6] - lore,
    ...
    async def modify_leader(self, kingdom, leader, title, modifier, column, economy_modifier, loyalty_modifier,
                            stability_modifier, guild_id, author):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        cursor.execute(
            f"""SELECT Economy, Loyalty, Stability, Unrest FROM leadership WHERE Kingdom = '{kingdom}' AND Title = '{title}'""")
        ...
        sql = f"""UPDATE kingdoms SET Economy = ?, Loyalty = ?, Stability = ?, Unrest = ? WHERE kingdom = '{kingdom}'"""
        val = (kingdom_values[0] - leader_values[0] + economy_modifier,
               kingdom_values[1] - leader_values[1] + loyalty_modifier,
               kingdom_values[2] - leader_values[2] + stability_modifier, kingdom_values[3] - leader_values[3])
        cursor.execute(sql, val)
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def remove_leader(self, kingdom, title, guild_id, author):
        ...
        kingdom_values = cursor.fetchone()
        cursor.execute(
            f"""SELECT VPEconomy, VPLoyalty, VPStability, VPUnrest FROM AA_Leadership_Roles WHERE Title = '{title}'""")
    ...
    async def claim_hex(self, kingdom, hex_terrain, guild_id, author):
        ...
        kingdom_info = cursor.fetchone()
        cursor.execute(
            f"""SELECT Amount FROM hexes WHERE kingdom = '{kingdom}' AND Hex_Terrain = '{hex_terrain}' AND Improvement = 'None'""")
        hex = cursor.fetchone()
        if hex is None:
        ...
        if hex is not None:
            value = (hex[0] + 1)
            cursor.execute(
                f"""UPDATE hexes SET Amount = {value} WHERE kingdom = '{kingdom}' AND hex_terrain = '{hex_terrain}' AND Improvement = 'None'""")
    ...
    async def relinquish_hex(self, kingdom, hex_terrain, guild_id, author):
        ...
        if hex[0] - 1 == 0:
            cursor.execute(
                f"""DELETE FROM hexes  WHERE kingdom = '{kingdom}' AND hex_terrain = '{hex_terrain}' AND Improvement = 'None'""")
        if hex[0] - 1 != 0:
            cursor.execute(
                f"""UPDATE hexes SET Amount = ({hex[0]} - 1) WHERE kingdom = '{kingdom}' AND hex_terrain = '{hex_terrain}' AND Improvement = 'None'""")
        sql = f"""UPDATE kingdoms SET Control_DC = ?, Size = ? where kingdom = '{kingdom}'"""
        val = (kingdom_info[0] - 1, kingdom_info[1] - 1)
        cursor.execute(sql, val)
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    ...
    async def improve_hex(self, kingdom, hex_terrain, improvement, guild_id, author):
        ...
        cursor = db.cursor()
        cursor.execute(
            f"""SELECT Build_points, Economy, Loyalty, Stability, Unrest, Consumption, Defence FROM Hexes_Improvements WHERE Improvement = '{improvement}'""")
        improvement_info = cursor.fetchone()
        cursor.execute(
            f"""SELECT Amount FROM hexes WHERE kingdom = '{kingdom}' AND Improvement = 'None' AND Hex_Terrain = '{hex_terrain}'""")
        hex_info = cursor.fetchone()
        cursor.execute(
            f"""SELECT Build_Points, Economy, Loyalty, Stability, Unrest, Consumption FROM kingdoms WHERE kingdom = '{kingdom}'""")
        kingdom_info = cursor.fetchone()
        cursor.execute(
            f"""SELECT Amount, Economy, Loyalty, Stability, Unrest, Consumption, Defence FROM hexes WHERE kingdom = '{kingdom}' AND Hex_Terrain = '{hex_terrain}' AND Improvement = '{improvement}'""")
        ...
        if hex_info[0] - 1 == 0:
            ...
            val = (hex_info[0] + -1)
            cursor.execute(
                f"""UPDATE hexes SET Amount = {val} WHERE Kingdom = '{kingdom}' AND Hex_Terrain = '{hex_terrain}' AND Improvement = 'None'""")
        if total is None:
            sql = f"""INSERT INTO hexes(kingdom, Hex_Terrain, Amount, Improvement, Economy, Loyalty, Stability, Unrest, Consumption, Defence) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
            val = (kingdom, hex_terrain, 1, improvement, improvement_info[1], improvement_info[2], improvement_info[3],
                   improvement_info[4], improvement_info[5], improvement_info[6])
            cursor.execute(sql, val)
        if total is not None:
            ...
            val = (total[0] + 1, total[1] + improvement_info[1], total[2] + improvement_info[2],
                   total[3] + improvement_info[3], total[4] + improvement_info[4], total[5] + improvement_info[5],
                   total[6] + improvement_info[6])
            cursor.execute(sql, val)
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def diminish_hex(self, kingdom, hex_terrain, improvement, guild_id, author):
        ...
        improvement_info = cursor.fetchone()
        cursor.execute(
            f"""SELECT Amount, Economy, Loyalty, Stability, Unrest, Consumption, Defence FROM hexes WHERE kingdom = '{kingdom}' AND Improvement = 'None' AND Hex_Terrain = '{hex_terrain}'""")
        hex_info = cursor.fetchone()
        cursor.execute(
            f"""SELECT Build_Points, Economy, Loyalty, Stability, Unrest, Consumption FROM kingdoms WHERE kingdom = '{kingdom}'""")
        kingdom_info = cursor.fetchone()
        ...
        total = cursor.fetchone()
        sql = f"""UPDATE kingdoms SET Build_Points = ?, Economy = ?, Loyalty = ?, Stability = ?, Unrest = ?, Consumption = ? WHERE kingdom = '{kingdom}'"""
        val = (kingdom_info[0] + (improvement_info[0] / 2), kingdom_info[1] - improvement_info[1],
               kingdom_info[2] - improvement_info[2], kingdom_info[3] - improvement_info[3],
               kingdom_info[4] - improvement_info[4], kingdom_info[5] - improvement_info[5])
        cursor.execute(sql, val)
        if hex_info is None:
            sql = f"""INSERT INTO hexes(kingdom, Hex_Terrain, Amount, Improvement, Economy, Loyalty, Stability, Unrest, Consumption, Defence) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
            val = (kingdom, hex_terrain, 1, 'None', 0, 0, 0, 0, 0, 0)
        ...
        if total[0] - 1 == 0:
            cursor.execute(
                f"""DELETE FROM hexes WHERE kingdom = '{kingdom}' AND Hex_Terrain = '{hex_terrain}' AND Improvement = '{improvement}'""")
        if total[0] - 1 >= 1:
            sql = f"""UPDATE hexes SET Amount = ?, Economy = ?, Loyalty = ?, Stability =?, Unrest = ?, Consumption = ?, Defence = ? WHERE kingdom = '{kingdom}' AND Hex_Terrain = '{hex_terrain}' AND Improvement = '{improvement}'"""
            val = (total[0] - 1, total[1] - improvement_info[1], total[2] - improvement_info[2],
                   total[3] - improvement_info[3], total[4] - improvement_info[4], total[5] - improvement_info[5],
    ...
    async def add_hex_improvements(self, improvement, build_points, road_multiplier, economy, loyalty, stability,
                                   unrest, consumption, defence, taxation, cavernous, coastline, desert, forest, hills,
                                   jungle, marsh, mountains, plains, water, guild_id, author):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        ...
        val = (
        improvement, build_points, road_multiplier, economy, loyalty, stability, unrest, consumption, defence, taxation,
    ...
    async def remove_hex_improvement(self, improvement, guild_id, author):
        ...
        cursor = db.cursor()
        cursor.execute(
            f"""SELECT kingdom, Amount, Economy, Loyalty, Stability, Unrest, Consumption, Defence FROM hexes where Improvement  = '{improvement}'""")
        ...
        for hex_improvements in hex_info:
            cursor.execute(
                f"""SELECT Control_DC, Build_Points, Size, Economy, Loyalty, Stability, Unrest, Consumption FROM kingdoms where kingdom = '{hex_improvements[0]}'""")
            kingdoms = cursor.fetchone()
            ...
            val = (kingdoms[0] - hex_improvements[1], kingdoms[1] + (improvement_info[0] * hex_improvements[1]),
                   kingdoms[2] - hex_improvements[1], kingdoms[3] - hex_improvements[2],
                   kingdoms[4] - hex_improvements[3], kingdoms[5] - hex_improvements[4],
                   kingdoms[6] - hex_improvements[5], kingdoms[7] - hex_improvements[6])
            cursor.execute(sql, val)
    ...
    async def modify_hex_improvement(self, old_improvement, new_improvement, new_build_points, new_road_multiplier,
                                     new_economy, new_loyalty, new_stability, new_unrest, new_consumption, new_defence,
                                     new_taxation, new_cavernous, new_coastline, new_desert, new_forest, new_hills,
                                     new_jungle, new_marsh, new_mountains, new_plains, new_water, guild_id, author):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        cursor.execute(
            f"""SELECT kingdom, Amount, Economy, Loyalty, Stability, Unrest, Consumption, Defence, new_taxation FROM hexes where Improvement  = '{old_improvement}'""")
        hex_info = cursor.fetchall()
        cursor.execute(
            f"""SELECT Build_points, Economy, Loyalty, Stability, Unrest, Consumption, Defence, Taxation, Cavernous, Coastline, Desert, Forest, Hills, Jungle, Marsh, Mountains, Plains, Water FROM Hexes_Improvements WHERE Improvement = '{old_improvement}'""")
        improvement_info = cursor.fetchone()
        ...
        for hex_improvements in hex_info:
            cursor.execute(
                f"""SELECT Build_Points, Economy, Loyalty, Stability, Unrest, Consumption FROM kingdoms where kingdom = '{hex_improvements[0]}'""")
            kingdoms = cursor.fetchone()
            sql = f"""UPDATE kingdoms SET Build_Points = ?, Economy = ?, Loyalty = ?, Stability = ?, Unrest = ?, Consumption = ? WHERE kingdom = '{hex_improvements[0]}'"""
            val = (
            kingdoms[0] - (adj_build_points * hex_improvements[1]), kingdoms[1] + (adj_economy * hex_improvements[1]),
            kingdoms[2] + (adj_loyalty * hex_improvements[1]), kingdoms[3] + (adj_stability * hex_improvements[1]),
            kingdoms[4] + (adj_unrest * hex_improvements[1]), kingdoms[5] + (adj_consumption * hex_improvements[1]))
            cursor.execute(sql, val)
            ...
            val = (new_improvement, adj_economy * hex_improvements[1], adj_loyalty * hex_improvements[1],
                   adj_stability * hex_improvements[1], adj_unrest * hex_improvements[1],
                   adj_consumption * hex_improvements[1], adj_defence * hex_improvements[1],
                   adj_taxation * hex_improvements[1])
            cursor.execute(sql, val)
        ...
        val = (new_improvement, new_build_points, new_economy, new_loyalty, new_stability, new_unrest, new_consumption,
               new_defence, new_taxation, new_cavernous, new_coastline, new_desert, new_forest, new_hills, new_jungle,
               new_marsh, new_mountains, new_plains, new_water)
        cursor.execute(sql, val)
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (author, 'N/A', time, 'Hexes_Improvements',
               f'Adjustment from {old_improvement} to the {new_improvement} hex improvement', 0, 'N/A')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
        db.close()
    ...
    async def balance_tables(self, guild_id, author):
        ...
        cursor.execute(sql, val)
        cursor.execute(
            f"""SELECT Kingdom, Alignment, Control_DC, Economy, Loyalty, Stability, Fame, Unrest, Consumption from kingdoms""")
        ...
        for kingdom in kingdoms:
            ...
            cursor.execute(
                f"""SELECT Control_DC, Economy, Loyalty, Stability, Fame, Unrest, Consumption from Kingdoms_Custom WHERE kingdom = '{kingdom[0]}'""")
            custom_info = cursor.fetchone()
            cursor.execute(
                f"""SELECT SUM(economy), SUM(Loyalty), SUM(Stability), SUM(Unrest) FROM leadership WHERE kingdom = '{kingdom[0]}'""")
        ...
        cursor.execute(sql, val)
        cursor.execute(
            f"""SELECT Kingdom, Settlement, Corruption, Crime, Law, Lore, Productivity, Society, Danger, Defence, Base_Value, Spellcasting, Supply FROM settlements""")
        settlement_info = cursor.fetchall()
        for settlement in settlement_info:
            cursor.execute(f"""SELECT Government FROM kingdoms WHERE kingdom = '{settlement[0]}'""")
            government = cursor.fetchone()
            cursor.execute(
                f"""SELECT Corruption, Crime, Law, Lore, Productivity, Society FROM AA_Government WHERE Government = '{government[0]}'""")
            government_info = cursor.fetchone()
            cursor.execute(
                f"""SELECT Corruption, Crime, Law, Lore, Productivity, Society, Danger, Defence, Base_Value, Spellcasting, Supply FROM Settlements_Custom WHERE Kingdom = '{settlement[0]}' AND Settlement = '{settlement[1]}'""")
            custom_info = cursor.fetchone()
            ...
            val = (
            corruption, crime, law, lore, productivity, society, danger, defence, base_value, spellcasting, supply)
            cursor.execute(sql, val)
        ...
        for holding in holding_info:
            cursor.execute(
                f"""SELECT Build_Points, Lots, Economy, Loyalty, Stability, Fame, Unrest, Corruption, Crime, Productivity, Law, Lore, Society, Danger, Defence, Base_value, Spellcasting, Supply from Buildings_Blueprints where Building = '{holding[2]}'""")
            building_info = cursor.fetchone()
            ...
            val = (building_info[1] * holding[3], building_info[2] * holding[3], building_info[3] * holding[3],
                   building_info[4] * holding[3], building_info[5] * holding[3], building_info[6] * holding[3],
                   building_info[7] * holding[3], building_info[8] * holding[3], building_info[9] * holding[3],
                   building_info[10] * holding[3], building_info[11] * holding[3], building_info[12] * holding[3],
                   building_info[13] * holding[3], building_info[14] * holding[3], building_info[15] * holding[3],
                   building_info[16] * holding[3], building_info[17] * holding[3])
            cursor.execute(sql, val)
            cursor.execute(
                f"""SELECT Size, Population, Corruption, Crime, Productivity, Law, Lore, Society, Danger, Defence, Base_Value, Spellcasting, Supply FROM Settlements WHERE Kingdom = '{holding[0]}' AND Settlement = '{holding[1]}'""")
            ...
            val = (settlement_info[0] + (holding[3] * building_info[1] / 4),
                   settlement_info[1] + holding[3] * building_info[1] * 50,
                   settlement_info[2] + building_info[7] * holding[3],
                   settlement_info[3] + building_info[8] * holding[3],
                   settlement_info[4] + building_info[9] * holding[3],
                   settlement_info[5] + building_info[10] * holding[3],
                   settlement_info[6] + building_info[11] * holding[3],
                   settlement_info[7] + building_info[12] * holding[3],
                   settlement_info[8] + building_info[13] * holding[3],
                   settlement_info[9] + building_info[14] * holding[3],
                   settlement_info[10] + building_info[15] * holding[3],
                   settlement_info[11] + building_info[16] * holding[3],
                   settlement_info[12] + building_info[17] * holding[3])
            cursor.execute(sql, val)
            cursor.execute(
                f"""SELECT Control_DC, Population, Economy, Loyalty, Stability, Fame, Unrest FROM kingdoms WHERE kingdom = '{holding[0]}'""")
            kingdom_info = cursor.fetchone()
            sql = f"""UPDATE kingdoms set Control_DC = ?, Population = ?, Economy = ?, Loyalty = ?, Stability = ?, Fame = ?, Unrest = ? WHERE kingdom = '{holding[0]}'"""
            val = (
            kingdom_info[0] + (holding[3] * building_info[1] / 4), kingdom_info[1] + holding[3] * building_info[1] * 50,
            kingdom_info[2] + building_info[2] * holding[3], kingdom_info[3] + building_info[3] * holding[3],
            kingdom_info[4] + building_info[4] * holding[3], kingdom_info[5] + building_info[5] * holding[3],
            kingdom_info[6] + building_info[6] * holding[3])
            cursor.execute(sql, val)
        """NOTE: UPDATE ALL KINGDOMS BASED OFF OF HEXES"""
        cursor.execute(
            f"""SELECT Kingdom, Amount, Improvement, Economy, Loyalty, Stability, Unrest, Consumption, Defence FROM Hexes""")
        hex_info = cursor.fetchall()
        for hexes in hex_info:
            cursor.execute(
                f"""SELECT Economy, Loyalty, Stability, Unrest, Consumption, Control_DC FROM kingdoms where kingdom = '{hexes[0]}'""")
            kingdom_info = cursor.fetchone()
            ...
            if hexes[2] != 'None':
                cursor.execute(
                    f"""SELECT Economy, Loyalty, Stability, Unrest, Consumption, Defence FROM Hexes_Improvements WHERE Improvement = '{hexes[2]}'""")
                ...
                val = (hexes[1] * renovation_info[0], hexes[1] * renovation_info[1], hexes[1] * renovation_info[2],
                       hexes[1] * renovation_info[3], hexes[1] * renovation_info[4], hexes[1] * renovation_info[5])
                cursor.execute(sql, val)
                sql = f"""UPDATE kingdoms SET Control_DC = ?, Economy = ?, Loyalty = ?, Stability = ?, Unrest = ?, Consumption = ? WHERE kingdom = '{hexes[1]}'"""
                val = (control_dc, kingdom_info[0] + hexes[1] * renovation_info[0],
                       kingdom_info[1] + hexes[1] * renovation_info[1], kingdom_info[2] + hexes[1] * renovation_info[2],
                       kingdom_info[3] + hexes[1] * renovation_info[3], kingdom_info[4] + hexes[1] * renovation_info[4])
                cursor.execute(sql, val)
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def adjust_build_points(self, kingdom, build_points, guild_id, character_name, author):
        ...
        cursor.execute(sql, val)
        cursor.execute(f"""SELECT gold from Player_Characters where Character_Name = ? AND Player_Name = ?""",
                       (character_name, author))
        gold = cursor.fetchone()
        cost = build_points * 4000
        value = gold[0] - cost
        cursor.execute(
            f"""UPDATE Player_Characters SET Gold = '{value}' WHERE Character_Name = ? AND Player_Name = ?""",
            (character_name, author))
        db.commit()
        cursor.close()
        db.close()
    ...
    async def adjust_stabilization_points(self, kingdom, stabilization_points, guild_id, author, character_name_true):
        ...
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (author, character_name_true, time, 'Kingdoms', f'adjusting stabilization points for {kingdom}',
               stabilization_points, 'N/A')
        cursor.execute(sql, val)
        cursor.execute(f"""SELECT gold from Player_Characters where Character_Name = ? AND Player_Name = ?""",
                       (character_name_true, author))
        gold = cursor.fetchone()
        cost = stabilization_points * 4000
        value = gold[0] - cost
        cursor.execute(f"""UPDATE Player_Characters SET Gold = ? WHERE ? AND Player_Name = ?""",
                       (value, character_name_true, author))
        db.commit()
        cursor.close()
        db.close()
    ...
    async def gold_change(self, guild_id, author_name, author_id, character_name, amount, expected_value,
                          lifetime_value, reason, source):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
    ...
    async def gold_set(self, guild_id, author_name, author_id, character_name, amount, expected_value, lifetime_value,
                       reason, source, table):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        new_gold = amount
        ...
        sql = "INSERT INTO A_Audit_Gold(Author_Name, Author_ID, Character_Name, Gold_Value, Effective_Gold_Value, Effective_Gold_Value_Max, Reason, Source_Command, Time) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)"
        val = (
        author_name, author_id, character_name, new_gold, new_gold_value, new_gold_value_max, reason, source, time)
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
        db.close()

    async def title_change(self, guild_id, author, author_id, true_character_name, title_name, total_fame,
                           total_prestige, reason, source):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    ...
    async def proposition_open(self, guild_id, author, author_id, true_character_name, item_name, prestige_cost, reason,
                               source):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    ...
    async def glorify(self, guild_id, author, character, fame, prestige, reason):
        ...
        cursor.execute(sql, val)
        sql = "INSERT INTO A_Audit_All(Author, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?)"
        val = (author, time, 'Player_Characters',
               f'Glorifying character by changing their fame to {fame} and prestige to {prestige}', prestige, reason)
        cursor.execute(sql, val)
    ...
    async def stg_gold_change(self, guild_id, author_name, author_id, character_name, amount, expected_value,
                              lifetime_value, reason, source):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
    ...
    async def undo_transaction(self, guild_id, transaction_id, amount, expected_value, lifetime_value, character_name,
                               player_name):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
    ...
    async def gold_transact(self, transaction_id, related_id, guild_id):
        ...
        cursor = db.cursor()
        cursor.execute(
            f"UPDATE A_Audit_Gold SET Related_Transaction_ID = {related_id} WHERE Transaction_ID = {transaction_id}")
        db.commit()
        cursor.close()
        db.close()
    ...
    async def adjust_milestones(self, character_name, milestone_total, remaining, character_level, guild_id, author):
        ...
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        cursor.execute(
            f"SELECT Player_Name, Player_ID, Character_Name, Level, Trials, Tier FROM Player_Characters where Character_Name = ?",
            (character_name,))
        characters = cursor.fetchone()
        cursor.execute(
            f"SELECT Tier, Trials, Trials_Required from AA_Trials  WHERE Trials <= {characters[4]} ORDER BY Trials DESC  LIMIT 1")
    ...
    async def adjust_personal_cap(self, guild_id, author, character_name, level_cap):
        ...
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        cursor.execute(
            f"SELECT Player_Name, Player_ID, Character_Name, Level, Trials, Tier, Milestones FROM Player_Characters where Player_Name = ? and Character_Name = ?",
            (author, character_name))
        characters = cursor.fetchone()
        cursor.execute(f"SELECT Search from Admin WHERE Identifier = 'Level_Cap'")
        max_level = cursor.fetchone()
        cursor.execute(
            f"SELECT Level, Minimum_Milestones, Milestones_To_Level FROM AA_Milestones WHERE Minimum_Milestones <= {characters[6]} ORDER BY Minimum_Milestones DESC  LIMIT 1")
        ...
        val = (level, characters[6], characters[6] - (milestone_information[0] + milestone_information[1]), true_tier,
               trials_required, level_cap, character_name, author)
        cursor.execute(sql, val)
        sql = "INSERT INTO A_Audit_All(Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?)"
    ...
    async def adjust_trials(self, character_name, total_trials, guild_id, author):
        ...
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        cursor.execute(
            f"SELECT Player_Name, Player_ID, Character_Name, Level, Trials FROM Player_Characters where Character_Name = ?",
            (character_name,))
        characters = cursor.fetchone()
        cursor.execute(
    ...
    async def wipe_unapproved(self, character_name, guild_id, author):
        ...
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (
        author, character_name, time, 'A_STG_player_characters', f"deleted {character_name}'s characters", 0, 'N/A')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
        db.close()

    async def stage_character(self, true_character_name, character_name, author, author_id, guild_id, nickname, titles,
                              description, oath_name, mythweavers, image_link, color, backstory):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        ...
        val = (
        author, author_id, true_character_name, character_name, nickname, titles, description, oath_name, 3, 0, 0, 3, 0,
        0, 0, 0, 0, mythweavers, image_link, color, 0, time, backstory)
        cursor.execute(sql, val)
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def create_character(self, guild_id, author, true_character_name):
        ...
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        cursor.execute(
            f"SELECT Player_Name, Player_ID, True_Character_Name, Character_Name, Nickname, Titles, Description, Oath, Level, Tier, Milestones, Milestones_Required, Trials, Trials_Required, Gold, Gold_Value, Gold_Value_Max, Mythweavers, Image_Link, Color, Flux FROM A_STG_Player_Characters WHERE True_Character_Name = ?",
            (true_character_name,))
        ...
        val = (character_info[0], character_info[1], character_info[2], character_info[3], character_info[4],
               character_info[5], character_info[6], character_info[7], character_info[8], character_info[9],
               character_info[10], character_info[11], character_info[12], character_info[13], character_info[14],
               character_info[15], character_info[16], character_info[17], character_info[18], character_info[19],
               character_info[20], 0, 0, time)
        cursor.execute(sql, val)
    ...
    async def create_bio(self, guild_id, true_character_name, bio, link):
        if guild_id == 883009758179762208:
            ...
            if 'worldanvil' in link:
                article_list = [article for article in client.world.articles('f7a60480-ea15-4867-ae03-e9e0c676060a',
                                                                             'c8fd1251-1077-4bbd-a9a5-797b3dbdf356')]
            ...
            cursor = db.cursor()
            cursor.execute(
                f"UPDATE Player_Characters SET Article_Link = ?, Article_ID = ? WHERE True_Character_Name = ?",
                (new_page['url'], new_page['id'], true_character_name))
            db.commit()
            cursor.close()
            db.close()
    ...
    async def report(self, guild_id, request_type, plot, overview, session_id, author, significance):
        if guild_id == 883009758179762208:
            ...
            if request_type == 1:
                ...
                cursor.execute(
                    f"UPDATE Sessions SET Article_Link = ?, Article_ID = ?, History_ID = ? WHERE Session_ID = ?",
    ...
    async def edit_stage_bio(self, guild_id, true_character_name, bio):
        if guild_id == 883009758179762208:
            db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
            cursor = db.cursor()
            cursor.execute(f"UPDATE a_stg_player_characters SET tmp_bio = ?, WHERE True_Character_Name = ?",
                           (bio, true_character_name))
            db.commit()
            cursor.close()
            db.close()
    ...
    async def clean_stg(self, guild_id, author, true_character_name):
        ...
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (
        author[0], true_character_name, time, 'player_characters', f'deleted {true_character_name} from stage', 0,
        'N/A')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
    ...
    async def edit_character(self, true_name, true_character_name, new_character_name, guild_id, new_nickname, titles,
                             description, oath_name, mythweavers, image_link, color, author):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        cursor.close()
        ...
        cursor = db.cursor()
        cursor.execute(
            f"UPDATE Player_Characters SET True_Character_Name = ?, Character_name = ?, Nickname = ?, Titles = ?, Description = ?, Oath = ?, Mythweavers = ?, Image_Link = ?, Color = ? WHERE True_Character_Name = ?",
            (true_character_name, new_character_name, new_nickname, titles, description, oath_name, mythweavers,
             image_link, color, true_name))
        print(true_name, true_character_name, new_character_name)
        if true_name != new_character_name:
            sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
            val = (
            author, true_name, time, 'player_characters', f'edited {true_name} to be {new_character_name}', 0, 'N/A')
            cursor.execute(sql, val)
            sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
            val = (
            author, new_character_name, time, 'player_characters', f'edited {true_name} to be {new_character_name}', 0,
            'N/A')
            cursor.execute(sql, val)
            cursor.execute("UPDATE A_Audit_Gold SET Character_Name = ? WHERE Character_Name = ?",
                           (new_character_name, true_name))
            cursor.execute("UPDATE A_Audit_Prestige SET Character_Name = ? WHERE Character_Name = ?",
                           (new_character_name, true_name))
            cursor.execute("UPDATE A_Audit_All SET Character = ? WHERE Character = ?", (new_character_name, true_name))
            cursor.execute("UPDATE Sessions_Participants SET Character_Name = ? WHERE Character_Name = ?",
                           (new_character_name, true_name))
            cursor.execute("UPDATE Sessions_Signups SET Character_Name = ? WHERE Character_Name = ?",
                           (new_character_name, true_name))
            cursor.execute("UPDATE Sessions_Archive SET Character_Name = ? WHERE Character_Name = ?",
                           (new_character_name, true_name))
        else:
            sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def edit_stg_character(self, true_name, true_character_name, new_character_name, guild_id, new_nickname,
                                 titles, description, oath_name, mythweavers, image_link, color, author):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        ...
        val = (
        true_character_name, new_character_name, new_nickname, titles, description, oath_name, mythweavers, image_link,
        color, true_name)
        cursor.execute(sql, val)
        if true_name != new_character_name:
            sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def session_rewards(self, author, guild_id, character_name, level, milestone_total, remaining, flux_total,
                              tier, trials_total, trials_required, fame_total, prestige_total, reason):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        ...
        val = (
        level, milestone_total, remaining, flux_total, tier, trials_total, trials_required, fame_total, prestige_total,
        character_name)
        cursor.execute(sql, val)
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def create_session(self, gm_name, session_name, session_range, session_range_id, play_location, play_time,
                             link, guild_id, author, overview, description, player_limit, overflow, plot):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        ...
        val = (gm_name, session_name, session_range, session_range_id, play_location, play_time, link, time, overview,
               description, player_limit, 1, overflow, plot)
        cursor.execute(sql, val)
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def edit_session(self, guild_id, author, session_id, session_name, session_range_name, session_range_id,
                           play_location, play_time, link, overflow):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
    ...
    async def accept_player(self, guild_id, session_name, session_id, character_name, level, gold_value, player_name,
                            player_id, author, tier):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        ...
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (
        author, character_name, time, 'Sessions_Participants', f'{character_name} is joining session {session_id}', 0,
        'Joining DND session')
        cursor.execute(f"DELETE FROM Sessions_Signups WHERE Character_Name = ? AND Session_ID = ?",
    ...
    async def remove_player(self, guild_id, session_id, player_name, author, character_name):
        ...
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        cursor.execute(
            f"DELETE FROM Sessions_Participants WHERE Session_ID = {session_id} AND Player_Name = '{player_name}'")
        db.commit()
        cursor.execute(
            f"DELETE FROM Sessions_Archive WHERE Session_ID = {session_id} AND Player_Name = '{player_name}'")
        db.commit()
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (
        author, character_name, time, 'Sessions_Participents', f'{player_name} is removed from session {session_id}', 0,
        'Removing player from DnD Session')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
        db.close()

    async def session_join(self, guild_id, session_name, session_id, player_name, player_id, character_name, level,
                           gold_value, tier):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        ...
        cursor.execute(sql, val)
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (player_name, character_name, time, 'Sessions_Signups',
               f'{character_name} is signing up for session {session_id}', 0, 'Joining DND session')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
        db.close()
    ...
    async def session_leave(self, guild_id, session_id, player_name, true_name):
        ...
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        cursor.execute(
            f"DELETE FROM Sessions_Signups WHERE Session_ID = {session_id} AND Player_Name = '{player_name}'")
        db.commit()
        cursor.execute(
            f"DELETE FROM Sessions_Participants WHERE Session_ID = {session_id} AND Player_Name = '{player_name}'")
        db.commit()
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (player_name, true_name, time, 'Sessions_Participants',
               f'{player_name} has declined to participate in session {session_id}', 0, 'leaving DND session')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
        db.close()
    ...
    async def session_cleanup(self, guild_id, session_id, author):
        ...
        cursor.execute(f"DELETE FROM Sessions_Participants WHERE Session_ID = {session_id}'")
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (author, 'N/A', time, 'Sessions', f'cleaning up session information from the stage tables', 0,
               'session_cleaning')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
    ...
    async def update_settings(self, guild_id, author, new_search, identifier):
        ...
        cursor.execute(sql, val)
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (author, 'admin', time, 'Admin', f'Updating {identifier} with new {new_search} qualifier', 0,
    ...
    async def session_log_player(self, guild_id, session_id, player_name, player_id, character_name, level, tier,
                                 effective_gold, rewarded, trials, received_gold, received_fame, received_prestige,
                                 received_flux):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        ...
        val = (session_id, player_name, player_id, character_name, level, tier, effective_gold, rewarded, trials,
               received_gold, received_fame, received_prestige, received_flux)
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
        db.close()

    async def session_log(self, guild_id, session_id, gold, flux, easy, medium, hard, deadly, trials, reward_all,
                          party_reward, rewards_message, rewards_thread, fame, prestige):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        ...
        val = (0, time, gold, flux, easy, medium, hard, deadly, trials, reward_all, party_reward, rewards_message,
               rewards_thread, fame, prestige, session_id)
        cursor.execute(sql, val)
        cursor.execute(f"DELETE FROM Sessions_Participants WHERE Session_ID = ?", (session_id,))
        db.commit()
        cursor.execute(f"DELETE FROM Sessions_Signups WHERE Session_ID = ?", (session_id,))
    ...
    async def update_session_log(self, guild_id, session_id, gold, flux, easy, medium, hard, deadly, trials, reward_all,
                                 party_reward, rewards_message_id, rewards_thread, fame, prestige):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        ...
        val = (
        gold, flux, easy, medium, hard, deadly, trials, reward_all, party_reward, rewards_message_id, rewards_thread,
        fame, prestige, session_id)
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
        db.close()

    async def update_session_log_player(self, guild_id, session_id, character_name, received_milestones, trials,
                                        received_gold, received_fame, received_prestige):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
    ...
    async def update_level_cap(self, guild_id, author, new_level):
        ...
        cursor.execute(
            f"SELECT Player_Name, Player_ID, Character_Name, Trials, Milestones, Personal_Cap FROM Player_Characters WHERE Milestones >= {minimum_milestones}")
        ...
        if characters_info is not None:
            for characters in characters_info:
                ...
                if personal_cap >= characters[4]:
                    sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
                    val = (author, 'admin', time, 'Player_Characters',
                           f'Updating {characters[2]} with new level cap of {new_level}', 0, 'administration change')
                    cursor.execute(sql, val)
                    cursor.execute(
                        f"SELECT Tier, Trials, Trials_Required from AA_Trials  WHERE Trials <= {characters[3]} ORDER BY Trials DESC  LIMIT 1")
                    current_mythic_information = cursor.fetchone()
                    cursor.execute(f"SELECT Search from Admin WHERE Identifier = 'Tier_Cap'")
                    max_tier = cursor.fetchone()
                    cursor.execute(f"SELECT Search from Admin WHERE Identifier = 'Tier_Rate_Limit_Breakpoint'")
                    ...
                    true_tier = current_mythic_information[0] if current_mythic_information[0] < int(
                        max_tier[0]) else int(max_tier[0])
                    print(true_tier)
                    true_tier = true_tier if true_tier <= rate_limited_tier else rate_limited_tier
                    print(true_tier)
                    if true_tier == int(max_tier[0]) or true_tier == rate_limited_tier:
                    ...
                    val = (
                    new_level, level_info[0] + level_info[1] - characters[4], true_tier, trials_required, characters[2])
                    cursor.execute(sql, val)
        db.commit()
        cursor.close()
        db.close()
    ...
    async def update_tier_cap(self, guild_id, author, new_tier, minimum_level):
        ...
        minimum_milestones = level_info[0]
        cursor.execute(
            f"SELECT Player_Name, Player_ID, Character_Name, Trials FROM Player_Characters WHERE Trials >= {minimum_milestones} AND Level >= {minimum_level}")
        characters_info = cursor.fetchall()
        if characters_info is not None:
            for characters in characters_info:
                sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
                val = (
                author, 'admin', time, 'Player_Characters', f'Updating {characters[2]} with new tier cap of {new_tier}',
    ...
    async def flux(self, guild_id, true_name, amount, new_flux, author):
        ...
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (
        author, f'{true_name}', time, 'Player_Characters', f'adjusting flux by {amount} to be {new_flux}', amount,
        'N/A')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
    ...
    async def set_range(self, guild_id, author, range_name, range_id, minimum, maximum):
        ...
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (
        author, f'N/A', time, 'Level_Range', f'Numbers {minimum}-{maximum} have been changed to be {range_name}', 0,
        'N/A')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
    ...
    async def customize_characters(self, guild_id, author, character_name, destination_name, destination_link,
                                   customized_name, link, flux_remaining, flux_cost):
        time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        ...
        cursor.execute(sql, val)
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (author, f'{character_name}', time, 'Player_Characters',
               f'{character_name} has been given {destination_name} of {customized_name} leaving them with {flux_remaining} flux',
               flux_cost, 'N/A')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
    ...
    async def add_fame_store(self, guild_id, author, fame_required, prestige_cost, name, effect, limit):
        ...
        cursor.execute(sql, val)
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (author, f'{name}', time, 'Store_Fame',
               f'added {name} with effect {effect} requiring {fame_required} fame and {prestige_cost} prestige', 0,
               'N/A')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
    ...
    async def add_title_store(self, guild_id, author, ubb_id, effect, fame, masculine_name, feminine_name):
        ...
        cursor.execute(sql, val)
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
        val = (author, f'{masculine_name} / {feminine_name}', time, 'Store_Title',
               f'added {masculine_name} / {feminine_name} with effect {effect}', fame, 'N/A')
        cursor.execute(sql, val)
        db.commit()
        cursor.close()
        db.close()
    ...
    async def remove_title_store(self, guild_id, author, fame, masculine_name, feminine_name):
        ...
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        cursor.execute(f"select fame, Character_Name from Player_Characters where Title = ? or Title = ?",
                       (masculine_name, feminine_name))
        characters = cursor.fetchall()
        if characters is not None:
            for character in characters:
                cursor.execute(f"UPDATE Player_Characters SET fame = ?, Title WHERE Character_Name = ?",
                               (character[0] - fame, None, character[1]))
        sql = "INSERT INTO A_Audit_All(Author, Character, Timestamp, Database_Changed, Modification, Amount, Reason) VALUES(?, ?, ?, ?, ?, ?, ?)"
    ...
    async def timesheet(self, guild_id, author, utc_offset, start_day, start_hours, start_minutes, end_day, end_hours,
                        end_minutes, change):
        db = sqlite3.connect(f"Pathparser_{guild_id}.sqlite")
        cursor = db.cursor()
        try:
                ...
                if change == 1:
                    ...
                    if start_day == end_day:
                        for col in time_columns:
                            ...
                            if start_minutes <= col_minutes <= end_minutes:
                        ...
                        print(f"ran query")
                        db.commit()
                        cursor.execute(f"UPDATE Player_Timecard SET utc_offset = ? WHERE Player_Name = ?",
                                       (utc_offset, author))
                        db.commit()
                    # START DAY IS NOT START DAY IN UTC TIME
                        ...
                        for col in time_columns:
                            col_minutes = time_to_minutes(col)
                            if start_minutes <= col_minutes <= 1470:
                                columns_to_nullify.append(f'"{col}" = 1')
                        # Build the SQL query dynamically
                        set_clause = ', '.join(columns_to_nullify)
                        print(set_clause)
    ...
    async def clear_timesheet(self, guild_id, change, day, author):
        ...
        async with aiosqlite.connect(f"Pathparser_{guild_id}.sqlite") as db:
            ...
            async with db.cursor() as cursor:
                try:
                    if change == 4:
                        await cursor.execute("""DELETE FROM Player_Timecard WHERE Player_Name = ?""", (author,))
                        await db.commit()
                    else:
                        time_columns = [
                            "01:00", "01:30", "02:00", "02:30", "03:00", "03:30", "04:00", "04:30",
                        ...
                        await cursor.execute(
                            f"SELECT UTC_Offset FROM player_Timecard WHERE player_name = ? and UTC_Offset is not Null",
                            (author,))
                        utc_offset_info = await cursor.fetchone()
                        utc_offset_zone = 'Universal' if utc_offset_info is None else utc_offset_info[0]
                        utc_offset_value = get_utc_offset(utc_offset_zone)
                        utc_offset_time = time_to_minutes(utc_offset_value)
                        if utc_offset_time == 0:
                            hour_clear_window_start = "1:00"
                            hour_clear_window_end = "24:30"
                            ...
                            """
                            # Execute the query
                            await cursor.execute(query, (author, day))
                            await db.commit()
                        elif utc_offset_time > 0:
                            day_clear_window_start = day - 1 if day > 1 else 7
                            day_clear_window_end = day if day < 7 else 1
                            columns_to_nullify = []
                            for col in time_columns:
                                col_minutes = time_to_minutes(col)
                                if 1470 - utc_offset_time <= col_minutes <= 1470:
                                    columns_to_nullify.append(f'"{col}" = NULL')
                            # Build the SQL query dynamically
                            set_clause = ', '.join(columns_to_nullify)
                            query = f"""
                                ...
                                WHERE Player_Name = ? AND Day = ?;
                            """
                            await cursor.execute(query, (author, day_clear_window_start))
                            await db.commit()
                            columns_to_nullify = []
                            for col in time_columns:
                                col_minutes = time_to_minutes(col)
                                if col_minutes >= 0 or col_minutes <= utc_offset_time:
                                    columns_to_nullify.append(f'"{col}" = NULL')
                            # Build the SQL query dynamically
                            set_clause = ', '.join(columns_to_nullify)
                            query = f"""
                                ...
                                WHERE Player_Name = ? AND Day = ?;
                            """
                            await cursor.execute(query, (author, day_clear_window_end))
                            await db.commit()
                        else:
                            ...
                            for col in time_columns:
                                col_minutes = time_to_minutes(col)
                                if abs(utc_offset_time) <= col_minutes <= 1470:
                                    columns_to_nullify.append(f'"{col}" = NULL')
                            # Build the SQL query dynamically
                            set_clause = ', '.join(columns_to_nullify)
                            query = f"""
    ...
    async def group_join(self, guild_id, group_id, author, character_name, choice):
        ...
        cursor = db.cursor()
        if choice == 1:
            cursor.execute(f"INSERT INTO Sessions_Presign(Group_ID, player_name, Character_Name) VALUES (?, ?, ?)",
                           (group_id, author, character_name))
        else:
            cursor.execute(f"Delete from Sessions_Presign where group_id = ? and character_name = ?",
    ...
    async def fix_dst(self, guild_id):
        async with aiosqlite.connect(f"Pathparser_{guild_id}.sqlite") as db:
            # Enable foreign key constraints
            await db.execute("PRAGMA foreign_keys = ON;")
            async with db.cursor() as cursor:
                try:
                    await cursor.execute("SELECT distinct(UTC_Offset) FROM Player_Timecard")
                    utc_zones = await cursor.fetchall()

                    for utc_zone in utc_zones:
                        zone_name = utc_zone[0]  # Assuming this is a valid timezone string, e.g., 'America/New_York'

                        if can_observe_dst(zone_name):  # Check if DST can be observed
                            await cursor.execute(
                                "SELECT distinct(Player_Name) FROM Player_Timecard where UTC_Offset = ?", (zone_name,))
                            players_in_zone = await cursor.fetchall()

                            for player_in_zone in players_in_zone:
                                player_name = player_in_zone[0]

                                # Select the day and time values for the player
                                await cursor.execute("""SELECT Day, "01:00", "01:30", "02:00", "02:30", "03:00", "03:30", "04:00", "04:30",
                                                        "05:00", "05:30", "06:00", "06:30", "07:00", "07:30", "08:00", "08:30",
                                                        "09:00", "09:30", "10:00", "10:30", "11:00", "11:30", "12:00", "12:30",
                                                        "13:00", "13:30", "14:00", "14:30", "15:00", "15:30", "16:00", "16:30",
                                                        "17:00", "17:30", "18:00", "18:30", "19:00", "19:30", "20:00", "20:30",
                                                        "21:00", "21:30", "22:00", "22:30", "23:00", "23:30", "24:00", "24:30"
                                                            FROM Player_Timecard where Player_Name = ? 
                                                            order by day asc""", (player_name,))
                                days = await cursor.fetchall()

                                for day in days:
                                    day_data = list(day[1:])  # Get the time values as a list

                                    # Check if DST is currently active
                                    now = datetime.now(
                                        pytz.timezone(zone_name))  # Get current time in player's timezone
                                    is_dst = now.dst() != datetime.timedelta(0)  # Check if DST is in effect

                                    # Offset the times based on DST
                                    for i in range(len(day_data)):
                                        if day_data[i] == 1:  # If the value is 1 (available)
                                            if is_dst:
                                                day_data[i] = 1  # Keep it as 1 during DST
                                            else:
                                                day_data[i] = None  # Set to None if not during DST

                                    # Update the player's time data in the database
                                    await cursor.execute("""UPDATE Player_Timecard SET 
